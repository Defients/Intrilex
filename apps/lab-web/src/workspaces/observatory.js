...
export async function renderOpeningPatterns() {
  ...
  if (!idx) {
    try {
      const { loadTraceIndex, loadTraceData } = await import('../data-loader.js');
  ...
  const { loadTraceData } = await import('../data-loader.js');
  const traceFiles = await Promise.all(idx.records.map(r => loadTraceData(r.matchId)));
  return _renderOpeningPatternsFromTraces(idx.records, traceFiles);
}
...
export function renderMechanics() {
  ...
  if (rankFilterEl) rankFilterEl.onchange = e => { state.mechanicsRankFilter = e.target.value; import('../app.js').then(m => m.render()); };
  const evidenceFilterEl = document.querySelector('#mechanics-evidence-filter');
  if (evidenceFilterEl) evidenceFilterEl.onchange = e => { state.mechanicsEvidenceFilter = e.target.value; import('../app.js').then(m => m.render()); };
  ...
  if (minSelEl) minSelEl.onchange = e => { state.mechanicsMinSelections = Number(e.target.value); import('../app.js').then(m => m.render()); };
...
export function renderSynergies() {
  ...
  if (synMechFilterEl) synMechFilterEl.onchange = e => { state.synergiesMechanicFilter = e.target.value; import('../app.js').then(m => m.render()); };
  const synDirFilterEl = document.querySelector('#synergies-direction-filter');
  if (synDirFilterEl) synDirFilterEl.onchange = e => { state.synergiesDirectionFilter = e.target.value; import('../app.js').then(m => m.render()); };
  ...
  if (synMinCohortEl) synMinCohortEl.onchange = e => { state.synergiesMinCohort = Number(e.target.value); import('../app.js').then(m => m.render()); };
  ...
  if (motifOutcomeEl) motifOutcomeEl.onchange = e => { state.motifOutcomeFilter = e.target.value; import('../app.js').then(m => m.render()); };
  const motifClearEl = document.querySelector('#motif-node-clear');
  if (motifClearEl) motifClearEl.onclick = () => { state.motifNodeFilter = null; import('../app.js').then(m => m.render()); };
...
export function renderHistory() {
  ...
  if (selectedMatchId) {
    ...
    if (summary) {
      ...
      document.querySelector('#history-detail-back').onclick = () => { state.historySelectedMatch = null; import('../app.js').then(m => m.render()); };
  ...
  document.querySelector('#history-clear-matchid-filter')?.addEventListener('click', () => { state.historyFilterMatchIds = null; state.historyPage = 0; import('../app.js').then(m => m.render()); });
  ...
  document.querySelectorAll('[data-match-id]').forEach(row => row.onclick = () => { state.historySelectedMatch = row.dataset.matchId; import('../app.js').then(m => m.render()); });
}
