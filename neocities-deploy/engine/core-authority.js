...
function targetAcceptsBaseAce(target) {
    ...
    if (payload.kind === "primary")
        return payload.stackClass === "ordinary-effect";
...
export function resolveCoreAuthorityAction(input, actorId, action) {
    ...
    switch (action.kind) {
        ...
        case "core-resolve-effect": {
            ...
            if (action.effect.kind === "black-joker-board-lock")
                return fail("CORE_BOARD_LOCK", "Board Lock must be declared as a Quick Effect, not as a Mini-Turn effect");
        ...
        case "core-declare-primary": {
            ...
            if (action.action.kind === "core-resolve-rank-action" && action.action.action.kind === "wild-sovereignty") {
                ...
                if (wsAction.copiedAction.kind === "total-clear" && wsAction.discardCostCardId !== undefined) {
                    if (state.cards[wsAction.discardCostCardId]?.zone === `${actorId}_HAND`)
                        moveCard(state, wsAction.discardCostCardId, "GY");
                }
            }
        ...
        case "core-declare-king-counter": {
            ...
            if (payload?.kind === "response" && payload.responseKind === "board-lock-quick")
                return fail("CORE_COUNTER_TARGET", "King cannot counter Board Lock — only ⭐A authority may directly counter Board Lock");
        ...
        case "core-declare-board-lock-quick": {
            ...
            if (state.activePlayerId !== actorId || !["Start", "Action", "End"].includes(state.phase))
                return fail("CORE_QUICK_WINDOW", "Board Lock Quick requires the controller's own Full Turn");
            if (state.priority?.open === true && state.priority.order[state.priority.index] !== actorId)
                return fail("CORE_PRIORITY", `${actorId} does not hold priority`);
            // Open-state declaration: stack must be empty, no triggers waiting, no suspended children, no pending declarations.
            if (state.stack.length > 0)
                return fail("CORE_QUICK_WINDOW", "Board Lock requires an empty stack (open-state declaration)");
            if (state.triggerQueue.length > 0)
                return fail("CORE_QUICK_WINDOW", "Board Lock requires an empty trigger queue (open-state declaration)");
            if (state.suspendedStackItemIds.length > 0)
                return fail("CORE_QUICK_WINDOW", "Board Lock requires no suspended stack items (open-state declaration)");
            if (state.pendingDeclaration !== null)
                return fail("CORE_QUICK_WINDOW", "Board Lock requires no pending declaration (open-state declaration)");
            // Cannot declare while Board Lock is already active.
            if (state.metadata.boardLock?.turnsRemaining > 0)
                return fail("CORE_BOARD_LOCK", "Board Lock is already active");
            const sourceProblem = responseSourceProblem(state, actorId, action.sourceCardId, "BJ");
            if (sourceProblem)
                return fail("CORE_QUICK_SOURCE", sourceProblem);
        ...
        case "core-declare-king-spade-counter": {
            ...
            if (tp?.kind === "response" && tp.responseKind === "board-lock-quick")
                return fail("CORE_KING_SPADE_TARGET", "K♠ cannot counter Board Lock — only ⭐A authority may directly counter Board Lock");
        ...
        case "core-resolve-response-top": {
            ...
            if (payload.responseKind === "board-lock-quick") {
                ...
                events.push({ type: "BLACK_JOKER_BOARD_LOCK_RESOLVED", payload: { stackItemId: item.id, sourceCardIds: responseSources, boardLock: state.metadata.boardLock } });
                reopenCorePriority(state, item.controllerId);
                break;
            }
