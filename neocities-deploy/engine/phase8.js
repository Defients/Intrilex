...
export function phase8Runtime(state) {
    const existing = state.metadata.phase8;
    return {
...
function inHand(state, cardId, playerId) {
...
function validateUltraRecipe(state, actorId, sourceIds, recipe) {
    if (state.players[actorId]?.limits.ultraPlayedThisFT)
        return "Ultra limit already used this FT";
    if (!sourceIds.every((id) => inHand(state, id, actorId)))
        return "Every Ultra source must be controlled in hand";
    const colors = sourceIds.map((id) => colorOf(state.cards[id].identity));
    if (colors.some((color) => color === null))
        return "Jokers cannot satisfy color recipes";
    if (recipe === "3-black" && (sourceIds.length !== 3 || colors.some((color) => color !== "black")))
        return "3 Black requires exactly three black-suited cards";
    if (recipe === "3-red" && (sourceIds.length !== 3 || colors.some((color) => color !== "red")))
        return "3 Red requires exactly three red-suited cards";
    if (recipe === "2-black-2-red" && (sourceIds.length !== 4 || colors.filter((c) => c === "black").length !== 2 || colors.filter((c) => c === "red").length !== 2))
        return "2 Black + 2 Red requires exactly two of each color";
    return null;
}
function scoreCard(state, cardId, playerId) {
    const card = state.cards[cardId];
...
function effectDestination(state, cardId) {
    return state.cards[cardId]?.state.exileBound === true ? "EXILE" : "GY";
}
function captureVoltage(state, playerId, runtime) {
    const values = { "3": 0, "4": 0, "5": 0 };
    for (const id of state.players[playerId].pr) {
        const card = state.cards[id];
        const parsed = card ? parseIdentity(card.identity) : null;
        if (!card || card.controllerId !== playerId || card.state.tapped === true || !parsed || !(parsed.rank in values))
            continue;
        values[parsed.rank] += typeof card.state.pointValue === "number" ? card.state.pointValue : cardPointValue(card);
    }
...
function rankOf(identity) { return parseIdentity(identity)?.rank ?? identity; }
function suitOf(identity) { return parseIdentity(identity)?.suit ?? null; }
function anchorCount(state, playerId) {
    return state.players[playerId].er.filter((id) => {
        const card = state.cards[id];
        if (!card || card.controllerId !== playerId || card.state.tapped === true || card.state.faceDownTrap === true)
            return false;
        const rank = parseIdentity(card.identity)?.rank;
        return rank === "A" || rank === "9" || rank === "Q" || rank === "K" || card.state.anchor === true;
    }).length;
}
export function exhaustedWinner(state) {
...
export function resolvePhase8Action(input, actorId, action) {
    if (!input.players[actorId])
        return fail("PHASE8_PLAYER", `Unknown actor ${actorId}`);
    ...
    switch (action.kind) {
        case "resolve-ultra": {
            ...
            if (problem)
                return fail("ULTRA_ILLEGAL", problem);
            ...
            if (action.recipe === "3-black") {
                if (!action.scoreCardId || !action.castCardId || !action.exileCardId || new Set([action.scoreCardId, action.castCardId, action.exileCardId]).size !== 3 || ![action.scoreCardId, action.castCardId, action.exileCardId].every((id) => action.sourceCardIds.includes(id)))
                    return fail("ULTRA_ROLES", "3 Black requires three distinct declared role assignments");
                scoreCard(state, action.scoreCardId, actorId);
                const cast = state.cards[action.castCardId];
                if (parseIdentity(cast.identity)?.rank === "10")
                    markExileBound(cast);
                ...
                if (!action.targetStackItemId)
                    return fail("ULTRA_TARGET", "3 Red requires a pending counter target");
                const index = state.stack.findIndex((item) => item.id === action.targetStackItemId);
                if (index < 0)
                    return fail("ULTRA_TARGET", "3 Red target stack item does not exist");
                const target = state.stack[index];
                state.stack.splice(index, 1);
                for (const sourceId of target.sourceCardIds)
                    if (state.cards[sourceId]?.zone === "ON_STACK")
                        moveCard(state, sourceId, target.sourceDestination);
                for (const sourceId of action.sourceCardIds)
                    moveCard(state, sourceId, "GY");
                const bottom = state.zones.gy.at(0);
                if (bottom)
                    moveCard(state, bottom, `${actorId}_HAND`, actorId);
                ...
                if (action.branch === "draw-two") {
                    ...
                    if (action.rummageCardId && state.cards[action.rummageCardId]?.zone === "EXILE")
        ...
        case "begin-rank10-resolution": {
            ...
            if (!card || card.controllerId !== actorId || parseIdentity(card.identity)?.rank !== "10")
                return fail("RANK10_SOURCE", "Rank-10 wrapper requires a controlled Ten");
            if (state.players[actorId].limits.rank10PlayedThisFT)
                return fail("RANK10_LIMIT", "Rank-10 effect limit already used this FT");
            ...
            if (action.destinationAfterResolution)
                moveCard(state, action.cardId, action.destinationAfterResolution);
            break;
        }
        case "capture-voltage": {
            const playerId = action.playerId;
            if (!state.players[playerId])
                return fail("VOLTAGE_PLAYER", `Unknown player ${playerId}`);
        ...
        case "resolve-voltage": {
            ...
            if (!snapshot)
                return fail("VOLTAGE_SNAPSHOT", "Voltage requires a captured Start snapshot");
            const used = runtime.voltageUsedThisFT[playerId] ?? { "3": false, "4": false, "5": false };
            if (used[action.rank])
                return fail("VOLTAGE_LIMIT", `Rank ${action.rank} Voltage already resolved this FT`);
            const threshold = Number(action.rank);
            const value = action.rank === "3" ? snapshot.rank3 : action.rank === "4" ? snapshot.rank4 : snapshot.rank5;
            if (value < threshold)
                return fail("VOLTAGE_THRESHOLD", `Snapshot ${value} does not meet threshold ${threshold}`);
            ...
            if (action.rank === "3") {
                ...
                if (chosen) {
                    if (action.disposition === "points")
                        scoreCard(state, chosen, playerId);
                    else if (action.disposition === "hand")
                        moveCard(state, chosen, `${playerId}_HAND`, playerId);
                    else
                        moveCard(state, chosen, effectDestination(state, chosen));
                    for (const id of revealed.filter((x) => x !== chosen))
                ...
                if (!top) {
                    events.push({ type: "VOLTAGE_4_RESOLVED", payload: { playerId, topCardId: null, result: "no-card" } });
                    break;
                }
                ...
                if (rankCorrect) {
                    if (action.rankMatchDisposition === "points")
                        scoreCard(state, top, playerId);
                    else
                        moveCard(state, top, effectDestination(state, top));
                    result = `rank-${action.rankMatchDisposition}`;
                }
                else if (suitCorrect) {
                    moveCard(state, top, `${playerId}_HAND`, playerId);
                    result = "suit-draw";
                }
                ...
                if (action.branch === "gy-bottom") {
                    ...
                    if (bottom)
                        moveCard(state, bottom, `${playerId}_HAND`, playerId);
                    events.push({ type: "VOLTAGE_5_RESOLVED", payload: { playerId, branch: action.branch, cardId: bottom ?? null } });
                }
                else {
                    const first = state.zones.dp[0];
                    if (first)
                        moveCard(state, first, `${playerId}_HAND`, playerId);
                    if (action.discardCardId && state.cards[action.discardCardId]?.zone === `${playerId}_HAND`)
                        moveCard(state, action.discardCardId, "GY");
                    const second = state.zones.dp[0];
                    if (second)
                        moveCard(state, second, `${playerId}_HAND`, playerId);
        ...
        case "begin-start-checkpoint": {
            const playerId = action.playerId;
            if (!state.players[playerId])
                return fail("START_PLAYER", `Unknown player ${playerId}`);
            state.phase = "Start";
        ...
        case "recover-exhausted": {
            ...
            for (const cardId of action.cardIds) {
                const card = state.cards[cardId];
                if (!card || (card.zone !== "EXILE" && card.zone !== "GY"))
                    return fail("EXHAUSTED_RECOVERY_SOURCE", `${cardId} is not recoverable from Exile or GY`);
                moveCard(state, cardId, "DP");
                moved.push(cardId);
            }
            if (moved.length > 0 && runtime.exhausted !== null)
                runtime.exhausted = null;
        ...
        case "process-end-phase": {
            ...
            if (runtime.boardLock && runtime.boardLock.activationFullTurnSequence !== state.fullTurnSequence) {
                ...
                if (runtime.boardLock.remaining <= 0) {
                    runtime.boardLock = null;
                    events.push({ type: "BOARD_LOCK_ENDED", payload: { step: 2 } });
                }
            }
