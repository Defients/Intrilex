...
export function phase15Runtime(state) {
    const current = state.metadata.phase15;
    return {
...
function canonicalIdentitySet(state) {
    const byIdentity = new Map();
    for (const cardId of state.zones.dp) {
        const identity = state.cards[cardId]?.identity;
        if (identity !== undefined)
            byIdentity.set(identity, cardId);
    }
    return byIdentity;
}
function categoryPriority(seedOrder, category) {
    if (seedOrder.length === 0)
        return [];
    const offset = (category - 1) % seedOrder.length;
    return [...seedOrder.slice(offset), ...seedOrder.slice(0, offset)];
}
function isCategoryIdentity(identity, category) {
    const parsed = parseIdentity(identity);
    if (!parsed)
        return false;
    if (category === 1)
        return parsed.rank === "A" && (parsed.suit === "♦" || parsed.suit === "♥");
    if (category === 2)
        return parsed.rank === "2";
    if (category === 3)
        return ["3", "4", "5", "6", "7"].includes(parsed.rank) && (parsed.suit === "♦" || parsed.suit === "♥");
    return ["J", "Q", "K"].includes(parsed.rank) && (parsed.suit === "♦" || parsed.suit === "♥");
}
function identityRequiredByCategoriesOneToFour(identity) {
    return [1, 2, 3, 4].some((category) => isCategoryIdentity(identity, category));
}
export function validateTournamentSeedPool(pool, playerCount) {
    if (!Number.isInteger(playerCount) || playerCount < 2 || playerCount > 4)
        return "Tournament Seed supports two to four players";
    if (new Set(pool).size !== pool.length)
        return "High-Impact Pool identities must be unique";
    if (pool.length < playerCount * 3)
        return "High-Impact Pool requires at least three legal identities per player";
    for (const identity of pool) {
        if (parseIdentity(identity) === null)
            return `Unknown High-Impact identity ${identity}`;
        if (TOURNAMENT_SEED_BAN_IDENTITIES.includes(identity))
            return `${identity} is in the Ban Pile`;
        if (identityRequiredByCategoriesOneToFour(identity))
            return `${identity} is categorically required by starting-hand categories 1–4`;
    }
    return null;
}
export function validateTournamentSeedConfiguration(enabledOptionalModules, eventSheetId, approvedOptionalModules = []) {
    ...
    for (const module of normalized)
        if (alwaysDisabled.has(module))
            return `Tournament Seed disables ${module}`;
    const tournamentSeedOnly = new Set(["tournament-seed"]);
    const extra = normalized.filter((module) => !tournamentSeedOnly.has(module));
    if (extra.length === 0)
        return null;
    if (!eventSheetId)
        return "Tournament Seed optional-module overrides require an exact event sheet";
...
export function tournamentSeedScuttleLegality(state, actorId, sourceCardId, targetCardId) {
    const source = state.cards[sourceCardId];
    const target = state.cards[targetCardId];
    if (!source || !target)
        return { legal: false, reason: "Scuttle requires existing source and target cards" };
    if (source.zone !== `${actorId}_HAND`)
        return { legal: false, reason: "Scuttle source must be in the actor's hand" };
    if (!target.zone.endsWith("_PR") || target.controllerId === actorId)
        return { legal: false, reason: "Scuttle target must be an enemy PR card" };
    if (hasAegis(target))
        return { legal: false, reason: "Aegis blocks Scuttle" };
    if (hasOrdinaryScuttleImmunity(target))
        return { legal: false, reason: "Target has ordinary Scuttle immunity" };
...
function configure(input, action) {
    const state = canonicalClone(input);
    if (state.phase !== "Setup")
        return fail("TS_PHASE", "Tournament Seed configuration occurs only during Setup");
    if (state.stack.length > 0 || state.pendingDeclaration !== null || state.priority?.open === true)
        return fail("TS_PENDING", "Tournament Seed cannot configure while gameplay objects are pending");
    if (action.seedOrder.length < 2 || action.seedOrder.length > 4 || new Set(action.seedOrder).size !== action.seedOrder.length || action.seedOrder.some((id) => !state.players[id]))
        return fail("TS_PLAYERS", "Seed order must contain two to four unique existing players");
    ...
    if (configFailure)
        return fail("TS_CONFIG", configFailure);
    ...
    if (poolFailure)
        return fail("TS_POOL", poolFailure);
    const byIdentity = canonicalIdentitySet(state);
    const missingBan = TOURNAMENT_SEED_BAN_IDENTITIES.filter((identity) => !byIdentity.has(identity));
    if (missingBan.length > 0)
        return fail("TS_BAN_MISSING", `Ban Pile identities missing from DP: ${missingBan.join(", ")}`);
    const banPileCardIds = [];
    ...
    const runtime = {
        ...
        categoryPriority: Object.fromEntries([1, 2, 3, 4, 5].map((category) => [String(category), categoryPriority(action.seedOrder, category)])),
        highImpactRankings: {},
...
function selectCategory(input, action) {
    const state = canonicalClone(input);
    const runtime = phase15Runtime(state);
    if (!runtime.enabled || runtime.status !== "selecting")
        return fail("TS_STATUS", "Tournament Seed selection is not active");
    if (action.category < 1 || action.category > 4)
        return fail("TS_CATEGORY", "Direct category selection supports categories 1–4");
    const priority = runtime.categoryPriority[String(action.category)] ?? [];
    if (!priority.includes(action.playerId))
        return fail("TS_PLAYER", "Player is not in category priority");
    const key = `category${action.category}`;
    if (runtime.selectionsByPlayer[action.playerId]?.[key] !== undefined)
        return fail("TS_SELECTED", `${action.playerId} already selected category ${action.category}`);
    const card = state.cards[action.cardId];
    if (!card || card.zone !== "DP")
        return fail("TS_CARD", "Selected card must remain in DP");
    if (!isCategoryIdentity(card.identity, action.category))
        return fail("TS_IDENTITY", `${card.identity} is illegal for category ${action.category}`);
...
function resolveHighImpact(input, action) {
    const state = canonicalClone(input);
    const runtime = phase15Runtime(state);
    if (!runtime.enabled || runtime.status !== "selecting")
        return fail("TS_STATUS", "Tournament Seed selection is not active");
    const priority = runtime.categoryPriority["5"] ?? [];
    if (priority.some((playerId) => !Array.isArray(action.rankingsByPlayer[playerId]) || action.rankingsByPlayer[playerId].length < 3))
        return fail("TS_RANKINGS", "Every player must rank at least three High-Impact identities");
    const byIdentity = canonicalIdentitySet(state);
    const assignments = {};
    const used = new Set();
    for (const playerId of priority) {
        const rankings = action.rankingsByPlayer[playerId];
        if (new Set(rankings).size !== rankings.length || rankings.some((identity) => !runtime.activeHighImpactPool.includes(identity)))
            return fail("TS_RANKINGS", `${playerId} rankings must be distinct identities in the active High-Impact Pool`);
        ...
        if (!selectedIdentity) {
            ...
            if (!fallback || fallback.length < 3 || new Set(fallback).size !== fallback.length || fallback.some((identity) => !runtime.activeHighImpactPool.includes(identity)))
                return fail("TS_FALLBACK", `${playerId} requires three legal fallback rankings`);
            selectedIdentity = fallback.find((identity) => !used.has(identity) && byIdentity.has(identity));
        }
        if (!selectedIdentity)
            return fail("TS_EXHAUSTED", `No legal High-Impact identity remains for ${playerId}`);
...
function finalizeSetup(input) {
    ...
    for (const playerId of runtime.seedOrder) {
        ...
        if ([selected.category1, selected.category2, selected.category3, selected.category4, selected.category5].some((id) => id === undefined))
            return fail("TS_INCOMPLETE", `${playerId} is missing a category 1–5 selection`);
    }
    ...
    for (const playerId of runtime.seedOrder) {
        ...
        if (!cardId)
            return fail("TS_DP", "DP exhausted before random sixth cards were dealt");
        moveCard(state, cardId, `${playerId}_HAND`, playerId);
    ...
    if (state.zones.dp.length < swapCount)
        return fail("TS_DP", "DP exhausted before Swap Bar construction");
    const swapCardIds = state.zones.dp.slice(0, swapCount);
    for (const cardId of swapCardIds)
    ...
    runtime.status = "complete";
    const randomSixthByPlayer = Object.fromEntries(runtime.seedOrder.map((id) => [id, runtime.selectionsByPlayer[id].category6]));
...
export function resolvePhase15Action(input, actorId, action) {
    if (!input.players[actorId])
        return fail("TS_ACTOR", `Unknown actor ${actorId}`);
    switch (action.kind) {
        ...
        case "validate-tournament-scuttle": {
            ...
            if (!result.legal)
                return fail("TS_SCUTTLE", result.reason ?? "Illegal Tournament Seed Scuttle");
            const state = canonicalClone(input);
...
//# sourceMappingURL=phase15.js.map