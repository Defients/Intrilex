...
export function phase12Runtime(state) {
    const current = state.metadata.phase12;
    return {
...
function saveRuntime(state, runtime) { state.metadata.phase12 = runtime; }
export function selectedBattleRealmSpec(state, playerId) {
    return phase12Runtime(state).selectedSpecByPlayer[playerId] ?? null;
}
export function qualifyingQueenIds(state, playerId) {
...
function unattached(cardState) {
    return cardState.attachedByJackId === undefined && cardState.attachmentGraph === undefined;
}
export function beautyMarriageBonus(state, playerId) {
    if (selectedBattleRealmSpec(state, playerId) !== "Beauty")
        return 0;
    const player = state.players[playerId];
    if (!player)
        return 0;
    const cards = [...player.pr, ...player.er].map((id) => state.cards[id]).filter((card) => card !== undefined && card.controllerId === playerId && card.state.tapped !== true && card.state.faceDownTrap !== true && unattached(card.state));
    const bonusBySuit = { "♣": 6, "♦": 7, "♥": 8, "♠": 9 };
    let total = 0;
    for (const suit of ["♣", "♦", "♥", "♠"]) {
        ...
        if (hasKing && hasQueen)
            total += bonusBySuit[suit];
    }
    return total;
}
export function battleRealmContinuousBonus(state, playerId) {
...
export function deriveBattleRealmSecuredPoints(state, playerId) {
    return deriveSecuredPoints(state, playerId);
}
function useSignature(runtime, playerId, expectedSpec, fullTurnSequence) {
    if (runtime.selectedSpecByPlayer[playerId] !== expectedSpec)
        return `${playerId} does not have ${expectedSpec}`;
    const remaining = runtime.signatureUsesRemaining[playerId] ?? 0;
    if (remaining <= 0)
        return `${expectedSpec} Signature has no uses remaining`;
    if (expectedSpec === "Beauty" && runtime.beautyExtraLuckyUsedOnFullTurn[playerId] === fullTurnSequence)
        return "Beauty Extra Lucky may be used at most once per FT";
    runtime.signatureUsesRemaining[playerId] = remaining - 1;
    if (expectedSpec === "Beauty")
        runtime.beautyExtraLuckyUsedOnFullTurn[playerId] = fullTurnSequence;
    return null;
}
function useUltimate(runtime, playerId, expectedSpec) {
    if (runtime.selectedSpecByPlayer[playerId] !== expectedSpec)
        return `${playerId} does not have ${expectedSpec}`;
    if (runtime.ultimateUsedByPlayer[playerId] === true)
        return `${expectedSpec} Ultimate has already been used`;
    runtime.ultimateUsedByPlayer[playerId] = true;
    return null;
}
function ensureEnemy(state, source, target) {
    if (!state.players[source] || !state.players[target])
...
export function resolvePhase12Action(input, actorId, action) {
    if (!input.players[actorId])
        return fail("BATTLEREALM_PLAYER", `Unknown actor ${actorId}`);
    ...
    switch (action.kind) {
        case "configure-battle-realm": {
            ...
            if (runtime.enabled || Object.keys(runtime.selectedSpecByPlayer).length > 0)
                return fail("BATTLEREALM_LOCKED", "Specs cannot change after BattleRealm configuration");
            if (playerIds.some((id) => action.specs[id] === undefined))
                return fail("BATTLEREALM_SPEC", "Every player must select exactly one Spec");
            for (const [id, spec] of Object.entries(action.specs)) {
                if (!state.players[id])
                    return fail("BATTLEREALM_SPEC", `Unknown player ${id}`);
                if (!(spec in BATTLE_REALM_REGISTRY))
                    return fail("BATTLEREALM_SPEC", `Unknown Spec ${spec}`);
        ...
        case "recalculate-continuous-bonuses": {
            if (!runtime.enabled)
                return fail("BATTLEREALM_DISABLED", "BattleRealm is not enabled");
            for (const id of Object.keys(state.players).sort())
                runtime.continuousBonusByPlayer[id] = battleRealmContinuousBonus(state, id);
        ...
        case "register-limited-play": {
            ...
            if (action.playClass === "ultra") {
                if (player.limits.ultraPlayedThisFT)
                    return fail("BATTLEREALM_ULTRA_CAP", "BattleRealm cannot permit more than one Ultra per player per FT");
                player.limits.ultraPlayedThisFT = true;
            }
            else {
                if (player.limits.rank10PlayedThisFT)
                    return fail("BATTLEREALM_RANK10_CAP", "BattleRealm cannot permit more than one Rank-10 effect per player per FT");
        ...
        case "courageous-assault": {
            ...
            if (reason)
                return fail("BATTLEREALM_SIGNATURE", reason);
            const target = state.cards[action.targetCardId];
            if (!target || (!target.zone.endsWith("_PR") && !target.zone.endsWith("_ER")))
                return fail("BATTLEREALM_TARGET", "Courageous Assault requires an enemy OTT target");
            const relationError = ensureEnemy(state, actorId, target.controllerId);
            if (relationError)
                return fail("BATTLEREALM_TARGET", relationError);
            if (hasAegis(target))
                return fail("AEGIS_BLOCK", "Courageous Assault never bypasses Aegis");
        ...
        case "extra-lucky": {
            ...
            if (!drawn || drawn.controllerId !== actorId || drawn.zone !== `${actorId}_HAND`)
                return fail("BATTLEREALM_DRAW", "Extra Lucky requires the just-drawn card in your hand");
            moveCard(state, drawn.id, "GY");
            let replacementCardId = null;
            if (action.sourcePosition === "dp-top" && state.zones.dp.length > 0)
                replacementCardId = state.zones.dp[0];
            if (action.sourcePosition === "dp-bottom" && state.zones.dp.length > 0)
                replacementCardId = state.zones.dp.at(-1);
            if (replacementCardId)
                moveCard(state, replacementCardId, `${actorId}_HAND`, actorId);
        ...
        case "mastermind": {
            ...
            if (canonicalClone(available).sort().join("|") !== canonicalClone(action.inspectedCardIds).sort().join("|"))
                return fail("BATTLEREALM_MASTERMIND", "Inspected cards must be exactly the current top group");
            if (action.drawCardIds.length !== Math.min(2, available.length) || action.drawCardIds.some((id) => !available.includes(id)))
                return fail("BATTLEREALM_MASTERMIND", "Draw exactly two available inspected cards when possible");
            const remaining = available.filter((id) => !action.drawCardIds.includes(id));
            if (canonicalClone(remaining).sort().join("|") !== canonicalClone(action.returnOrder).sort().join("|"))
                return fail("BATTLEREALM_MASTERMIND", "Return order must contain every undrawn inspected card exactly once");
            if (action.viewerId !== undefined && ensureEnemy(state, actorId, action.viewerId))
                return fail("BATTLEREALM_MASTERMIND", "The selected viewer must be an Enemy");
            ...
            for (const id of action.drawCardIds)
                moveCard(state, id, `${actorId}_HAND`, actorId);
            for (const id of [...action.returnOrder].reverse()) {
        ...
        case "counter-distortion": {
            if (runtime.selectedSpecByPlayer[action.defendingPlayerId] !== "Brilliance")
                return fail("BATTLEREALM_SPEC", "Counter Distortion requires Brilliance");
            if (runtime.brillianceCounterDistortionUsed[action.defendingPlayerId])
                return fail("BATTLEREALM_PASSIVE", "Counter Distortion has already been used");
            if (ensureEnemy(state, action.defendingPlayerId, action.jackControllerId))
                return fail("BATTLEREALM_TARGET", "Counter Distortion requires an enemy J Disrupt");
            let discardedJackId = null;
            if (action.discardJackId !== undefined) {
                const candidate = state.cards[action.discardJackId];
                if (!candidate || candidate.controllerId !== action.defendingPlayerId || candidate.zone !== `${action.defendingPlayerId}_HAND` || parseIdentity(candidate.identity)?.rank !== "J")
                    return fail("BATTLEREALM_PASSIVE", "Discard selection must be a Jack in the defending player's hand");
                moveCard(state, candidate.id, "GY");
                discardedJackId = candidate.id;
            }
            let drawCardId = null;
            if (state.zones.dp.length > 0) {
                drawCardId = state.zones.dp[0];
                moveCard(state, drawCardId, `${action.jackControllerId}_HAND`, action.jackControllerId);
            }
        ...
        case "goal-shock": {
            if (runtime.selectedSpecByPlayer[actorId] !== "Brilliance")
                return fail("BATTLEREALM_SPEC", "Goal Shock requires Brilliance");
            const changed = {};
            for (const id of action.enemyPlayerIds) {
                const reason = ensureEnemy(state, actorId, id);
                if (reason)
                    return fail("BATTLEREALM_TARGET", reason);
        ...
        case "hard-jack": {
            if (runtime.selectedSpecByPlayer[actorId] !== "Bravery")
                return fail("BATTLEREALM_SPEC", "Hard Jack requires Bravery");
            const jack = state.cards[action.jackCardId];
            const host = state.cards[action.hostCardId];
            if (!jack || !host || jack.controllerId !== actorId || host.controllerId !== actorId || host.state.attachedByJackId !== jack.id)
                return fail("BATTLEREALM_ATTACHMENT", "Hard Jack requires your valid PR Jack Attachment");
        ...
        case "harmonized-mimic": {
            ...
            if (reason)
                return fail("BATTLEREALM_ULTIMATE", reason);
            if (action.keepCardIds.length > 5 || action.drawnCardIds.some((id) => !state.cards[id]) || action.keepCardIds.some((id) => !action.drawnCardIds.includes(id)))
                return fail("BATTLEREALM_ULTIMATE", "Harmonized Mimic may keep up to five cards drawn by the mimicked ⭐6");
        ...
        case "chromatic-ten": {
            ...
            if (!card || card.controllerId !== actorId || card.zone !== `${actorId}_HAND` || card.identity !== "10♥")
                return fail("BATTLEREALM_ULTIMATE", "Chromatic Ten requires your physical 10♥ in hand");
            if (state.players[actorId].limits.rank10PlayedThisFT)
                return fail("BATTLEREALM_RANK10_CAP", "Chromatic Ten remains subject to the Rank-10 limit");
        ...
        case "preserve-intercepted-play": {
            ...
            if (!item)
                return fail("BATTLEREALM_STACK", "Intercepted generated play must remain pending");
            if (ensureEnemy(state, action.interceptorId, action.originalControllerId) !== null && action.interceptorId !== action.originalControllerId)
                return fail("BATTLEREALM_STACK", "Interceptor relation is invalid");
...
//# sourceMappingURL=phase12.js.map