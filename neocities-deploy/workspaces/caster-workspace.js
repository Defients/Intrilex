...
import { policyOptions } from '../router.js?v=3dca2dc8fde5';

// Lazy-loaded @intrilex/replay-caster (browser-bundleable subset).
let casterModule = null;
async function getCaster() {
  if (!casterModule) {
    casterModule = await import('../replay-caster/browser-entry.js?v=3dca2dc8fde5');
  }
  return casterModule;
}
...
async function getBrowserOllamaClient() {
  // The browser analytics-ai controller exposes an OllamaClient.
  // Lazy-loaded to avoid pulling it into the initial bundle.
  const mod = await import('../browser-analytics.js?v=3dca2dc8fde5');
  if (mod.OllamaClient) return new mod.OllamaClient({});
  throw new Error('OllamaClient not available in browser analytics module');
}
