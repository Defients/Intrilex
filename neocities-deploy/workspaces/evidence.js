...
import { ENGINE_VERSION, RULES_VERSION } from '../version.js?v=42162e3d88b3';
...
export function renderEvidence() {
  ...
  if (anomalyTypeEl) anomalyTypeEl.onchange = e => { state.anomalyTypeFilter = e.target.value; import('../app.js?v=42162e3d88b3').then(m => m.render()); };
  const anomalySevEl = document.querySelector('#anomaly-severity-filter');
  if (anomalySevEl) anomalySevEl.onchange = e => { state.anomalySeverityFilter = e.target.value; import('../app.js?v=42162e3d88b3').then(m => m.render()); };
