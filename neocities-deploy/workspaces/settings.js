...
import { app, esc, state, showToast, persistSetting } from '../state.js?v=4f30833b427f';
import { getAuthState, getProfile, signOut, subscribe } from '../play/network/auth-controller.js?v=4f30833b427f';
import { isSupabaseConfigured } from '../play/network/supabase-client.js?v=4f30833b427f';
import { validateMatchServerUrl } from '../play/network/match-server-config.js?v=4f30833b427f';
...
function wireSettingsActions(container) {
  ...
  if (hapticsToggle) {
    hapticsToggle.addEventListener('change', () => {
      ...
      try { import('../play/touch/haptics.js?v=4f30833b427f').then(m => m.setHapticsEnabled(state.haptics)); } catch { /* ignore */ }
  ...
  if (seasonalToggle) {
    seasonalToggle.addEventListener('change', () => {
      ...
      if (state.seasonalThemes) {
        import('../landing/seasonal-theme.js?v=4f30833b427f').then(m => m.applySeasonalTheme()).catch(() => {});
      } else {
  ...
  if (clearSavesBtn) {
    clearSavesBtn.addEventListener('click', async () => {
      ...
      try {
        const { isIndexedDBAvailable, listSaves, deleteSave } = await import('../play/persistence.js?v=4f30833b427f');
