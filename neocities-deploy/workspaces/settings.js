...
import { app, esc, state, showToast, persistSetting } from '../state.js?v=73653ac8207b';
import { getAuthState, getProfile, signOut, subscribe } from '../play/network/auth-controller.js?v=73653ac8207b';
import { isSupabaseConfigured } from '../play/network/supabase-client.js?v=73653ac8207b';
import { validateMatchServerUrl } from '../play/network/match-server-config.js?v=73653ac8207b';
...
function wireSettingsActions(container) {
  ...
  if (hapticsToggle) {
    hapticsToggle.addEventListener('change', () => {
      ...
      try { import('../play/touch/haptics.js?v=73653ac8207b').then(m => m.setHapticsEnabled(state.haptics)); } catch { /* ignore */ }
  ...
  if (seasonalToggle) {
    seasonalToggle.addEventListener('change', () => {
      ...
      if (state.seasonalThemes) {
        import('../landing/seasonal-theme.js?v=73653ac8207b').then(m => m.applySeasonalTheme()).catch(() => {});
      } else {
  ...
  if (clearSavesBtn) {
    clearSavesBtn.addEventListener('click', async () => {
      ...
      try {
        const { isIndexedDBAvailable, listSaves, deleteSave } = await import('../play/persistence.js?v=73653ac8207b');
