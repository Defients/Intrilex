...
import { state, app, esc, fmt, pct, short, definitionList } from '../state.js?v=3dca2dc8fde5';
import { barChart, heatmap, donutChart, sparkline, lineChart, stackedBarChart, chartTableAlternative, sankeyFlow } from '../chart-toolkit.js?v=3dca2dc8fde5';
// IRX-C06: Use rerender bus instead of dynamic import('../app.js?v=3dca2dc8fde5') to break backedge
...
export async function renderOpeningPatterns() {
  ...
  if (!idx) {
    try {
      const { loadTraceIndex, loadTraceData } = await import('../data-loader.js?v=3dca2dc8fde5');
  ...
  const { loadTraceData } = await import('../data-loader.js?v=3dca2dc8fde5');
  const traceFiles = await Promise.all(idx.records.map(r => loadTraceData(r.matchId)));
  return _renderOpeningPatternsFromTraces(idx.records, traceFiles);
}
