...
import { radarChart } from '../chart-toolkit.js?v=73b458295383';
...
export function renderRanks() {
  ...
  document.querySelectorAll('[data-rank]').forEach(button => button.onclick = () => { state.selectedRank = button.dataset.rank; state.anatomyTab = 'overall'; import('../app.js?v=73b458295383').then(m => m.render()); });
  const profileFilter = document.querySelector('#variant-profile-filter');
  if (profileFilter) profileFilter.onchange = () => { state.variantProfileFilter = profileFilter.value; import('../app.js?v=73b458295383').then(m => m.render()); };
  const originFilter = document.querySelector('#origin-filter');
  if (originFilter) originFilter.onchange = () => { state.originFilter = originFilter.value; import('../app.js?v=73b458295383').then(m => m.render()); };
  document.querySelectorAll('[data-anatomy-tab]').forEach(button => button.onclick = () => { state.anatomyTab = button.dataset.anatomyTab; import('../app.js?v=73b458295383').then(m => m.render()); });
