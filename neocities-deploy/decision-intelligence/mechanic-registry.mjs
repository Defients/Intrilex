...
// ── Taxonomy Dimensions ──
// Each telemetry tag belongs to exactly one dimension. The dimension determines
// how the tag is displayed and whether it appears in the canonical mechanics
// leaderboard or in a separate dimension view.
//
// Dimensions:
//   canonical-mechanic — A distinct gameplay mechanic with its own rules entry
//   action-family       — The primary classification of an action (e.g. 'score', 'swap-bar')
//   action-mode         — The mode/sub-type of an action (e.g. 'face-down', 'face-up')
//   rank-effect         — Rank-specific card effects (e.g. 'effect-three', 'rank10')
//   diagnostic          — Implementation/debug tags that are NOT gameplay mechanics
...
// Tags that are action families (the `action.family` field) but NOT canonical mechanics.
// These are structural/timing categories, not distinct gameplay mechanics.
const ACTION_FAMILY_TAGS = new Set([
  'phase', 'response-decline', 'private-choice', 'pass',
  'instant', 'quick', 'interrupt',
  'score', // action.family for scoring plays — the canonical mechanic is 'play-for-points'
]);

// Tags that are action modes (the `action.mode` field).
// These describe HOW an action is performed, not WHAT mechanic it is.
const ACTION_MODE_TAGS = new Set([
  'face-down', 'face-up', 'enter-action', 'decline', 'points', 'ordinary', 'top',
  'forced-mini-turn', 'exhausted-pass',
  'face-up-draw', // mode of draw action
  '♣', '♦', '♥', '♠',
]);

// Tags that are rank-specific effects. These are canonical mechanics that are
// further sub-classified by rank for the rank-effect dimension view.
// This includes rank identifiers (ace, jack, queen, king) and rank-specific
// effect variants (seven-topdeck, five-recycle, club-foundation, etc.).
const RANK_EFFECT_TAGS = new Set([
  ...
  // Rank identifier tags — used when a card of a specific rank is played
  'ace', 'jack', 'queen', 'king', 'nine',
  // Rank-specific effect variants observed in telemetry
...
// Tags that are purely diagnostic/implementation artifacts.
// Note: 'draw' is NOT here — it's a registered canonical mechanic.
// Note: 'score' is NOT here — it's an action-family tag.
...
/**
 * Classify a telemetry tag into its taxonomy dimension.
 * @param {string} tag - The telemetry tag to classify
 * @returns {'canonical-mechanic' | 'action-family' | 'action-mode' | 'rank-effect' | 'diagnostic'}
 */
export function classifyTagDimension(tag) {
  ...
  // If it's in the mechanic registry and not already classified, it's a canonical mechanic
  if (MECHANIC_REGISTRY[tag]) return 'canonical-mechanic';
  // Unknown tags go to diagnostic by default
  return 'diagnostic';
}

/**
 * Get an analytics entity definition for a tag, including its dimension.
 * @param {string} tag - The telemetry tag
 * @returns {{ id: string, label: string, dimension: string, description: string, source?: string, deprecated?: boolean }}
 */
export function analyticsEntityDefinition(tag) {
  ...
  if (registryEntry) {
    return {
      ...
      source: registryEntry.authorityRefs?.[0],
      deprecated: dimension === 'diagnostic',
    };
  }
  return {
    id: tag,
    label: tag,
    dimension,
    description: dimension === 'diagnostic'
      ? 'Diagnostic/implementation tag — not a gameplay mechanic.'
      : 'Unregistered telemetry tag — pending canonical classification.',
    deprecated: dimension === 'diagnostic',
  };
}

/**
 * Get the set of tags that should be excluded from synergy pair analysis
 * because they are aliases, parent/child, or mechanically inseparable.
 * @returns {Set<string>}
 */
export function synergyExcludedTags() {
  return new Set([
    ...DIAGNOSTIC_TAGS,
    ...ACTION_FAMILY_TAGS,
    'exhausted-pass', 'private-choice', 'effect-private-choice', 'anchor-private-choice',
  ]);
}

/**
 * Check if two tags are mechanically inseparable (e.g. parent and child,
 * aliases, or an action family paired with its own mode).
 * @param {string} a - First tag
...
export function areTagsInseparable(a, b) {
  ...
  // Rank10 variants are children of 'rank10'
  if ((a === 'rank10' && b.startsWith('rank10-')) || (b === 'rank10' && a.startsWith('rank10-'))) return true;
  // Effect variants and their parent effect family
  if (a.startsWith('effect-') && b.startsWith('effect-') && a !== b) {
    // Check if one is a prefix of the other (e.g. 'effect-private-choice' and 'effect-private-choice')
    return false;
  }
  return false;
}
