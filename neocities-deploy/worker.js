...
const engineModule = import('./engine/browser-entry.js?v=e2bd7e8507fa');
const autonomyModule = import('./autonomy-runtime.js?v=e2bd7e8507fa');
const analyticsModule = import('./browser-analytics.js?v=e2bd7e8507fa');
...
self.onmessage = async (event) => {
  ...
  if (type === 'run-counterfactual') {
    try {
      const { runCounterfactualBranch } = await import('./decision-intelligence.js?v=e2bd7e8507fa');
  ...
  if (type === 'run-paired-counterfactual') {
    try {
      const { runPairedCounterfactual } = await import('./decision-intelligence.js?v=e2bd7e8507fa');
      const cfg = event.data.config ?? {};
  ...
  if (type === 'get-legal-actions') {
    try {
      const { getCheckpointLegalActions } = await import('./decision-intelligence.js?v=e2bd7e8507fa');
  ...
  if (type === 'run-all-actions') {
    try {
      const { getCheckpointLegalActions, runCounterfactualBranch } = await import('./decision-intelligence.js?v=e2bd7e8507fa');
  ...
  if (type === 'run-diagnostics') {
    try {
      const { diagnosePolicy } = await import('./decision-intelligence.js?v=e2bd7e8507fa');
      const summaries = JSON.parse(event.data.summariesJson ?? '[]');
