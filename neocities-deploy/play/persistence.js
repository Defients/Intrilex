...
export async function importSave(jsonString) {
  ...
  if (data.version === 1) {
    const { canMigrateSave, migrateSave } = await import('./save-integrity.js?v=9ea1c2f9e91d');
    const migration = canMigrateSave(data);
    if (migration.canMigrate) {
      const engineModule = await import('../engine/browser-entry.js?v=9ea1c2f9e91d');
      const autonomyModule = await import('../autonomy-runtime.js?v=9ea1c2f9e91d');
  ...
  const { validateSaveEnvelope, canMigrateSave: canMigrate, migrateSave: migrate } = await import('./save-integrity.js?v=9ea1c2f9e91d');
