...
export async function importSave(jsonString) {
  ...
  if (data.version === 1) {
    const { canMigrateSave, migrateSave } = await import('./save-integrity.js?v=75c53031ef21');
    const migration = canMigrateSave(data);
    if (migration.canMigrate) {
      const engineModule = await import('../engine/browser-entry.js?v=75c53031ef21');
      const autonomyModule = await import('../autonomy-runtime.js?v=75c53031ef21');
  ...
  const { validateSaveEnvelope, canMigrateSave: canMigrate, migrateSave: migrate } = await import('./save-integrity.js?v=75c53031ef21');
