...
test('Defect version: all version strings updated to current release', async () => {
  ...
  assert.equal(pkg.version, '1.0.0', 'package.json version should be 1.0.0');
  const { LAB_VERSION, REPLAY_DATA_VERSION } = await import('../packages/simulation-runtime/src/runtime.mjs');
  // LAB_VERSION is now the product version (1.0.0), separate from REPLAY_DATA_VERSION.
  assert.equal(LAB_VERSION, '1.0.0', 'LAB_VERSION should be the product version 1.0.0');
