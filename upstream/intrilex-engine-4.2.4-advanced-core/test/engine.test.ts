...
test("4.1.3 Jack Disrupt records the action type, draws one, and never counters the root", async () => {
  ...
  const runtime = state.metadata.autonomy as { disruptedActionTypesByPlayer?: Record<string, string[]> };
...
test("4.2.2 root effects are sealed on stack and every enumerated response is engine-accepted without command-vault leakage", async () => {
  ...
  for (const expected of ["pass-priority:pass", "counter:ace-base", "instant:eight-spade-free-scuttle", "instant:nine-tap", "disrupt:jack"]) assert.equal(families.has(expected), true, expected);
...
test("4.2.2 Jack Disrupt resolves without negating the root and records the action restriction", async () => {
  ...
  for (let i = 0; i < 16 && (state.stack.length > 0 || state.priority?.open); i++) {
    ...
    if (d.status === "PLAYER_DECISION_REQUIRED") { const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="pass-priority"); if(!pass) break; const r=engine.execute(state,pass.command); types.push(...r.events.map((event)=>event.type)); state=r.state; }
  }
...
test("4.2.2 audited Quick and Instant responses resolve through stack authority", async () => {
  ...
  for(let i=0;i<10&&(state.stack.length>0||state.priority?.open);i++){d=advanceCoreToDecision(state);state=d.state;if(d.status==="PLAYER_DECISION_REQUIRED"){const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="pass-priority");if(!pass) break;state=engine.execute(state,pass.command).state;}}
...
test("4.2.3 Three enforces exact discard-to-two and permits taking up to two presented cards", async () => {
  ...
  const settleToChoice = (start: any) => {
    ...
    for (let i=0;i<20;i++) {
      ...
      if (d.status === "PLAYER_DECISION_REQUIRED") {
        const pass = d.legalActionFrame!.actions.find((entry) => entry.family === "pass-priority");
        if (!pass) throw new Error("Expected priority pass");
...
test("4.2.3 Six choices are chooser-private and public projections expose no stable hidden IDs", async () => {
  ...
  for(let i=0;i<12;i++){d=advanceCoreToDecision(state);state=d.state;if((state.metadata.coreAuthority as any)?.privateChoice)break;const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="pass-priority");assert.ok(pass);state=engine.execute(state,pass.command).state;}
...
test("4.2.4 Stack Theft transfers resolution authority but charges only its two printed skips", async () => {
  ...
  for(let i=0;i<20&&state.stack.length;i++){
    if(state.priority?.open===true){d=advanceCoreToDecision(state);state=d.state;const pass=d.legalActionFrame!.actions.find((x)=>x.family==="pass-priority");assert.ok(pass);state=engine.execute(state,pass.command).state;}
...
test("4.2.4 3 Red Ultra counters a root and preserves its bottom-GY draw rider", async () => {
  ...
  assert.equal(state.stack.length,0);assert.equal(state.players.P2!.limits.ultraPlayedThisFT,true);assert.equal(ultraSources.filter((id)=>state.players.P2!.hand.includes(id)).length,1);assert.equal(state.cards[rootSourceId]!.zone,"GY");assert.equal(state.phase,"End");
});
...
test("4.2.4 incomplete advanced declarations fail closed with exact rewind", async () => {
  ...
  let d=advanceCoreToDecision(state); state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  for(const identity of ["2♣","2♦","10♣"]) moveCard(state,by[identity]!,"P1_HAND","P1");
...
test("4.2.4 Super Eight prunes object Aegis and Super Four preserves tap state while replacing Aegis", async () => {
  ...
  for(let i=0;i<20&&state.stack.length;i++){
    if(state.priority?.open){d=advanceCoreToDecision(state);state=d.state;const pass=d.legalActionFrame!.actions.find((x)=>x.family==="pass-priority");assert.ok(pass);state=engine.execute(state,pass.command).state;}
