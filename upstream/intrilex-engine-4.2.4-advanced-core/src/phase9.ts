...
import type { CardId, EngineState, FirstContactAction, FirstContactDeclarationClass, FirstContactMiniTurnActionType, FirstContactOptionalModule, FirstContactPrimaryAction, FirstContactPrivateChoiceState, FirstContactPrivateChoiceSubmission, FirstContactStackClass, FirstContactStackPayload, PlayerId, StackItem, StackItemId, TriggerEvent, Visibility, ZoneName } from "./types.js";
...
export const FIRST_CONTACT_PROFILE: FirstContactProfile = Object.freeze({
  id: "first-contact",
  goal: 15,
  playerCount: 2,
  miniTurnsPerCompletedFullTurn: 1,
...
export interface Phase9EventSpec { type: string; payload: Record<string, unknown>; visibility?: Visibility; }
export type Phase9Resolution =
  | { ok: true; state: EngineState; events: Phase9EventSpec[] }
  | { ok: false; code: string; message: string; details?: unknown };

function fail(code: string, message: string, details?: unknown): Phase9Resolution {
  return details === undefined ? { ok: false, code, message } : { ok: false, code, message, details };
}

function runtimeFrom(overrideId?: string): FirstContactRuntime {
  return {
    active: true,
    profileId: "first-contact",
...
export function isFirstContact(state: EngineState): boolean {
  return (state.metadata.firstContact as Partial<FirstContactRuntime> | undefined)?.active === true;
}

export function validateFirstContactConfiguration(enabledModules: OptionalModule[], teachingOverrideId?: string): string | null {
  const unique = [...new Set(enabledModules)];
  if (unique.length === 0) return null;
  if (teachingOverrideId?.trim()) return null;
  return `First Contact cannot be combined with optional modules: ${unique.join(", ")}`;
}

export function validateFirstContactDeclaration(
  declarationClass: FirstContactDeclarationClass,
  rank?: string,
  effectKey?: string
): string | null {
  if (declarationClass !== "generic-effect") return `First Contact disables ${declarationClass}`;
  if (!rank || !FIRST_CONTACT_PROFILE.allowedGenericRanks.includes(rank)) return `Rank ${rank ?? "<missing>"} has no enabled generic First Contact effect`;
  if (effectKey?.includes("spade") || effectKey?.includes("suit") || effectKey?.includes("ultra") || effectKey?.includes("super")) return "Suit-specific and advanced effect keys are disabled in First Contact";
  return null;
}

function normalizeSetup(state: EngineState, playerIds: [PlayerId, PlayerId], teachingOverrideId?: string): string | null {
  if (state.turnOrder.length !== 2 || new Set(state.turnOrder).size !== 2) return "First Contact requires exactly two players";
  if (!playerIds.every((id) => state.players[id] !== undefined) || new Set(playerIds).size !== 2) return "First Contact setup requires two distinct known players";
  if (state.zones.swapBar.length > 0) return "First Contact has no Swap Bar";
  if (state.zones.exile.length > 0) return "First Contact setup cannot begin with cards in Exile";
...
function autoUntap(state: EngineState, playerId: PlayerId): CardId[] {
  const player = state.players[playerId];
  if (!player) return [];
  const untapped: CardId[] = [];
  for (const cardId of [...player.pr, ...player.er]) {
    const card = state.cards[cardId];
    if (!card || card.controllerId !== playerId || card.state.tapped !== true) continue;
...
function routeDestination(state: EngineState, cardId: CardId, requested: ZoneName, controllerId?: PlayerId): ZoneName {
  const card = state.cards[cardId];
  if (!card) throw new Error(`Unknown card ${cardId}`);
  const profileDestination: ZoneName = requested === "EXILE" ? "GY" : requested;
  if (profileDestination === "GY" && card.state.exileBound === true) delete card.state.exileBound;
  return moveCard(state, cardId, profileDestination, controllerId);
}

interface AutonomyRuntimeState {
  profileId?: string;
  ...
  disruptedActionTypesByPlayer?: Record<PlayerId, FirstContactMiniTurnActionType[]>;
  privateChoice?: FirstContactPrivateChoiceState | null;
}

function autonomyRuntime(state: EngineState): AutonomyRuntimeState | null {
  const runtime = state.metadata.autonomy;
  return typeof runtime === "object" && runtime !== null ? runtime as AutonomyRuntimeState : null;
}

function choiceTokenCore(choice: Omit<FirstContactPrivateChoiceState, "token">): unknown {
  return {
...
function createPrivateChoice(state: EngineState, input: Omit<FirstContactPrivateChoiceState, "schemaVersion" | "choiceId" | "token" | "createdRevision" | "optionsHash">): FirstContactPrivateChoiceState {
  ...
  return { ...core, token: hashCanonical(choiceTokenCore({ ...core, token: "" } as FirstContactPrivateChoiceState)) };
}

function activePrivateChoice(state: EngineState): FirstContactPrivateChoiceState | null {
  const choice = autonomyRuntime(state)?.privateChoice;
  return choice && typeof choice === "object" ? choice : null;
}

function setPrivateChoice(state: EngineState, choice: FirstContactPrivateChoiceState): void {
  autonomyRuntime(state)!.privateChoice = choice;
}

function clearPrivateChoice(state: EngineState): void {
  autonomyRuntime(state)!.privateChoice = null;
}

function validatePrivateChoiceSubmission(state: EngineState, actorId: PlayerId, token: string, submission: FirstContactPrivateChoiceSubmission): FirstContactPrivateChoiceState | string {
  if (!isPrivateChoiceProfile(state)) return "First Contact Private Choice Authority profile is not active";
  const choice = activePrivateChoice(state);
  if (!choice) return "No private choice is pending";
  if (choice.chooserId !== actorId) return `${actorId} is not authorized for ${choice.choiceId}`;
  if (choice.token !== token) return "Private choice token is stale or invalid";
  const expected = hashCanonical(choiceTokenCore({ ...choice, token: "" } as FirstContactPrivateChoiceState));
  if (expected !== choice.token || hashCanonical([...choice.optionCardIds].sort()) !== choice.optionsHash) return "Private choice seal verification failed";
  if (choice.kind !== submission.kind) return `Choice kind mismatch: expected ${choice.kind}`;
  const selected = submission.selectedCardIds;
  if (new Set(selected).size !== selected.length) return "Private choice selections must be unique";
  if (selected.length < choice.minSelections || selected.length > choice.maxSelections) return `Private choice requires ${choice.minSelections}-${choice.maxSelections} selections`;
  if (selected.some((id) => !choice.optionCardIds.includes(id))) return "Private choice selected an unavailable card";
  return choice;
}
...
function autonomyProfileId(state: EngineState): string | null {
  const id = autonomyRuntime(state)?.profileId;
  return typeof id === "string" ? id : null;
}

function isEssentialsProfile(state: EngineState): boolean { return ["first-contact-essentials", "first-contact-response", "first-contact-private-choice", "first-contact-trigger-closure"].includes(autonomyProfileId(state) ?? ""); }
function isResponseProfile(state: EngineState): boolean { return ["first-contact-response", "first-contact-private-choice", "first-contact-trigger-closure"].includes(autonomyProfileId(state) ?? ""); }
function isPrivateChoiceProfile(state: EngineState): boolean { return ["first-contact-private-choice", "first-contact-trigger-closure"].includes(autonomyProfileId(state) ?? ""); }
function isTriggerClosureProfile(state: EngineState): boolean { return autonomyProfileId(state) === "first-contact-trigger-closure"; }

function validateAutonomyAction(state: EngineState, actorId: PlayerId): string | null {
  const profileId = autonomyProfileId(state);
  if (!profileId || !SUPPORTED_AUTONOMY_PROFILES.has(profileId)) return "A supported First Contact autonomy profile is not active";
  if (state.activePlayerId !== actorId) return `${actorId} is not the active player`;
  if (state.phase !== "Action") return `Player action requires Action phase; found ${state.phase}`;
  if (state.winner !== null) return "Match is already terminal";
  if (state.stack.length > 0 || state.pendingDeclaration !== null || state.priority?.open === true || activePrivateChoice(state) !== null) return "Autonomy action cannot begin with pending game objects or choices";
  return null;
}

function validateEssentialsEffect(state: EngineState, actorId: PlayerId): string | null {
  const problem = validateAutonomyAction(state, actorId);
  if (problem) return problem;
  if (!isEssentialsProfile(state)) return "First Contact Essentials profile is not active";
  const lock = state.metadata.boardLock as { turnsRemaining?: number } | undefined;
  if ((lock?.turnsRemaining ?? 0) > 0) return "Board Lock forbids non-counter effect and Anchor plays";
  return null;
}

function finishAutonomyAction(state: EngineState, actorId: PlayerId): void {
  state.players[actorId]!.limits.miniTurnsUsed = 1;
  state.players[actorId]!.limits.miniTurnsRemaining = 0;
  state.phase = "End";
}

function futureStart(state: EngineState, playerId: PlayerId) {
  return { playerId, startSequence: (state.startPhaseSequenceByPlayer[playerId] ?? 0) + 1 };
}

function clearReveal(cardId: CardId, state: EngineState): void { delete state.cards[cardId]!.state.revealedUntil; }

function drawFromTop(state: EngineState, playerId: PlayerId, count: number): CardId[] {
  const drawn: CardId[] = [];
...
function shuffleIntoDp(state: EngineState, cardIds: CardId[]): void {
  for (const cardId of cardIds) if (state.cards[cardId]?.zone !== "VOID") moveCard(state, cardId, "VOID");
  ...
  for (const cardId of cardIds) moveCard(state, cardId, "DP");
}

function rankOf(state: EngineState, cardId: CardId): string | null {
  const card = state.cards[cardId];
  return card ? parseIdentity(card.identity)?.rank ?? null : null;
}

function stagePrivateChoiceSource(state: EngineState, cardId: CardId, actorId: PlayerId): void {
...
function completePrivateChoiceSource(state: EngineState, cardId: CardId, actorId: PlayerId): void {
...
function beginChoice(state: EngineState, input: Omit<FirstContactPrivateChoiceState, "schemaVersion" | "choiceId" | "token" | "createdRevision" | "optionsHash">, events: Phase9EventSpec[]): FirstContactPrivateChoiceState {
...
function queueSevenScoringTrigger(state: EngineState, controllerId: PlayerId, sourceCardId: CardId, events: Phase9EventSpec[]): TriggerEvent {
...
function flushFirstContactTriggerQueue(state: EngineState, actorId: PlayerId, events: Phase9EventSpec[]): string | null {
  if (!isTriggerClosureProfile(state)) return "First Contact Trigger Closure profile is not active";
  if (state.activePlayerId !== actorId) return `${actorId} is not the active player`;
  if (state.priority?.open === true || state.stack.length > 0 || activePrivateChoice(state) !== null) return "Trigger queue cannot flush while another authority object is pending";
  const trigger = state.triggerQueue[0];
  if (!trigger) return "No trigger is queued";
  if (trigger.controllerId !== actorId) return `${actorId} does not control the queued trigger`;
  if (trigger.kind !== FIRST_CONTACT_SEVEN_SCORING_TRIGGER_KIND || !trigger.sourceCardId) return `Unsupported trigger kind ${trigger.kind}`;
...
interface PrimaryDescriptor {
  actionType: FirstContactMiniTurnActionType;
  stackClass: FirstContactStackClass;
  sourceCardIds: CardId[];
  costCardIds: CardId[];
  targetCardIds: CardId[];
}

function primaryDescriptor(action: FirstContactPrimaryAction): PrimaryDescriptor {
  switch (action.kind) {
    ...
    case "autonomy-pass": return { actionType: "pass", stackClass: "pass", sourceCardIds: [], costCardIds: [], targetCardIds: [] };
...
function openPriorityAfter(state: EngineState, actorId: PlayerId): void {
...
function finishCounteredRoot(state: EngineState): void {
...
function sendStackSourcesToGy(state: EngineState, item: StackItem): CardId[] {
  const moved: CardId[] = [];
...
function topStackItem(state: EngineState, id: StackItemId): StackItem | null {
  const top = state.stack.at(-1);
  return top?.id === id ? top : null;
}

function eligibleForBaseAce(target: StackItem): boolean {
...
function responseSourceProblem(state: EngineState, actorId: PlayerId, cardId: CardId, rank: string): string | null {
  const card = state.cards[cardId];
  if (!card || card.controllerId !== actorId || card.zone !== `${actorId}_HAND`) return `${cardId} is not a controlled hand source`;
  if (rankOf(state, cardId) !== rank) return `${cardId} is not rank ${rank}`;
  return null;
}

function declareAuthorityItem(state: EngineState, actorId: PlayerId, commandTag: string, sourceCardIds: CardId[], targetCardIds: CardId[], payload: FirstContactStackPayload, stackClass: FirstContactStackClass): StackItem {
  for (const cardId of sourceCardIds) moveCard(state, cardId, "ON_STACK");
  const item: StackItem = {
...
function validatePriorityHolder(state: EngineState, actorId: PlayerId): string | null {
  if (!isResponseProfile(state)) return "First Contact Response Authority profile is not active";
  const priority = state.priority;
  if (!priority?.open) return "No response priority window is open";
  if (priority.order[priority.index] !== actorId) return `${actorId} does not hold response priority`;
  return null;
}

function reopenAfterResolution(state: EngineState, actorId: PlayerId): void {
  if (state.stack.length > 0) openPriorityAfter(state, actorId);
  else state.priority = null;
}

function clearDisruptionsAtTurnEnd(state: EngineState, playerId: PlayerId, events: Phase9EventSpec[]): void {
...
export function resolvePhase9Action(input: EngineState, actorId: PlayerId, action: FirstContactAction): Phase9Resolution {
  if (!input.players[actorId]) return fail("PHASE9_PLAYER", `Unknown actor ${actorId}`);
  const state = canonicalClone(input);
  const events: Phase9EventSpec[] = [];
  ...
  switch (action.kind) {
    case "validate-profile-configuration": {
      ...
      if (problem) return fail("FIRST_CONTACT_CONFIGURATION", problem, { enabledModules: action.enabledModules });
    ...
    case "validate-declaration": {
      ...
      if (problem) return fail("FIRST_CONTACT_DECLARATION", problem, { declarationClass: action.declarationClass, rank: action.rank ?? null, effectKey: action.effectKey ?? null, sourceCardIds: action.sourceCardIds ?? [] });
      if (action.sourceCardIds) {
        for (const cardId of action.sourceCardIds) {
          const card = state.cards[cardId];
          if (!card || card.controllerId !== actorId || card.zone !== `${actorId}_HAND`) return fail("FIRST_CONTACT_SOURCE", `${cardId} is not a controlled hand source`);
          const parsed = parseIdentity(card.identity);
          if (parsed?.rank !== action.rank) return fail("FIRST_CONTACT_SOURCE", `${cardId} does not match declared rank ${action.rank}`);
        }
      }
    ...
    case "apply-setup": {
      ...
      if (problem) return fail("FIRST_CONTACT_SETUP", problem);
      events.push({ type: "FIRST_CONTACT_SETUP_APPLIED", payload: { playerIds: action.playerIds, goal: 15, miniTurns: 1 } });
      break;
    }
    case "begin-start": {
      if (!state.players[action.playerId]) return fail("FIRST_CONTACT_START", `Unknown player ${action.playerId}`);
      if (!isFirstContact(state)) return fail("FIRST_CONTACT_INACTIVE", "First Contact profile is not active");
      state.phase = "Start";
    ...
    case "route-destination": {
      ...
      if (!from) return fail("FIRST_CONTACT_CARD", `Unknown card ${action.cardId}`);
    ...
    case "grant-mini-turns": {
      ...
      if (!player) return fail("FIRST_CONTACT_PLAYER", `Unknown player ${action.playerId}`);
    ...
    case "enter-hand": {
      if (!isFirstContact(state)) return fail("FIRST_CONTACT_INACTIVE", "First Contact profile is not active");
      if (!state.players[action.playerId]) return fail("FIRST_CONTACT_PLAYER", `Unknown player ${action.playerId}`);
      const card = state.cards[action.cardId];
      if (!card) return fail("FIRST_CONTACT_CARD", `Unknown card ${action.cardId}`);
    ...
    case "autonomy-declare-response-action": {
      if (!isResponseProfile(state)) return fail("AUTONOMY_RESPONSE_PROFILE", "First Contact Response Authority profile is not active");
      const problem = validateAutonomyAction(state, actorId);
      if (problem) return fail("AUTONOMY_ACTION", problem);
      const probe = resolvePhase9Action(state, actorId, action.action);
      if (!probe.ok) return probe;
      const descriptor = primaryDescriptor(action.action);
      for (const cardId of descriptor.costCardIds) moveCard(state, cardId, "GY");
    ...
    case "autonomy-declare-ace-counter": {
      ...
      if (problem) return fail("AUTONOMY_PRIORITY", problem);
      const sourceProblem = responseSourceProblem(state, actorId, action.sourceCardId, "A");
      if (sourceProblem) return fail("AUTONOMY_COUNTER_SOURCE", sourceProblem);
      const target = topStackItem(state, action.targetStackItemId);
      if (!target || !eligibleForBaseAce(target)) return fail("AUTONOMY_COUNTER_TARGET", "Base Ace requires the current top eligible ordinary effect or counter");
    ...
    case "autonomy-declare-eight-scuttle-counter": {
      ...
      if (sourceProblem) return fail("AUTONOMY_COUNTER_SOURCE", sourceProblem);
      const target = topStackItem(state, action.targetStackItemId);
      if (!target || target.firstContactAuthority?.kind !== "primary" || target.firstContactAuthority.stackClass !== "scuttle") return fail("AUTONOMY_COUNTER_TARGET", "Eight Scuttle Counter requires the current pending Scuttle");
    ...
    case "autonomy-declare-king-counter": {
      ...
      if (sourceProblem) return fail("AUTONOMY_COUNTER_SOURCE", sourceProblem);
      const target = topStackItem(state, action.targetStackItemId);
      if (!target || target.firstContactAuthority?.kind !== "primary" || !["anchor", "goal-mod"].includes(target.firstContactAuthority.stackClass) || target.sourceCardIds.length !== 1) return fail("AUTONOMY_COUNTER_TARGET", "King requires the current pending single-card Anchor or Goal-Mod play");
    ...
    case "autonomy-declare-jack-disrupt": {
      ...
      if (sourceProblem) return fail("AUTONOMY_DISRUPT_SOURCE", sourceProblem);
      const target = topStackItem(state, action.targetStackItemId);
      const payload = target?.firstContactAuthority;
      if (!target || payload?.kind !== "primary" || target.controllerId === actorId) return fail("AUTONOMY_DISRUPT_TARGET", "Jack Disrupt requires the opponent's current Mini-Turn Action declaration");
    ...
    case "autonomy-pass-priority": {
      ...
      events.push({ type: "AUTONOMY_PRIORITY_PASSED", payload: { playerId: actorId, consecutivePasses: priority.consecutivePasses } });
      if (priority.consecutivePasses >= priority.order.length) {
        priority.open = false;
        events.push({ type: "AUTONOMY_PRIORITY_CLOSED", payload: { reason: "all-players-passed", stackDepth: state.stack.length } });
      }
      break;
    }
    case "autonomy-flush-trigger-queue": {
      const problem = flushFirstContactTriggerQueue(state, actorId, events);
      if (problem) return fail("AUTONOMY_TRIGGER_QUEUE", problem);
      break;
    }
    case "autonomy-resolve-response-top": {
      if (!isResponseProfile(state)) return fail("AUTONOMY_RESPONSE_PROFILE", "First Contact Response Authority profile is not active");
      if (state.priority?.open === true) return fail("AUTONOMY_PRIORITY_OPEN", "Cannot resolve while response priority is open");
      const item = state.stack.at(-1);
      const payload = item?.firstContactAuthority;
      if (!item || !payload) return fail("AUTONOMY_STACK", "No First Contact response stack item is available");
      if (payload.kind === "seven-scoring-trigger") {
        if (!isTriggerClosureProfile(state)) return fail("AUTONOMY_TRIGGER_PROFILE", "Seven scoring trigger requires Trigger Closure Authority");
        state.stack.pop();
        state.priority = null;
      ...
      if (payload.kind === "primary") {
        state.stack.pop();
        state.priority = null;
        for (const cardId of item.sourceCardIds) if (state.cards[cardId]?.zone === "ON_STACK") moveCard(state, cardId, `${item.controllerId}_HAND`, item.controllerId);
        const descriptor = primaryDescriptor(payload.action);
        for (const cardId of descriptor.costCardIds) if (state.cards[cardId]?.zone === "GY") moveCard(state, cardId, `${item.controllerId}_HAND`, item.controllerId);
        ...
        if (!resolved.ok) {
          if (resolved.code === "AUTONOMY_DRAW_EMPTY" && payload.action.kind === "autonomy-draw") {
            for (const cardId of item.sourceCardIds) if (state.cards[cardId]?.zone === `${item.controllerId}_HAND`) routeDestination(state, cardId, "GY");
            for (const cardId of descriptor.costCardIds) if (state.cards[cardId]?.zone === `${item.controllerId}_HAND`) routeDestination(state, cardId, "GY");
      ...
      if (payload.kind === "jack-disrupt") {
        ...
        if (!target || targetPayload?.kind !== "primary") return fail("AUTONOMY_DISRUPT_TARGET", "Jack Disrupt target is no longer pending");
        const runtime = autonomyRuntime(state)!;
        runtime.disruptedActionTypesByPlayer ??= {};
        const list = runtime.disruptedActionTypesByPlayer[target.controllerId] ?? [];
        if (!list.includes(payload.disruptedActionType)) list.push(payload.disruptedActionType);
      ...
      if (targetIndex < 0) return fail("AUTONOMY_COUNTER_TARGET", "Counter target is no longer pending");
      ...
      if (targetWasRoot && state.stack.length === 0) finishCounteredRoot(state);
      else reopenAfterResolution(state, item.controllerId);
      break;
    }
    case "autonomy-enter-action": {
      const profileId = autonomyProfileId(state);
      if (!profileId || !SUPPORTED_AUTONOMY_PROFILES.has(profileId)) return fail("AUTONOMY_PROFILE", "A supported First Contact autonomy profile is not active");
      if (state.activePlayerId !== actorId || state.phase !== "Start") return fail("AUTONOMY_PHASE", "Action phase entry requires the active player's completed Start phase");
      const runtime = autonomyRuntime(state)!;
    ...
    case "autonomy-draw": {
      const problem = validateAutonomyAction(state, actorId);
      if (problem) return fail("AUTONOMY_ACTION", problem);
      if (state.zones.dp.length === 0) return fail("AUTONOMY_DRAW_EMPTY", "Cannot draw from an empty DP");
    ...
    case "autonomy-score": {
      ...
      if (problem) return fail("AUTONOMY_ACTION", problem);
      const card = state.cards[action.cardId];
      if (!card || card.controllerId !== actorId || card.zone !== `${actorId}_HAND`) return fail("AUTONOMY_SCORE_SOURCE", "Play for Points requires a controlled hand card");
      ...
      if (isTriggerClosureProfile(state) && rankOf(state, action.cardId) === "7") queueSevenScoringTrigger(state, actorId, action.cardId, events);
      break;
    }
    case "autonomy-scuttle": {
      ...
      if (!source || source.controllerId !== actorId || source.zone !== `${actorId}_HAND`) return fail("AUTONOMY_SCUTTLE_SOURCE", "Scuttle source must be controlled in hand");
      if (!target || target.controllerId === actorId || !target.zone.endsWith("_PR")) return fail("AUTONOMY_SCUTTLE_TARGET", "Scuttle target must be an opponent PR card");
      if (hasAegis(target)) return fail("AEGIS_BLOCK", "Aegis blocks Scuttle");
      if (hasOrdinaryScuttleImmunity(target)) return fail("SCUTTLE_IMMUNITY", `${target.identity} has ordinary Scuttle immunity`);
      if (compareScuttle(source, target) <= 0) return fail("SCUTTLE_RANK", "Scuttle source must have higher rank or equal rank with higher suit");
    ...
    case "autonomy-three-hand-raid": {
      ...
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      const source = state.cards[action.sourceCardId];
      const target = state.players[action.targetPlayerId];
      if (!isPrivateChoiceProfile(state)) return fail("AUTONOMY_PRIVATE_CHOICE_PROFILE", "Three Hand Raid requires Private Choice Authority");
      if (!source || rankOf(state, source.id) !== "3" || source.zone !== `${actorId}_HAND` || source.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Three Hand Raid requires a controlled Three in hand");
      if (!target || action.targetPlayerId === actorId) return fail("AUTONOMY_EFFECT_TARGET", "Three Hand Raid requires an opponent");
    ...
    case "autonomy-three-bounce": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (!isPrivateChoiceProfile(state)) return fail("AUTONOMY_PRIVATE_CHOICE_PROFILE", "Three Bounce requires Private Choice Authority");
      const source = state.cards[action.sourceCardId];
      const target = state.cards[action.targetCardId];
      if (!source || rankOf(state, source.id) !== "3" || source.zone !== `${actorId}_HAND` || source.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Three Bounce requires a controlled Three in hand");
      if (!target || target.controllerId === actorId || (!target.zone.endsWith("_PR") && !target.zone.endsWith("_ER"))) return fail("AUTONOMY_EFFECT_TARGET", "Three Bounce requires an opponent OTT card");
      ...
      if (!protection.legal) return fail("PROTECTION_BLOCK", `Three Bounce blocked by ${protection.blockedBy.join(", ")}`, protection);
      ...
      if (action.destination === "top") state.zones.dp.unshift(target.id); else state.zones.dp.push(target.id);
    ...
    case "autonomy-five-recycle": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (!isPrivateChoiceProfile(state)) return fail("AUTONOMY_PRIVATE_CHOICE_PROFILE", "Five Recycle requires Private Choice Authority");
      const source = state.cards[action.sourceCardId];
      if (!source || rankOf(state, source.id) !== "5" || source.zone !== `${actorId}_HAND` || source.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Five Recycle requires a controlled Five in hand");
      stagePrivateChoiceSource(state, source.id, actorId);
      const milled: CardId[] = [];
      for (let index = 0; index < 2 && state.zones.dp.length > 0; index += 1) {
        const cardId = state.zones.dp[0]!;
    ...
    case "autonomy-six-dig": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (!isPrivateChoiceProfile(state)) return fail("AUTONOMY_PRIVATE_CHOICE_PROFILE", "Six Dig requires Private Choice Authority");
      const source = state.cards[action.sourceCardId];
      if (!source || rankOf(state, source.id) !== "6" || source.zone !== `${actorId}_HAND` || source.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Six Dig requires a controlled Six in hand");
      stagePrivateChoiceSource(state, source.id, actorId);
      const drawn: CardId[] = [];
    ...
    case "autonomy-seven-topdeck": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (!isPrivateChoiceProfile(state)) return fail("AUTONOMY_PRIVATE_CHOICE_PROFILE", "Seven Topdeck requires Private Choice Authority");
      const source = state.cards[action.sourceCardId];
      if (!source || rankOf(state, source.id) !== "7" || source.zone !== `${actorId}_HAND` || source.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Seven Topdeck requires a controlled Seven in hand");
      stagePrivateChoiceSource(state, source.id, actorId);
    ...
    case "autonomy-nine-anchor": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (!isPrivateChoiceProfile(state)) return fail("AUTONOMY_PRIVATE_CHOICE_PROFILE", "Nine Anchor requires Private Choice Authority");
      const source = state.cards[action.sourceCardId];
      const target = state.players[action.targetPlayerId];
      if (!source || rankOf(state, source.id) !== "9" || source.zone !== `${actorId}_HAND` || source.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Nine Anchor requires a controlled Nine in hand");
      if (!target || action.targetPlayerId === actorId) return fail("AUTONOMY_EFFECT_TARGET", "Nine Anchor requires an opponent");
      const previous = state.players[actorId]!.er.filter((id) => state.cards[id]?.state.firstContactNineAnchor === true);
      for (const id of previous) routeDestination(state, id, "GY");
    ...
    case "autonomy-four-clear": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (rankOf(state, action.sourceCardId) !== "4" || state.cards[action.sourceCardId]?.zone !== `${actorId}_HAND`) return fail("AUTONOMY_EFFECT_SOURCE", "Row Clear requires a controlled Four in hand");
      const targetPlayer = state.players[action.targetPlayerId];
      if (!targetPlayer || action.targetPlayerId === actorId) return fail("AUTONOMY_EFFECT_TARGET", "Row Clear requires an opponent");
      ...
      for (const cardId of affected) routeDestination(state, cardId, "GY");
    ...
    case "autonomy-nine-tap": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (rankOf(state, action.sourceCardId) !== "9" || state.cards[action.sourceCardId]?.zone !== `${actorId}_HAND`) return fail("AUTONOMY_EFFECT_SOURCE", "Tap requires a controlled Nine in hand");
      const target = state.cards[action.targetCardId];
      if (!target || target.controllerId === actorId || !target.zone.endsWith("_PR")) return fail("AUTONOMY_EFFECT_TARGET", "Nine Tap requires an opponent PR card");
      ...
      if (!protection.legal) return fail("PROTECTION_BLOCK", `Nine Tap blocked by ${protection.blockedBy.join(", ")}`, protection);
    ...
    case "autonomy-nine-goal-shift": {
      const problem = validateEssentialsEffect(state, actorId);
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      if (rankOf(state, action.sourceCardId) !== "9" || state.cards[action.sourceCardId]?.zone !== `${actorId}_HAND`) return fail("AUTONOMY_EFFECT_SOURCE", "Goal Shift requires a controlled Nine in hand");
      const target = state.players[action.targetPlayerId];
      if (!target || action.targetPlayerId === actorId) return fail("AUTONOMY_EFFECT_TARGET", "Goal Shift requires an opponent");
      if (action.delta === 5) {
        if (!action.discardCardId || action.discardCardId === action.sourceCardId || state.cards[action.discardCardId]?.zone !== `${actorId}_HAND` || state.cards[action.discardCardId]?.controllerId !== actorId) return fail("AUTONOMY_EFFECT_COST", "+5 Goal Shift requires one other controlled hand card");
    ...
    case "autonomy-jack-pr-attachment": {
      ...
      const target = state.cards[action.targetCardId];
      if (!jack || rankOf(state, jack.id) !== "J" || jack.zone !== `${actorId}_HAND` || jack.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Jack PR Attachment requires a controlled Jack in hand");
      if (!target || target.controllerId === actorId || !target.zone.endsWith("_PR") || rankOf(state, target.id) === "RJ" || target.state.attachedByJackId !== undefined || target.state.attachmentGraph !== undefined) return fail("AUTONOMY_EFFECT_TARGET", "Jack requires an unattached Vulnerable opponent PR host");
      ...
      if (!protection.legal) return fail("PROTECTION_BLOCK", `Jack Attachment blocked by ${protection.blockedBy.join(", ")}`, protection);
    ...
    case "autonomy-queen-anchor": {
      ...
      if (!queen || rankOf(state, queen.id) !== "Q" || queen.zone !== `${actorId}_HAND` || queen.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Queen Anchor requires a controlled Queen in hand");
    ...
    case "autonomy-king-anchor": {
      ...
      if (!king || rankOf(state, king.id) !== "K" || king.zone !== `${actorId}_HAND` || king.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "King Anchor requires a controlled King in hand");
    ...
    case "autonomy-red-joker": {
      ...
      if (!joker || rankOf(state, joker.id) !== "RJ" || joker.zone !== `${actorId}_HAND` || joker.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Red Joker mode requires a controlled Red Joker in hand");
      moveCard(state, joker.id, "VOID");
      let payload: Record<string, unknown> = { playerId: actorId, sourceCardId: joker.id, mode: action.mode };
      if (action.mode === "hand-swap") {
        const target = action.targetPlayerId ? state.players[action.targetPlayerId] : undefined;
        if (!target || action.targetPlayerId === actorId) return fail("AUTONOMY_EFFECT_TARGET", "Hand Swap requires an opponent");
        const own = [...state.players[actorId]!.hand];
        const theirs = [...target.hand];
        for (const id of [...own, ...theirs]) moveCard(state, id, "VOID");
        for (const id of own) { clearReveal(id, state); moveCard(state, id, `${action.targetPlayerId}_HAND`, action.targetPlayerId); }
        for (const id of theirs) { clearReveal(id, state); moveCard(state, id, `${actorId}_HAND`, actorId); }
        ...
        for (const id of discarded) moveCard(state, id, "GY");
        ...
        const target = action.targetPlayerId ? state.players[action.targetPlayerId] : undefined;
        if (!target || action.targetPlayerId === actorId) return fail("AUTONOMY_EFFECT_TARGET", "Opponent Attack requires an opponent");
    ...
    case "autonomy-black-joker-board-lock": {
      ...
      if (problem) return fail("AUTONOMY_EFFECT", problem);
      const joker = state.cards[action.sourceCardId];
      if (!joker || rankOf(state, joker.id) !== "BJ" || joker.zone !== `${actorId}_HAND` || joker.controllerId !== actorId) return fail("AUTONOMY_EFFECT_SOURCE", "Board Lock requires a controlled Black Joker in hand");
      moveCard(state, joker.id, "GY");
      state.metadata.boardLock = { turnsRemaining: 2, activationFullTurnId: state.fullTurnSequence, activatorId: actorId };
      events.push({ type: "AUTONOMY_BLACK_JOKER_BOARD_LOCK_RESOLVED", payload: state.metadata.boardLock as Record<string, unknown> });
      finishAutonomyAction(state, actorId);
      break;
    }
    case "autonomy-pass": {
      ...
      state.players[actorId]!.limits.miniTurnsRemaining = 0;
      state.phase = "End";
      events.push({ type: "AUTONOMY_PASS_COMPLETED", payload: { playerId: actorId } });
      break;
    }
    case "autonomy-submit-private-choice": {
      ...
      if (typeof validated === "string") return fail("AUTONOMY_PRIVATE_CHOICE", validated);
      const choice = validated;
      const selected = [...action.submission.selectedCardIds];
      const context = choice.context as Record<string, unknown>;
      if (action.submission.kind === "rank3-present") {
        const targetPlayerId = String(context.targetPlayerId);
        if (selected.some((id) => state.cards[id]?.zone !== `${targetPlayerId}_HAND`)) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Presented cards must remain in the opponent hand");
      ...
      if (action.submission.kind === "rank3-take") {
        ...
        if (state.cards[cardId]?.zone !== `${targetPlayerId}_HAND`) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Chosen presented card is no longer in the opponent hand");
      ...
      if (action.submission.kind === "rank3-discard") {
        const targetPlayerId = String(context.targetPlayerId);
        if (selected.some((id) => state.cards[id]?.zone !== `${targetPlayerId}_HAND`)) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Discard choices must remain in the opponent hand");
        for (const id of selected) moveCard(state, id, "GY");
      ...
      if (action.submission.kind === "rank5-rummage") {
        const cardId = selected[0]!;
        if (state.cards[cardId]?.zone !== "GY") return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Rummage choice must remain in GY");
        moveCard(state, cardId, `${choice.controllerId}_HAND`, choice.controllerId);
        let bottomDraw: CardId | null = null;
      ...
      if (action.submission.kind === "rank6-dig") {
        const drawn = [...((context.drawnCardIds as CardId[] | undefined) ?? [])];
        const discardOptions = [...((context.discardOptionCardIds as CardId[] | undefined) ?? [])];
        if (drawn.some((id) => state.cards[id]?.state.privateChoiceHeldBy !== choice.controllerId)) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Drawn Six cards are no longer sealed for this choice");
        if (action.submission.mode === "keep-all-discard") {
          if (selected.length !== 1 || !discardOptions.includes(selected[0]!)) return fail("AUTONOMY_PRIVATE_CHOICE_SELECTION", "Keep-all requires one legal discard");
          for (const id of drawn) { delete state.cards[id]!.state.privateChoiceHeldBy; moveCard(state, id, `${choice.controllerId}_HAND`, choice.controllerId); }
          if (state.cards[selected[0]!]!.zone !== `${choice.controllerId}_HAND`) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Six discard is no longer in hand");
          ...
          if (selected.length !== keepCount || selected.some((id) => !drawn.includes(id))) return fail("AUTONOMY_PRIVATE_CHOICE_SELECTION", `Six keep-return requires exactly ${keepCount} drawn cards to keep`);
          const keep = new Set(selected);
          const returned = drawn.filter((id) => !keep.has(id));
          for (const id of selected) { delete state.cards[id]!.state.privateChoiceHeldBy; moveCard(state, id, `${choice.controllerId}_HAND`, choice.controllerId); }
          for (const id of returned) {
            ...
            if (action.submission.mode === "keep-return-top") state.zones.dp.unshift(id); else state.zones.dp.push(id);
          }
      ...
      if (action.submission.kind === "rank7-assign") {
        const revealed = [...((context.revealedCardIds as CardId[] | undefined) ?? [])];
        if (revealed.some((id) => state.cards[id]?.state.privateChoiceHeldBy !== choice.controllerId)) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Revealed Seven cards are no longer sealed for this choice");
        let handCardId: CardId | null = null;
        let effectCardId: CardId | null = null;
        if (revealed.length === 1) {
          if (selected.length !== 1 || selected[0] !== revealed[0]) return fail("AUTONOMY_PRIVATE_CHOICE_SELECTION", "Single-card Seven choice must select the revealed card");
          if (action.submission.mode === "hand-only") handCardId = selected[0]!;
          else if (action.submission.mode === "effect-only") effectCardId = selected[0]!;
          else return fail("AUTONOMY_PRIVATE_CHOICE_SELECTION", "Single-card Seven cannot use hand-and-effect");
        } else {
          if (action.submission.mode !== "hand-and-effect" || selected.length !== 2 || new Set(selected).size !== 2 || selected.some((id) => !revealed.includes(id))) return fail("AUTONOMY_PRIVATE_CHOICE_SELECTION", "Two-card Seven requires ordered [hand, effect] assignment");
        ...
        if (!effectCardId) {
          for (const id of revealed.filter((entry) => entry !== handCardId)) { delete state.cards[id]!.state.privateChoiceHeldBy; delete state.cards[id]!.state.privateChoicePublicReveal; moveCard(state, id, "DP"); }
      ...
      if (action.submission.kind === "rank7-generated-effect") {
        ...
        if (selected.length !== 1 || selected[0] !== generatedCardId || state.cards[generatedCardId]?.state.privateChoiceHeldBy !== choice.controllerId) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Generated Seven effect card is unavailable");
        ...
        if (descriptor.sourceCardIds.length !== 1 || descriptor.sourceCardIds[0] !== generatedCardId || parseIdentity(state.cards[generatedCardId]!.identity)?.rank !== parseIdentity(state.cards[descriptor.sourceCardIds[0]!]!.identity)?.rank) return fail("AUTONOMY_PRIVATE_CHOICE_GENERATED", "Generated action must use exactly the sealed Seven card");
        ...
        if (!probe.ok) return fail("AUTONOMY_PRIVATE_CHOICE_GENERATED", probe.message, { code: probe.code });
      ...
      if (action.submission.kind === "rank7-scoring-trigger") {
        const revealed = [...((context.revealedCardIds as CardId[] | undefined) ?? [])];
        if (selected.length !== 1 || !revealed.includes(selected[0]!)) return fail("AUTONOMY_PRIVATE_CHOICE_SELECTION", "Seven scoring trigger requires one revealed card");
        if (revealed.some((id) => state.cards[id]?.state.privateChoiceHeldBy !== choice.controllerId || state.cards[id]?.state.privateChoicePublicReveal !== true)) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Seven scoring trigger cards are no longer sealed and public");
      ...
      if (action.submission.kind === "nine-anchor-discard") {
        const targetPlayerId = String(context.targetPlayerId);
        const cardId = selected[0]!;
        if (state.cards[cardId]?.zone !== `${targetPlayerId}_HAND`) return fail("AUTONOMY_PRIVATE_CHOICE_STALE", "Nine Anchor discard must remain in the opponent hand");
        moveCard(state, cardId, "GY");
        clearPrivateChoice(state);
    ...
    case "autonomy-complete-turn": {
      const profileId = autonomyProfileId(state);
      if (!profileId || !SUPPORTED_AUTONOMY_PROFILES.has(profileId)) return fail("AUTONOMY_PROFILE", "A supported First Contact autonomy profile is not active");
      if (state.activePlayerId !== actorId || state.phase !== "End") return fail("AUTONOMY_PHASE", "Full Turn completion requires the active player's End phase");
