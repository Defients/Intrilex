...
export function isCorePrivateChoiceProfile(state: Readonly<EngineState>): boolean {
  return (state.metadata.coreAuthority as CoreRuntimeWithChoice | undefined)?.profileId === CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id || (state.metadata.coreAuthority as CoreRuntimeWithChoice | undefined)?.profileId === "core-advanced-authority";
}
...
export function resolveCorePrivateChoiceSubmission(
  ...
  if (submission.kind === "core-rank7-assign") {
    ...
    if (revealed.length === 1) {
      ...
      else return fail("CORE_PRIVATE_CHOICE_SELECTION", "Single-card Seven cannot use hand-and-effect");
    } else {
      if (submission.mode !== "hand-and-effect" || selected.length !== 2 || new Set(selected).size !== 2 || selected.some((id) => !revealed.includes(id))) return fail("CORE_PRIVATE_CHOICE_SELECTION", "Two-card Seven requires ordered [hand,effect] assignment");
  ...
  if (submission.kind === "core-rank7-generated-effect") {
    ...
    if (!generatedEffect) {
      if (legalEffects.length > 0) return fail("CORE_PRIVATE_CHOICE_GENERATED", "A legal generated effect must be selected");
