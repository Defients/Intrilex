...
import {
  ...
  markExileBound,
  markPlayedForEffect,
  processStartPhaseLifecycles,
  releaseNineTapsForScoring,
  revealUntilStart
} from "./lifecycle.js";
import { moveCard, zoneList } from "./state.js";
...
function nextEventId(state: EngineState, index: number): string {
  return `EV-${String(state.revision + index + 1).padStart(6, "0")}`;
}

function commitEvents(state: EngineState, commandId: string, specs: EventSpec[], previousStateHash: string): EngineEvent[] {
...
function fail(state: EngineState, code: string, message: string, details?: unknown): CommandResult {
  return {
    accepted: false,
    state,
    events: [],
...
function checkRequirement(state: EngineState, requirement: Requirement): string | null {
  switch (requirement.kind) {
    case "other-hand-cards": {
      const player = state.players[requirement.playerId];
      if (!player) return `Unknown player ${requirement.playerId}`;
    ...
    case "target-unprotected": {
      const card = state.cards[requirement.cardId];
      if (!card) return `Unknown target ${requirement.cardId}`;
...
function stageDeclaration(state: EngineState, command: Extract<EngineCommand, { type: "DECLARE_PLAY" | "RESPOND_WITH_PLAY" }>): { ok: true; state: EngineState } | { ok: false; reason: string } {
  ...
  if (!actor) return { ok: false, reason: `Unknown actor ${command.actorId}` };

  for (const id of [...sourceIds, ...costIds]) {
    const card = staged.cards[id];
    if (!card) return { ok: false, reason: `Unknown card ${id}` };
    if (card.controllerId !== command.actorId) return { ok: false, reason: `${id} is not controlled by ${command.actorId}` };
    if (card.zone !== `${command.actorId}_HAND`) return { ok: false, reason: `${id} is not in ${command.actorId}'s hand` };
  }

  const beforeImageHash = hashCanonical(state);
  for (const id of [...sourceIds, ...costIds]) moveCard(staged, id, "STAGING");
...
function commitDeclaration(staged: EngineState, command: Extract<EngineCommand, { type: "DECLARE_PLAY" | "RESPOND_WITH_PLAY" }>): EngineState {
  const state = canonicalClone(staged);
  const pending = state.pendingDeclaration;
  if (!pending) throw new EngineError("DECLARATION", "No staged declaration to commit");
  const itemId = `SI-${command.id}`;
  for (const id of pending.stagedSourceIds) moveCard(state, id, "ON_STACK");
  for (const id of pending.stagedCostIds) moveCard(state, id, "GY");
...
function targetLegal(state: EngineState, cardId: CardId): boolean {
...
function revalidate(state: EngineState, item: StackItem): { fizzle: boolean; legalTargetIds: CardId[] } {
...
function lockCards(state: EngineState, item: StackItem, ids: CardId[]): void {
  for (const id of ids) {
    const card = state.cards[id];
    if (card && card.zone !== "ON_STACK") card.state.lockedBy = item.id;
  }
}

function unlockCards(state: EngineState, itemId: StackItemId): void {
  for (const card of Object.values(state.cards)) {
    if (card.state.lockedBy === itemId) delete card.state.lockedBy;
  }
}

function executeInstruction(state: EngineState, item: StackItem, instruction: Instruction, legalTargetIds: CardId[], specs: EventSpec[]): boolean {
  switch (instruction.op) {
    case "discard": {
      const player = state.players[instruction.playerId];
      if (!player) throw new EngineError("INSTRUCTION", `Unknown player ${instruction.playerId}`);
      const available = instruction.cardIds.filter((id) => player.hand.includes(id));
      if (available.length < instruction.requiredMinimum) return false;
      for (const id of available) moveCard(state, id, "GY");
    ...
    case "draw-keep-return": {
      ...
      for (const id of drawn) {
        if (keepSet.has(id)) moveCard(state, id, `${instruction.playerId}_HAND`);
        else if (returnSet.has(id)) moveCard(state, id, "DP");
    ...
    case "take-card": {
      if (!state.cards[instruction.cardId]) return false;
    ...
    case "remove-target": {
      if (!legalTargetIds.includes(instruction.cardId) && item.revalidationClass !== "structural") return true;
      if (!state.cards[instruction.cardId]) return false;
    ...
    case "rebind-stack-item": {
      ...
      if (instruction.replacementInstructions !== undefined) target.instructions = canonicalClone(instruction.replacementInstructions);
      if (instruction.replacementTargetIds !== undefined) target.targetCardIds = [...instruction.replacementTargetIds];
...
function finishSources(state: EngineState, item: StackItem): void {
  for (const id of item.sourceCardIds) {
    const card = state.cards[id];
    if (!card || card.zone !== "ON_STACK") continue;
    moveCard(state, id, item.sourceDestination);
  }
}

function triggerToStackItem(trigger: TriggerEvent): StackItem {
  return {
...
function resolveTop(state: EngineState): { state: EngineState; specs: EventSpec[] } {
  const next = canonicalClone(state);
  const item = next.stack.at(-1);
  if (!item) throw new EngineError("STACK_EMPTY", "Cannot resolve an empty stack");
  item.status = "resolving";
  const specs: EventSpec[] = [{ type: "RESOLUTION_BEGAN", payload: { stackItemId: item.id, kind: item.kind } }];
  ...
  if (item.counterTargetId) {
    ...
    if (ownIndex >= 0) next.stack.splice(ownIndex, 1);
  } else {
  ...
  if (next.triggerQueue.length > 0) {
    ...
    next.triggerQueue = [];
    for (const trigger of queued) next.stack.push(triggerToStackItem(trigger));
...
function commitDirectCommand(original: EngineState, command: EngineCommand, next: EngineState, specs: EventSpec[], previousHash: string): CommandResult {
  const events = commitEvents(next, command.id, specs, previousHash);
  assertValidState(next);
  return { accepted: true, state: next, events };
}

function requireCard(state: EngineState, cardId: CardId) {
  const card = state.cards[cardId];
  if (!card) throw new EngineError("CARD_UNKNOWN", `Unknown card ${cardId}`);
  return card;
}
    ...
    try {
      switch (command.type) {
        ...
        case "RESPOND_WITH_PLAY": {
          ...
          if (!staged.ok) {
            if (hashCanonical(original) !== previousHash) throw new EngineError("ROLLBACK", "Illegal declaration changed the before-image");
        ...
        case "PASS_PRIORITY": {
          ...
          if (!priority?.open) return fail(original, "PRIORITY_CLOSED", "No priority window is open");
          if (priority.order[priority.index] !== command.actorId) return fail(original, "PRIORITY", `${command.actorId} does not hold priority`);
          priority.consecutivePasses += 1;
          priority.index = (priority.index + 1) % priority.order.length;
          const specs: EventSpec[] = [{ type: "PRIORITY_PASSED", payload: { playerId: command.actorId, consecutivePasses: priority.consecutivePasses } }];
        ...
        case "RESOLVE_TOP": {
          if (original.priority?.open) return fail(original, "PRIORITY_OPEN", "Cannot resolve while priority is open");
        ...
        case "COUNTER_TOP": {
          const priority = original.priority;
          if (!priority?.open || priority.order[priority.index] !== command.actorId) return fail(original, "PRIORITY", `${command.actorId} does not hold priority`);
          const target = original.stack.at(-1);
          if (!target) return fail(original, "STACK_EMPTY", "No stack item to counter");
          ...
          const stagedCommand: Extract<EngineCommand, { type: "RESPOND_WITH_PLAY" }> = { id: command.id, type: "RESPOND_WITH_PLAY", actorId: command.actorId, play };
          const staged = stageDeclaration(original, stagedCommand);
          if (!staged.ok) return fail(original, "DECLARATION_ILLEGAL", staged.reason);
        ...
        case "APPLY_AEGIS": {
          ...
          if (!isOttZone(card.zone)) return fail(original, "AEGIS_ZONE", "Aegis can be applied only to an OTT card");
        ...
        case "APPLY_TAP": {
          const next = canonicalClone(original);
          const card = requireCard(next, command.cardId);
          if (!isOttZone(card.zone)) return fail(original, "TAP_ZONE", "Tap State can be applied only to an OTT card");
        ...
        case "GRANT_REVEAL_UNTIL_START": {
          const next = canonicalClone(original);
          const card = requireCard(next, command.cardId);
          if (!isHandZone(card.zone)) return fail(original, "REVEAL_ZONE", "Revealed-Until-Start can be applied only to a hand card");
          if (!next.players[command.expiresAt.playerId] || command.expiresAt.startSequence <= (next.startPhaseSequenceByPlayer[command.expiresAt.playerId] ?? 0)) {
        ...
        case "SET_PLAYED_FOR_EFFECT": {
          const next = canonicalClone(original);
          const card = requireCard(next, command.cardId);
          if (command.value && !isOttZone(card.zone)) return fail(original, "PLAYED_FOR_EFFECT_ZONE", "Played-for-Effect persists only while the card is OTT");
        ...
        case "CHANGE_CONTROLLER": {
          const next = canonicalClone(original);
          const card = requireCard(next, command.cardId);
          if (!next.players[command.controllerId]) return fail(original, "PLAYER_UNKNOWN", `Unknown controller ${command.controllerId}`);
        ...
        case "BEGIN_START_PHASE": {
          const next = canonicalClone(original);
          if (!next.players[command.playerId]) return fail(original, "PLAYER_UNKNOWN", `Unknown player ${command.playerId}`);
          if (next.stack.length > 0 || next.pendingDeclaration !== null || next.priority?.open === true) return fail(original, "START_BLOCKED", "Cannot begin a Start Phase while game objects are pending");
        ...
        case "SCORE_CARD": {
          const next = canonicalClone(original);
          const card = requireCard(next, command.cardId);
          if (!next.players[command.playerId]) return fail(original, "PLAYER_UNKNOWN", `Unknown scoring player ${command.playerId}`);
          if (card.controllerId !== command.playerId) return fail(original, "SCORE_CONTROLLER", `${command.playerId} does not control ${command.cardId}`);
          if (!isHandZone(card.zone)) return fail(original, "SCORE_ZONE", "This Phase 5 scoring primitive requires a hand card");
          const from = card.zone;
        ...
        case "RESOLVE_RANK_ACTION": {
          ...
          if (!resolved.ok) return fail(original, resolved.code, resolved.message, resolved.details);
        ...
        case "HIDDEN_CHOICE": {
          const next = canonicalClone(original);
          const choices = (next.metadata.hiddenChoices as Record<string, unknown> | undefined) ?? {};
