...
import type { CardId, EngineState, Phase20Action, PlayerId } from "./types.js";
...
export type Phase20Resolution =
  | { ok: true; state: EngineState; events: Phase20EventSpec[] }
  | { ok: false; code: string; message: string; details?: unknown };

function fail(code: string, message: string, details?: unknown): Phase20Resolution {
  return details === undefined ? { ok: false, code, message } : { ok: false, code, message, details };
}

function inHand(state: EngineState, playerId: PlayerId, cardId: CardId): boolean {
...
function resolveSuperSevenSingle(input: EngineState, actorId: PlayerId, action: Extract<Phase20Action, { kind: "resolve-super-seven-single" }>): Phase20Resolution {
  ...
  if (state.zones.dp.length !== 1) return fail("PHASE20_SEVEN_COUNT", "This canonical closure case requires exactly one available DP card");
  ...
  state.metadata.phase20 = {
    ...(state.metadata.phase20 as Record<string, unknown> | undefined),
    lastSequentialSeven: {
...
function resolveSuperFive(input: EngineState, actorId: PlayerId, action: Extract<Phase20Action, { kind: "resolve-super-five" }>): Phase20Resolution {
  const state = canonicalClone(input);
  ...
  if (!revealed.includes(action.chosenCardId)) return fail("PHASE20_FIVE_CHOICE", "⭐5 may play only a card milled by this effect");
  for (const id of revealed) moveCard(state, id, "GY");
  const chosen = state.cards[action.chosenCardId]!;
  if (action.disposition === "points") {
    const parsed = parseIdentity(chosen.identity);
    if (!parsed) return fail("PHASE20_FIVE_IDENTITY", "Chosen card identity is not canonical");
...
function consumeSkippedTurnSlot(input: EngineState, actorId: PlayerId): Phase20Resolution {
  const state = canonicalClone(input);
  const player = state.players[actorId];
  if (!player) return fail("PHASE20_PLAYER", `Unknown player ${actorId}`);
  if (state.activePlayerId !== actorId) return fail("PHASE20_TURN", "Only the scheduled active player's slot may be skipped");
  if (player.limits.pendingFullTurnSkips < 1) return fail("PHASE20_SKIP", "No pending Full-Turn skip exists");
  const before = {
    ...
    timers: canonicalClone((state.metadata.phase8 as Record<string, unknown> | undefined) ?? null)
  };
  ...
  state.metadata.phase20 = {
    ...
    lastSkippedTurnSlot: {
      ...
      timersUnchanged: JSON.stringify((state.metadata.phase8 as Record<string, unknown> | undefined) ?? null) === JSON.stringify(before.timers),
...
export function resolvePhase20Action(input: EngineState, actorId: PlayerId, action: Phase20Action): Phase20Resolution {
  switch (action.kind) {
