...
export interface Phase11Runtime {
  ...
  swapBarLayout: { capacity: number; faceDownCardIds: CardId[]; faceUpCardIds: CardId[] } | null;
  thatsUrzAssignments: Record<PlayerId, PlayerId>;
  tournamentSeedAssignments: Record<PlayerId, CardId>;
  lastTargetEvaluation: { sourcePlayerId: PlayerId; targetPlayerId: PlayerId; relation: "self" | "ally" | "enemy"; legal: boolean } | null;
  winningTeamId: string | null;
  lastTeamTotals: Record<string, { anchors: number; points: number }>;
}

export interface Phase11EventSpec { type: string; payload: Record<string, unknown>; visibility?: "public" | "private" | "authorized"; }
export type Phase11Resolution =
  | { ok: true; state: EngineState; events: Phase11EventSpec[] }
  | { ok: false; code: string; message: string; details?: unknown };

function fail(code: string, message: string, details?: unknown): Phase11Resolution {
  return details === undefined ? { ok: false, code, message } : { ok: false, code, message, details };
}

export function phase11Runtime(state: EngineState): Phase11Runtime {
  const current = state.metadata.phase11 as Partial<Phase11Runtime> | undefined;
  return {
...
function saveRuntime(state: EngineState, runtime: Phase11Runtime): void { state.metadata.phase11 = runtime; }

export function relationBetween(state: EngineState, sourcePlayerId: PlayerId, targetPlayerId: PlayerId): "self" | "ally" | "enemy" {
  if (sourcePlayerId === targetPlayerId) return "self";
...
export function areAllies(state: EngineState, a: PlayerId, b: PlayerId): boolean { return relationBetween(state, a, b) === "ally"; }
export function areEnemies(state: EngineState, a: PlayerId, b: PlayerId): boolean { return relationBetween(state, a, b) === "enemy"; }

export function validateMultiplayerTarget(state: EngineState, sourcePlayerId: PlayerId, targetPlayerId: PlayerId, hostile: boolean, allowsAlly = false): string | null {
  if (!state.players[sourcePlayerId] || !state.players[targetPlayerId]) return "Source and target players must exist";
  const relation = relationBetween(state, sourcePlayerId, targetPlayerId);
  if (hostile && relation === "self") return "A hostile opponent interaction cannot target its controller";
  if (hostile && relation === "ally" && !allowsAlly) return "Hostile effects cannot target an Ally";
  return null;
}

export function nextPlayerAssignments(turnOrder: PlayerId[]): Record<PlayerId, PlayerId> {
  if (turnOrder.length < 2 || new Set(turnOrder).size !== turnOrder.length) throw new Error("Turn order must contain at least two unique players");
  return Object.fromEntries(turnOrder.map((id, index) => [id, turnOrder[(index + 1) % turnOrder.length]!]));
}

export function expectedPriorityOrder(turnOrder: PlayerId[], declaringPlayerId: PlayerId): PlayerId[] {
  const index = turnOrder.indexOf(declaringPlayerId);
  if (index < 0) throw new Error(`Declaring player ${declaringPlayerId} is not in turn order`);
  return [...turnOrder.slice(index + 1), ...turnOrder.slice(0, index + 1)];
}

export function swapBarShape(playerCount: number): { capacity: number; faceDown: number; faceUp: number } {
  if (playerCount === 2) return { capacity: 3, faceDown: 2, faceUp: 1 };
  if (playerCount === 3) return { capacity: 4, faceDown: 2, faceUp: 2 };
  if (playerCount === 4) return { capacity: 5, faceDown: 3, faceUp: 2 };
  throw new Error("Multiplayer supports exactly three or four players");
}

function activeAnchorCount(state: EngineState, playerId: PlayerId): number {
...
export function teamTotals(state: EngineState): Record<string, { anchors: number; points: number }> {
  const totals: Record<string, { anchors: number; points: number }> = {};
...
function validateTeams(state: EngineState, order: PlayerId[], assignments: Record<PlayerId, string>): string | null {
  if (order.length !== 4) return "Teams mode requires exactly four players";
  if (new Set(order).size !== 4 || order.some((id) => !state.players[id])) return "Team turn order must contain four unique existing players";
  const teamIds = order.map((id) => assignments[id]);
  if (teamIds.some((id) => typeof id !== "string" || id.length === 0)) return "Every player requires a team assignment";
  const counts = new Map<string, number>();
  for (const teamId of teamIds) counts.set(teamId!, (counts.get(teamId!) ?? 0) + 1);
  if (counts.size !== 2 || [...counts.values()].some((count) => count !== 2)) return "Teams mode requires exactly two teams of two";
  if (teamIds[0] === teamIds[1] || teamIds[1] === teamIds[2] || teamIds[2] === teamIds[3]) return "Team turn order must alternate teams";
  return null;
}

function validateSetupAssignments(state: EngineState, action: Extract<Phase11Action, { kind: "apply-multiplayer-setup" }>, mode: MultiplayerMode): string | null {
  const expected = mode === "ffa-3" ? [5, 6, 6] : [5, 5, 5, 5];
  if (action.turnOrder.length !== expected.length) return `Expected ${expected.length} players`;
  const all = [...Object.values(action.handAssignments).flat(), ...action.swapBarFaceDownCardIds, ...action.swapBarFaceUpCardIds];
  if (new Set(all).size !== all.length) return "Setup assignments cannot reuse a card";
  if (all.some((id) => state.cards[id]?.zone !== "DP")) return "Every setup card must begin in DP";
  for (let i = 0; i < action.turnOrder.length; i += 1) {
    const id = action.turnOrder[i]!;
    if (!state.players[id]) return `Unknown setup player ${id}`;
    if ((action.handAssignments[id]?.length ?? -1) !== expected[i]) return `${id} must receive ${expected[i]} cards`;
  }
  const shape = swapBarShape(action.turnOrder.length);
  if (action.swapBarFaceDownCardIds.length !== shape.faceDown || action.swapBarFaceUpCardIds.length !== shape.faceUp) return "Swap Bar assignment does not match player-count scaling";
  return null;
}

export function resolvePhase11Action(input: EngineState, actorId: PlayerId, action: Phase11Action): Phase11Resolution {
  if (!input.players[actorId]) return fail("PHASE11_PLAYER", `Unknown actor ${actorId}`);
  const state = canonicalClone(input);
  const runtime = phase11Runtime(state);
  const events: Phase11EventSpec[] = [];

  switch (action.kind) {
    case "configure-multiplayer": {
      if (action.mode === "ffa-3") {
        if (action.turnOrder.length !== 3 || new Set(action.turnOrder).size !== 3 || action.turnOrder.some((id) => !state.players[id])) return fail("MULTIPLAYER_CONFIG", "FFA requires three unique existing players");
        for (const id of action.turnOrder) state.players[id]!.teamId = null;
      } else {
        const problem = validateTeams(state, action.turnOrder, action.teamAssignments ?? {});
        if (problem) return fail("MULTIPLAYER_CONFIG", problem);
        for (const id of action.turnOrder) state.players[id]!.teamId = action.teamAssignments![id]!;
      }
    ...
    case "apply-multiplayer-setup": {
      ...
      if (problem) return fail("MULTIPLAYER_SETUP", problem);
      if (action.mode === "teams-4") {
        const teamProblem = validateTeams(state, action.turnOrder, action.teamAssignments ?? {});
        if (teamProblem) return fail("MULTIPLAYER_SETUP", teamProblem);
      }
      for (const playerId of action.turnOrder) {
        ...
        for (const cardId of action.handAssignments[playerId]!) moveCard(state, cardId, `${playerId}_HAND`, playerId);
      }
      for (const cardId of [...action.swapBarFaceDownCardIds, ...action.swapBarFaceUpCardIds]) moveCard(state, cardId, "SWAP_BAR");
      state.turnOrder = [...action.turnOrder];
      state.activePlayerId = action.turnOrder[0]!;
      runtime.mode = action.mode;
      runtime.teamByPlayer = Object.fromEntries(action.turnOrder.map((id) => [id, state.players[id]!.teamId ?? null]));
    ...
    case "assign-thats-urz": {
      ...
      if (canonicalClone(keys).sort().join("|") !== canonicalClone(action.turnOrder).sort().join("|") || new Set(recipients).size !== action.turnOrder.length || recipients.some((id) => !action.turnOrder.includes(id)) || action.turnOrder.some((id) => assignments[id] === id)) return fail("THATS_URZ_ASSIGNMENT", "Assignments must be a self-free bijection over turn order");
    ...
    case "validate-player-target": {
      ...
      if (problem) return fail("MULTIPLAYER_TARGET", problem, runtime.lastTargetEvaluation);
    ...
    case "record-priority-cycle": {
      ...
      if (action.passOrder.length !== expected.length || action.passOrder.some((id, index) => id !== expected[index])) return fail("MULTIPLAYER_PRIORITY", "Priority must begin with the next player and continue in turn order through the declarer", { expected, actual: action.passOrder });
    ...
    case "assign-tournament-seed": {
      ...
      const assigned: Record<PlayerId, CardId> = {};
      for (const playerId of action.priorityOrder) {
        ...
        if (!chosen) return fail("TOURNAMENT_SEED_POOL", `${playerId} has no available registered High-Impact identity`);
        available.delete(chosen);
        assigned[playerId] = chosen;
      }
      for (const [playerId, cardId] of Object.entries(assigned)) moveCard(state, cardId, `${playerId}_HAND`, playerId);
    ...
    case "intercept-generated-play": {
      ...
      if (!item) return fail("MULTIPLAYER_INTERCEPT", `Missing stack item ${action.stackItemId}`);
      ...
      for (const targetId of item.targetCardIds) {
        ...
        if (!targetController) return fail("MULTIPLAYER_INTERCEPT_TARGET", `Unknown target ${targetId}`);
        const problem = validateMultiplayerTarget(state, action.interceptorId, targetController, true, false);
        if (problem) return fail("MULTIPLAYER_INTERCEPT_TARGET", problem, { targetId, targetController });
      }
    ...
    case "partner-royal-marriage": {
      if (!areAllies(state, action.initiatorId, action.allyId)) return fail("PARTNER_MARRIAGE", "Partner Royal Marriage requires an Ally");
      const king = state.cards[action.kingCardId];
      const queen = state.cards[action.queenCardId];
      if (!king || !queen || king.zone !== `${action.initiatorId}_HAND` || queen.zone !== `${action.allyId}_HAND` || !king.identity.startsWith("K") || !queen.identity.startsWith("Q") || king.identity.slice(1) !== queen.identity.slice(1)) return fail("PARTNER_MARRIAGE", "Matching King and Queen must be held by the initiating Allies");
    ...
    case "resolve-team-endgame": {
      ...
      if (action.kindOfCheck === "normal") {
        ...
        if (!active?.teamId) return fail("TEAM_ENDGAME", "Normal team victory requires an active team player");
        if (deriveSecuredPoints(state, action.activePlayerId) >= active.goal) winningTeamId = active.teamId;
      } else if (action.kindOfCheck === "sudden-death") {
        const activator = state.players[action.activatorId ?? ""];
        if (!activator?.teamId) return fail("TEAM_ENDGAME", "Sudden Death activator must belong to a team");
