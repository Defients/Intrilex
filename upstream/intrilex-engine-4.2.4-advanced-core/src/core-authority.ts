...
import { compareScuttle, cardPointValue, hasOrdinaryScuttleImmunity, parseIdentity } from "./ranks.js";
...
import { CORE_ADVANCED_AUTHORITY_PROFILE, advancedSourceIds, advancedTargetIds, resolveAdvancedCoreAction } from "./core-advanced.js";
import type { CardId, CoreAuthorityAction, CoreAuthorityProfileId, CoreFoundationActionType, CorePrimaryAction, CoreStackPayload, EngineCommand, EngineEvent, EngineState, PlayerId, StackItem, StackItemId, Visibility } from "./types.js";
...
export const CORE_FOUNDATION_AUTHORITY_PROFILE = Object.freeze({
  ...
  supportedActions: ["face-down-swap", "draw", "face-up-swap", "play-for-points", "scuttle", "pass"] as const,
...
function isSupportedCoreProfile(profileId: unknown): profileId is CoreAuthorityProfileId {
  return profileId === CORE_FOUNDATION_AUTHORITY_PROFILE.id || profileId === CORE_EFFECT_DECLARATION_PROFILE.id || profileId === CORE_RESPONSE_AUTHORITY_PROFILE.id || profileId === CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id || profileId === CORE_ADVANCED_AUTHORITY_PROFILE.id;
}
...
function isResponseRuntime(state: EngineState): boolean { const id = runtime(state)?.profileId; return id === CORE_RESPONSE_AUTHORITY_PROFILE.id || id === CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id || id === CORE_ADVANCED_AUTHORITY_PROFILE.id; }
...
function targetAcceptsBaseAce(target: StackItem): boolean {
  ...
  if (payload.kind === "primary") return ["ordinary-effect", "anchor"].includes(payload.stackClass);
...
export function resolveCoreAuthorityAction(input: EngineState, actorId: PlayerId, action: CoreAuthorityAction): CoreActionResolution | CoreActionFailure {
  ...
  switch (action.kind) {
    ...
    case "core-begin-start": {
      ...
      if (core.profileId === CORE_ADVANCED_AUTHORITY_PROFILE.id) { const snapshot=captureCoreVoltageSnapshot(state,actorId); events.push({type:"VOLTAGE_SNAPSHOT_CAPTURED",payload:{playerId:actorId,snapshot}}); }
    ...
    case "core-score": {
      ...
      if (runtime(state)?.profileId === CORE_ADVANCED_AUTHORITY_PROFILE.id && (coreRank(state, card.id) === "7" || card.identity === "10♣" || card.identity === "BJ")) return fail("CORE_SCORING_RIDER_UNSUPPORTED", `${card.identity} requires an uncertified scoring rider in the Advanced Core profile`);
    ...
    case "core-resolve-advanced": {
      ...
      if (isVoltage) {
        const core = requireRuntime(state); if (typeof core === "string" || core.profileId !== CORE_ADVANCED_AUTHORITY_PROFILE.id) return fail("CORE_ADVANCED_PROFILE", "Advanced Core Authority profile is not active");
    ...
    case "core-pass": {
      const problem = validateActionWindow(state, actorId); if (problem) return fail("CORE_ACTION", problem);
      consumeMiniTurn(state, actorId);
      events.push({ type: "CORE_PASS_RESOLVED", payload: { playerId: actorId } });
      break;
    }
    ...
    case "core-pass-priority": {
      ...
      events.push({ type: "CORE_PRIORITY_PASSED", payload: { playerId: actorId, consecutivePasses: priority.consecutivePasses } });
      if (priority.consecutivePasses >= priority.order.length) { priority.open = false; events.push({ type: "CORE_PRIORITY_CLOSED", payload: { stackDepth: state.stack.length } }); }
      break;
    }
    ...
    case "core-declare-rank10-stack-theft": {
      ...
      const target = topStackItem(state, action.targetStackItemId); const tp = target?.coreAuthority;
      if (!target || tp?.kind !== "primary" || target.sourceCardIds.length !== 1 || ["ultra","royal-marriage","voltage"].includes(tp.stackClass)) return fail("CORE_STACK_THEFT_TARGET", "Stack Theft requires a pending single effect play");
