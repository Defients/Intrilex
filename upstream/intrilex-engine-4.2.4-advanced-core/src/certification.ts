...
export interface FinalReleaseCertification {
  ...
  sourceTestCoverage: { canonicalTotal: 120; uniqueSourceIds: number; missingSourceIds: string[]; duplicateSourceIds: string[] };
  independentRuntime: { runtime: string; verifiedPairCount: number; failureCount: number; aggregateHash: string };
...
function invalidProbe(id: string): EngineCommand {
...
export async function runReleaseCandidateCertification(projectRoot: string): Promise<FinalReleaseCertification> {
  ...
  const publicLeakFailures:string[]=[];
  for(const name of replayFiles){
    const replay=parseCertifiedReplay(await readFile(path.join(projectRoot,"replays",name),"utf8")); verifyCertifiedReplay(replay);
    const publicText=canonicalize(publicCertifiedReplayView(replay));
    for(const token of ["rngBefore","rngAfter","stateHashBefore","stateHashAfter","initialStateHash","finalStateHash","selectedCardId"]) if(publicText.includes(token)) publicLeakFailures.push(`${name}: leaked ${token}`);
    if(replay.fixtureId==="CT-120"&&(publicText.includes("A♠")||publicText.includes("K♣"))) publicLeakFailures.push(`${name}: leaked CT-120 private identity`);
  }
  const engine=new IntrilexEngine(); const invariantFuzzFailures:string[]=[]; let invariantFuzzCases=0; const sourceIds:string[]=[];
  for(const file of FIXTURE_FILES) for(const fixture of await loadFixtures(path.join(projectRoot,"fixtures",file))){
    ...
    for(let repeat=0;repeat<5;repeat+=1){ invariantFuzzCases+=1; const before=hashCanonical(fixture.initialState); const result=engine.execute(fixture.initialState,invalidProbe(`${fixture.id}-${repeat}`)); if(result.accepted||result.events.length!==0||hashCanonical(result.state)!==before) invariantFuzzFailures.push(`${fixture.id}#${repeat}`); }
  }
  const counts=new Map<string,number>(); sourceIds.forEach(id=>counts.set(id,(counts.get(id)??0)+1));
  const canonical=Array.from({length:120},(_,i)=>`CT-${String(i+1).padStart(3,"0")}`); const sourceSet=new Set(sourceIds);
  const missingSourceIds=canonical.filter(id=>!sourceSet.has(id)); const duplicateSourceIds=[...counts.entries()].filter(([,n])=>n>1).map(([id])=>id).sort();
  const independent=await readJson<{verdict:string;runtime:string;verifiedPairCount:number;failureCount:number;aggregateHash:string}>(path.join(projectRoot,"reports","independent-python-replay-verification.json"));
  const preserved=await readJson<{aggregateHash:string}>(path.join(projectRoot,"reports","phase2-18-regression-report.json"));
  const combined=await readJson<{aggregateHash:string}>(path.join(projectRoot,"reports","conformance-report.json"));
  const simulation=await readJson<{fullMatchCampaign?:{campaignHash?:string};deterministicReproduction?:{matched?:boolean}}>(path.join(projectRoot,"reports","simulation-baseline.json"));
  const boundaries=[
  ...
  const core={
    engineVersion:"4.1.0" as const, certifiedReplayCount:replayFiles.length, publicLeakFailures, invariantFuzzCases,invariantFuzzFailures,
    sourceTestCoverage:{canonicalTotal:120 as const,uniqueSourceIds:sourceSet.size,missingSourceIds,duplicateSourceIds},
  ...
  const report:FinalReleaseCertification={verdict:pass?"PASS":"FAIL",...core,aggregateHash:hashCanonical(core)};
