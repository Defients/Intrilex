...
const profile=(s:EngineState)=>(s.metadata.coreAuthority as any)?.profileId===CORE_ADVANCED_AUTHORITY_PROFILE.id;
...
export function isAdvancedProfile(s:Readonly<EngineState>):boolean{return (s.metadata.coreAuthority as any)?.profileId===CORE_ADVANCED_AUTHORITY_PROFILE.id;}
...
export function advancedSourceIds(a:CoreAdvancedAction):CardId[]{
  switch(a.kind){
    ...
    case "advanced-super-two":case "advanced-super-four-exchange":case "advanced-super-eight-scuttle":case "advanced-super-j-tempo":case "advanced-ultra-three-black":case "advanced-ultra-two-black-two-red": return [...a.sourceCardIds];
...
export function advancedTargetIds(a:CoreAdvancedAction):CardId[]{switch(a.kind){case "advanced-super-two":case "advanced-super-eight-scuttle":return [a.targetCardId];case "advanced-rank10-spade-recovery":return [a.recoverCardId];default:return [];}}
export function advancedStackClass(a:CoreAdvancedAction):"super"|"ultra"|"royal-marriage"|"rank10"|"voltage"{if(a.kind==="advanced-royal-marriage")return "royal-marriage";if(a.kind.startsWith("advanced-super"))return "super";if(a.kind.startsWith("advanced-ultra"))return "ultra";if(a.kind.startsWith("advanced-rank10"))return "rank10";return "voltage";}
...
export function resolveAdvancedCoreAction(input:EngineState,actorId:PlayerId,a:CoreAdvancedAction):AdvancedResolution{
  ...
  switch(a.kind){
    ...
    case "advanced-rank10-club-foundation": {
      return fail(
        "RANK10_CLUB_UNSUPPORTED",
        "10♣ requires an engine-owned queued scoring trigger"
      );
    }
    ...
    case "advanced-voltage-four": {
      return fail(
        "VOLTAGE_MODE_UNSUPPORTED",
        "Voltage Three and Four require sealed hidden-choice continuations"
      );
    }
    case "advanced-voltage-five": {
      if (a.branch !== "gy-bottom") {
        return fail(
          "VOLTAGE_MODE_UNSUPPORTED",
          "Voltage Five refine requires a sealed post-draw discard continuation"
        );
      }
