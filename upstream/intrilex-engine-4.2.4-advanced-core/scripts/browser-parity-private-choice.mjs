...
const reportPath = path.join(root, 'reports/browser-private-choice-parity.json');
...
const work = await mkdtemp(path.join(tmpdir(), 'intrilex-414-browser-'));
...
await writeFile(path.join(engineDir, 'browser-entry.js'), [
  ...
  "export { FIRST_CONTACT_PRIVATE_CHOICE_AUTHORITY_PROFILE, createMatchState, enumerateLegalActions, authorizedLegalActionView, advanceToDecision, autonomousCapabilities, runRandomLegalMatch } from './autonomy.js';",
  ''
].join('\n'));
...
const setup = { profileId: 'first-contact-private-choice', playerIds: ['P1', 'P2'], enabledModules: [], eventApprovedModules: [], seed: 0x414c001, seatOrder: ['P1', 'P2'] };
...
const profileDir = await mkdtemp(path.join(tmpdir(), 'intrilex-414-chromium-'));
...
try {
  ...
  const browserProgram = `async () => {
    ...
    const match = __engine.runRandomLegalMatch({profileId:'first-contact-private-choice',playerIds:['P1','P2'],enabledModules:[],eventApprovedModules:[],seed:0x414c001,seatOrder:['P1','P2']},5000);
    return {
  ...
  const report = {
    ...
    certifiedReplayCorpus: nodeReplay, privateChoiceAuthorityMatch: nodeMatch,
  ...
  console.log(`BROWSER PRIVATE CHOICE PARITY PASS: replays=${nodeReplay.replayCount}; replayHash=${nodeReplay.aggregateHash}; matchHash=${nodeMatch.finalStateHash}`);
} finally {
