> **v4.1.1 Interrupt Timing Hotfix — Governing Ruling**
> `Interrupt` is timing authority only. It does not inherently spend a Mini-Turn, create a Full-Turn skip, or create any other penalty. Only explicit printed effects create skips. `10♠ Stack Theft` keeps its printed Full-Turn skip consequences. `Time Bomb Defuse` keeps its printed Action-Phase skip. Courageous Assault, Rejuvenation, and Red Nine — Goal Shock create no inherited skip.

# INTRILEX — Complete Player Rulebook v4.1

> **Edition:** Player-focused complete rules  
> **Rules basis:** Canonical v4.0 contract plus the accepted July 2026 forensic clarification set, canonized as v4.1  
...
7. **Pass** — Do nothing; the Mini-Turn is still spent.
...
- **Interrupt:** during any legal response window, including another player's FT. It is timing authority only and creates no inherent Mini-Turn cost or skip.
- **Interrupt with a printed penalty:** uses Interrupt timing and applies only the consequence explicitly written by that card or module.
...
- **Interrupt:** playable during any legal response window, including another player's FT, without spending a Mini-Turn. The keyword itself creates no Full-Turn skip, Action-Phase skip, or other penalty.
- **Printed Interrupt consequence:** a card or module may explicitly create a skip or another cost. Apply only that printed consequence.

Pending skips still stack when a specific rule creates them. Each Full-Turn skip consumes the next scheduled turn slot of that player that has not already begun.
...
- its controller gains the explicitly printed Stack Theft Full-Turn skip;
- the original caster gains no skip because Stack Theft did not resolve;
...
Interrupt defines Courageous Assault's legal timing only. It creates no Mini-Turn cost and no pending skip.
...
Interrupt defines Rejuvenation's legal timing only. It creates no Mini-Turn cost and no pending skip.
...
Interrupt defines Goal Shock's legal timing only. It creates no Mini-Turn cost and no pending skip.
...
This Action-Phase skip is a separately printed Defuse consequence; it is not inherited from the Interrupt keyword.
...
- **Interrupt:** Free play during any legal response window, including another player's FT. Timing authority only; no inherent turn tax.
...
#### Does declaring an Interrupt automatically skip my next Full Turn?

No. As of the v4.1.1 hotfix, Interrupt is timing authority only. It creates no inherent Mini-Turn cost, Full-Turn skip, or Action-Phase skip.

#### Which Interrupts still create skips?

Only rules with explicit printed consequences. `10♠ Stack Theft` retains its printed Full-Turn skips. `Time Bomb Defuse` retains its printed Action-Phase skip. Courageous Assault, Rejuvenation, and Red Nine — Goal Shock create no skip.
