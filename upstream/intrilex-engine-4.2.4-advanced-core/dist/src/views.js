...
export function publicStateView(state) {
    ...
    for (const card of Object.values(clone.cards)) {
        ...
        if (card.zone.endsWith("_HAND") || card.state.faceDownTrap === true || card.state.swapBarFaceDown === true || (card.zone === "STAGING" && card.state.draftFaceUp === false) || privateChoiceHidden)
...
export function privateStateView(state, viewerId) {
    ...
    for (const card of Object.values(clone.cards)) {
        ...
        if (((card.zone.endsWith("_HAND") || card.state.faceDownTrap === true || card.state.swapBarFaceDown === true) && card.controllerId !== viewerId) || (card.zone === "STAGING" && card.state.draftFaceUp === false) || privateChoiceHidden)
            card.identity = "HIDDEN";
        delete card.state.privateChoiceHeldBy;
        delete card.state.privateChoicePublicReveal;
        delete card.state.privateChoiceSource;
