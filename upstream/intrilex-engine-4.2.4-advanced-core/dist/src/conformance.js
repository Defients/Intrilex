...
function assertExpectation(fixture, state, accepted) {
    const failures = [];
    const exp = fixture.expectation;
    if (exp.accepted !== undefined && !equal(exp.accepted, accepted))
        failures.push(`accepted mismatch: expected ${canonicalize(exp.accepted)}, got ${canonicalize(accepted)}`);
    ...
    if (final.hands !== undefined) {
        for (const [playerId, expected] of Object.entries(final.hands)) {
            ...
            if (!equal(expected, actual))
                failures.push(`hand ${playerId}: expected ${canonicalize(expected)}, got ${canonicalize(actual)}`);
        }
    }
    if (final.playerZones !== undefined) {
        for (const [playerId, expectedZones] of Object.entries(final.playerZones)) {
            ...
            if (!player) {
                failures.push(`missing player ${playerId}`);
                continue;
            }
            for (const field of ["hand", "pr", "er"]) {
                const expected = expectedZones[field];
                if (expected !== undefined && !equal(expected, player[field]))
                    failures.push(`${playerId}.${field}: expected ${canonicalize(expected)}, got ${canonicalize(player[field])}`);
            }
        }
    }
    if (final.goals !== undefined) {
        for (const [playerId, expected] of Object.entries(final.goals)) {
            ...
            if (actual !== expected)
                failures.push(`goal ${playerId}: expected ${expected}, got ${actual}`);
        }
    }
    if (final.securedPoints !== undefined) {
        for (const [playerId, expected] of Object.entries(final.securedPoints)) {
            ...
            if (actual !== expected)
                failures.push(`secured points ${playerId}: expected ${expected}, got ${actual}`);
        }
    }
    if (final.playerLimits !== undefined) {
        for (const [playerId, expected] of Object.entries(final.playerLimits)) {
            ...
            if (!actual) {
                failures.push(`missing limits for ${playerId}`);
                continue;
            }
            for (const [key, value] of Object.entries(expected)) {
                if (!equal(actual[key], value))
                    failures.push(`limit ${playerId}.${key}: expected ${canonicalize(value)}, got ${canonicalize(actual[key])}`);
            }
        }
    }
    if (final.zones !== undefined) {
        for (const [zone, expected] of Object.entries(final.zones)) {
            const actual = state.zones[zone];
            if (!equal(expected, actual))
                failures.push(`zone ${zone}: expected ${canonicalize(expected)}, got ${canonicalize(actual)}`);
        }
    }
    if (final.cardZones !== undefined) {
        for (const [cardId, expected] of Object.entries(final.cardZones)) {
            ...
            if (actual !== expected)
                failures.push(`card zone ${cardId}: expected ${expected}, got ${actual}`);
        }
    }
    if (final.controllers !== undefined) {
        for (const [cardId, expected] of Object.entries(final.controllers)) {
            ...
            if (actual !== expected)
                failures.push(`controller ${cardId}: expected ${expected}, got ${actual}`);
        }
    }
    if (final.originalOwners !== undefined) {
        for (const [cardId, expected] of Object.entries(final.originalOwners)) {
            ...
            if (actual !== expected)
                failures.push(`original owner ${cardId}: expected ${expected}, got ${actual}`);
        }
    }
    if (final.activePlayerId !== undefined && state.activePlayerId !== final.activePlayerId)
        failures.push(`active player: expected ${final.activePlayerId}, got ${state.activePlayerId}`);
    if (final.fullTurnSequence !== undefined && state.fullTurnSequence !== final.fullTurnSequence)
        failures.push(`full turn sequence: expected ${final.fullTurnSequence}, got ${state.fullTurnSequence}`);
    if (final.startPhaseSequenceByPlayer !== undefined) {
        for (const [playerId, expected] of Object.entries(final.startPhaseSequenceByPlayer)) {
            ...
            if (actual !== expected)
                failures.push(`start sequence ${playerId}: expected ${expected}, got ${actual}`);
        }
    }
    if (final.phase !== undefined && state.phase !== final.phase)
        failures.push(`phase: expected ${final.phase}, got ${state.phase}`);
    if (final.winner !== undefined && state.winner !== final.winner)
        failures.push(`winner: expected ${final.winner}, got ${state.winner}`);
    if (final.stackDepth !== undefined && state.stack.length !== final.stackDepth)
        failures.push(`stackDepth: expected ${final.stackDepth}, got ${state.stack.length}`);
    if (final.triggerQueueDepth !== undefined && state.triggerQueue.length !== final.triggerQueueDepth)
        failures.push(`triggerQueueDepth: expected ${final.triggerQueueDepth}, got ${state.triggerQueue.length}`);
    if (final.markers !== undefined) {
        for (const [cardId, expected] of Object.entries(final.markers)) {
            const actual = state.cards[cardId]?.state;
            for (const [key, value] of Object.entries(expected)) {
                if (!equal(actual?.[key], value))
                    failures.push(`marker ${cardId}.${key}: expected ${canonicalize(value)}, got ${canonicalize(actual?.[key])}`);
            }
        }
    }
    if (final.absentMarkers !== undefined) {
        for (const [cardId, keys] of Object.entries(final.absentMarkers)) {
            ...
            for (const key of keys) {
                if (actual !== undefined && Object.prototype.hasOwnProperty.call(actual, key))
                    failures.push(`marker ${cardId}.${key}: expected absent, got ${canonicalize(actual[key])}`);
            }
        }
    }
    if (final.metadata !== undefined) {
        for (const [key, value] of Object.entries(final.metadata)) {
            if (!equal(state.metadata[key], value))
                failures.push(`metadata ${key}: expected ${canonicalize(value)}, got ${canonicalize(state.metadata[key])}`);
        }
    }
    return failures;
}
function assertCt120PublicRedaction(replay) {
    if (replay.fixtureId !== "CT-120")
        return [];
    const text = canonicalize(publicReplayView(replay));
    const failures = [];
    if (text.includes("selectedCardId"))
        failures.push("CT-120 public replay leaked hidden choice key selectedCardId");
    if (text.includes('"A♠"') || text.includes('"K♣"'))
        failures.push("CT-120 public replay leaked a private hand identity");
    if (!text.includes("redacted"))
        failures.push("CT-120 public replay contains no redaction marker");
    return failures;
}
export async function loadFixtures(fixturePath) {
    const raw = await readFile(fixturePath, "utf8");
    return parseFixtures(JSON.parse(raw));
}
async function executeFixture(projectRoot, fixture, repeatCount) {
    const failures = [];
    const initialValidation = validateState(fixture.initialState);
    if (initialValidation.length > 0)
        failures.push(`initial validation failed: ${canonicalize(initialValidation)}`);
    const roundTripped = roundTripState(fixture.initialState);
    if (!equal(roundTripped, fixture.initialState))
        failures.push("round-trip serialization changed initial state");
    ...
    try {
        replayAndVerify(replay);
    }
    catch (error) {
        failures.push(`replay verification failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    for (let run = 2; run <= repeatCount; run += 1) {
        ...
        if (currentSignature !== signature)
            failures.push(`run ${run} diverged from run 1`);
    }
    ...
    try {
        verifyCertifiedReplay(certified);
    }
    catch (error) {
        failures.push(`certified replay verification failed: ${error instanceof Error ? error.message : String(error)}`);
    }
...
function buildReport(fixtureFiles, reports, repeatCount) {
...
async function writeReport(projectRoot, report, basename, title) {
...
export async function runConformance(projectRoot, repeatCount = 5) {
    ...
    for (const file of suiteFiles) {
        ...
        for (const fixture of fixtures)
            reports.push(await executeFixture(projectRoot, fixture, repeatCount));
    ...
    const phase7Reports = phase7Order.map((id) => phase7Map.get(id)).filter((entry) => entry !== undefined);
    ...
    const phase8Reports = phase8Order.map((id) => phase8Map.get(id)).filter((entry) => entry !== undefined);
    ...
    const phase10Reports = phase10Order.map((id) => phase10Map.get(id)).filter((entry) => entry !== undefined);
    ...
    const phase12Reports = phase12Order.map((id) => phase12Map.get(id)).filter((entry) => entry !== undefined);
    ...
    const phase13Reports = phase13Order.map((id) => phase13Map.get(id)).filter((entry) => entry !== undefined);
    ...
    const phase14Reports = phase14Order.map((id) => phase14Map.get(id)).filter((entry) => entry !== undefined);
    ...
    const select = (ids) => ids.map((id) => allMap.get(id)).filter((entry) => entry !== undefined);
