...
export const CORE_FOUNDATION_AUTHORITY_PROFILE = Object.freeze({
    ...
    goal: 21,
    enabledModules: [],
    supportedActions: ["face-down-swap", "draw", "face-up-swap", "play-for-points", "scuttle", "pass"],
...
function isResponseRuntime(state) { const id = runtime(state)?.profileId; return id === CORE_RESPONSE_AUTHORITY_PROFILE.id || id === CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id || id === CORE_ADVANCED_AUTHORITY_PROFILE.id; }
...
export function resolveCoreAuthorityAction(input, actorId, action) {
    ...
    switch (action.kind) {
        ...
        case "core-score": {
            ...
            if (runtime(state)?.profileId === CORE_ADVANCED_AUTHORITY_PROFILE.id && (coreRank(state, card.id) === "7" || card.identity === "10♣" || card.identity === "BJ"))
        ...
        case "core-resolve-advanced": {
            ...
            if (isVoltage) {
                const core = requireRuntime(state);
                if (typeof core === "string" || core.profileId !== CORE_ADVANCED_AUTHORITY_PROFILE.id)
        ...
        case "core-pass-priority": {
            ...
            if (priority.consecutivePasses >= priority.order.length) {
                priority.open = false;
                events.push({ type: "CORE_PRIORITY_CLOSED", payload: { stackDepth: state.stack.length } });
            }
            break;
        }
        ...
        case "core-declare-rank10-stack-theft": {
            ...
            const target = topStackItem(state, action.targetStackItemId);
            const tp = target?.coreAuthority;
            if (!target || tp?.kind !== "primary" || target.sourceCardIds.length !== 1 || ["ultra", "royal-marriage", "voltage"].includes(tp.stackClass))
