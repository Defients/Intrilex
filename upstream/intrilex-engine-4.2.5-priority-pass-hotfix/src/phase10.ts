...
export interface Phase10EventSpec { type: string; payload: Record<string, unknown>; visibility?: "public" | "private" | "authorized"; }
export type Phase10Resolution =
  | { ok: true; state: EngineState; events: Phase10EventSpec[] }
  | { ok: false; code: string; message: string; details?: unknown };

const TRAP_IDENTITIES = new Set(["4♠", "4♥", "4♦", "4♣", "5♠", "5♥", "5♦", "5♣"]);

function fail(code: string, message: string, details?: unknown): Phase10Resolution {
  return details === undefined ? { ok: false, code, message } : { ok: false, code, message, details };
}

export function trapRuntime(state: EngineState): TrapRuntime {
  const current = state.metadata.phase10 as Partial<TrapRuntime> | undefined;
  return {
    enabled: true,
...
function saveRuntime(state: EngineState, runtime: TrapRuntime): void {
  state.metadata.phase10 = runtime;
}

function boardLockActive(state: EngineState): boolean {
  const phase8 = state.metadata.phase8 as { boardLock?: unknown } | undefined;
  return phase8?.boardLock !== null && phase8?.boardLock !== undefined;
}

function requireTrap(state: EngineState, trapCardId: CardId): { ok: true; card: EngineState["cards"][CardId] } | { ok: false; reason: string } {
  const card = state.cards[trapCardId];
  if (!card) return { ok: false, reason: `Unknown Trap ${trapCardId}` };
  if (card.state.faceDownTrap !== true) return { ok: false, reason: `${trapCardId} is not a face-down Trap` };
  return { ok: true, card };
}

function disabledTrap(card: EngineState["cards"][CardId]): DisabledTrapState | null {
  const value = card.state.disabledTrap;
  if (typeof value !== "object" || value === null) return null;
  const candidate = value as Partial<DisabledTrapState>;
  return typeof candidate.counteringPlayerId === "string" && Number.isInteger(candidate.expiresAfterCompletedFullTurnSequence)
    ? candidate as DisabledTrapState
    : null;
}

function controlledTrapCount(state: EngineState, playerId: PlayerId): number {
...
function revealAndScrap(state: EngineState, trapCardId: CardId): void {
  const card = state.cards[trapCardId]!;
...
function clearOpponentPrForTotalPressure(state: EngineState, opponentId: PlayerId): CardId[] {
  const player = state.players[opponentId];
  if (!player) return [];
  const cleared: CardId[] = [];
  for (const id of [...player.pr]) {
    const card = state.cards[id];
    if (!card || hasAegis(card)) continue;
...
function expireDisabledTraps(state: EngineState, playerId: PlayerId, completedCount: number): CardId[] {
  const expired: CardId[] = [];
  for (const card of Object.values(state.cards)) {
    const disabled = disabledTrap(card);
    if (!disabled || disabled.counteringPlayerId !== playerId || disabled.expiresAfterCompletedFullTurnSequence > completedCount) continue;
    delete card.state.disabledTrap;
...
export function validateTrapPlacement(state: EngineState, actorId: PlayerId, cardId: CardId, row: "pr" | "er"): string | null {
  if (boardLockActive(state)) return "Board Lock prohibits Trap placement";
  const card = state.cards[cardId];
  if (!card || card.controllerId !== actorId || card.zone !== `${actorId}_HAND`) return "Trap source must be controlled in hand";
  if (controlledTrapCount(state, actorId) >= 2) return "A player may control at most two Traps OTT";
  if (row !== "pr" && row !== "er") return "Trap row must be PR or ER";
  return null;
}

export function resolvePhase10Action(input: EngineState, actorId: PlayerId, action: Phase10Action): Phase10Resolution {
  if (!input.players[actorId]) return fail("PHASE10_PLAYER", `Unknown actor ${actorId}`);
  const state = canonicalClone(input);
  const runtime = trapRuntime(state);
  const events: Phase10EventSpec[] = [];

  switch (action.kind) {
    case "place-trap": {
      const problem = validateTrapPlacement(state, actorId, action.cardId, action.row);
      if (problem) return fail("TRAP_PLACEMENT", problem);
      const destination: ZoneName = action.row === "pr" ? `${actorId}_PR` : `${actorId}_ER`;
    ...
    case "check-trigger": {
      ...
      if (!found.ok) return fail("TRAP_TRIGGER", found.reason);
      if (boardLockActive(state)) return fail("TRAP_SUPPRESSED", "Board Lock suppresses Trap triggers");
      if (disabledTrap(found.card)) return fail("TRAP_DISABLED", "Disabled Trap cannot trigger");
      if (action.duringAtomicResolution) return fail("TRAP_ATOMIC", "Trap cannot trigger during atomic resolution");
      if (!action.qualifyingEvent) return fail("TRAP_INELIGIBLE", "Trap condition is not satisfied");
      if (runtime.triggerUsedThisActiveFT[found.card.controllerId] === true) return fail("TRAP_TRIGGER_LIMIT", "Trap trigger limit already used this active FT");
    ...
    case "resolve-total-pressure": {
      const found = requireTrap(state, action.trapCardId);
      if (!found.ok || found.card.identity !== "4♠") return fail("TRAP_IDENTITY", "Total Pressure requires a face-down 4♠ Trap");
      if (boardLockActive(state)) return fail("TRAP_SUPPRESSED", "Board Lock suppresses Trap triggers");
    ...
    case "reveal-trap-as-effect": {
      const found = requireTrap(state, action.trapCardId);
      if (!found.ok) return fail("TRAP_REVEAL", found.reason);
    ...
    case "module3-counter": {
      const found = requireTrap(state, action.trapCardId);
      if (!found.ok) return fail("TRAP_COUNTER", found.reason);
      const counter = state.cards[action.counterCardId];
      if (!counter || counter.controllerId !== actorId || counter.zone !== `${actorId}_HAND` || !/^(3♣|3♦|3♥)$/.test(counter.identity)) return fail("TRAP_COUNTER_SOURCE", "Module-3 counter requires a controlled non-Spade 3 in hand");
      if (runtime.module3CounterPending || runtime.module3CounterUsedFullTurnSequence === state.fullTurnSequence) return fail("TRAP_COUNTER_LIMIT", "Global module-3 Trap counter limit already pending or used this FT");
    ...
    case "complete-full-turn": {
      ...
      if (!state.players[playerId]) return fail("TRAP_TURN", `Unknown player ${playerId}`);
    ...
    case "resolve-jacked-points": {
      const found = requireTrap(state, action.trapCardId);
      if (!found.ok || found.card.identity !== "5♣") return fail("TRAP_IDENTITY", "Jacked Points requires a face-down 5♣ Trap");
      const scoreCard = state.cards[action.pendingScoreCardId];
      if (!scoreCard || scoreCard.zone !== `${action.scoringPlayerId}_HAND`) return fail("TRAP_PENDING_SCORE", "Scoring card must still be held as a pending non-stack declaration");
      revealAndScrap(state, action.trapCardId);
