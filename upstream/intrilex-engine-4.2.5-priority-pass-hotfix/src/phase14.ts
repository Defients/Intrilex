...
export type Phase14Resolution =
  | { ok: true; state: EngineState; events: Phase14EventSpec[] }
  | { ok: false; code: string; message: string; details?: unknown };

function fail(code: string, message: string, details?: unknown): Phase14Resolution {
  return details === undefined ? { ok: false, code, message } : { ok: false, code, message, details };
}

export const DEFAULT_DEFFY_ADDONS: DeffyAddOns = Object.freeze({
  speedRun: false,
  thatsUrz: false,
  thirdPartied: false,
  mirrorMe: false
...
export function phase14Runtime(state: EngineState): Phase14Runtime {
  const current = state.metadata.phase14 as Partial<Phase14Runtime> | undefined;
  return {
...
function saveRuntime(state: EngineState, runtime: Phase14Runtime): void {
  state.metadata.phase14 = runtime;
}

export function deffyPoolShape(subMode: DeffySubMode, playerCount: number): { total: number | "all"; faceUp: number | "all"; faceDown: number; privatePools: boolean } {
  switch (subMode) {
    ...
    case "soda": {
      if (playerCount !== 2) throw new Error("Soda is available only with exactly two players");
...
export function normalDeffyTargets(turnOrder: PlayerId[]): Record<PlayerId, number> {
  if (turnOrder.length === 2) return { [turnOrder[0]!]: 5, [turnOrder[1]!]: 6 };
  if (turnOrder.length === 3) return { [turnOrder[0]!]: 5, [turnOrder[1]!]: 6, [turnOrder[2]!]: 6 };
  if (turnOrder.length === 4) return Object.fromEntries(turnOrder.map((id) => [id, 5]));
  throw new Error("Deffy Mode supports two to four players");
}

export function validateAssignmentBijection(turnOrder: PlayerId[], assignments: Record<PlayerId, PlayerId>): string | null {
  if (turnOrder.length < 2 || new Set(turnOrder).size !== turnOrder.length) return "Draft order must contain unique players";
  const sources = Object.keys(assignments).sort();
  const expected = [...turnOrder].sort();
  if (JSON.stringify(sources) !== JSON.stringify(expected)) return "Every drafter requires exactly one assignment";
  const targets = turnOrder.map((id) => assignments[id]);
  if (targets.some((id) => id === undefined || !turnOrder.includes(id))) return "Every assignment target must be a draft participant";
  if (new Set(targets).size !== turnOrder.length) return "That's Urz assignment must be bijective";
  if (turnOrder.some((id) => assignments[id] === id)) return "That's Urz assignment must be self-free";
  return null;
}

export function deterministicShuffle<T>(values: readonly T[], state: EngineState): T[] {
  const output = [...values];
...
function isPoolCard(state: EngineState, cardId: CardId): boolean {
  return state.cards[cardId]?.zone === "STAGING" && state.zones.staging.includes(cardId);
}

function nextEligibleDrafter(runtime: Phase14Runtime): PlayerId | null {
...
function pickCard(state: EngineState, runtime: Phase14Runtime, drafterId: PlayerId, cardId: CardId, events: Phase14EventSpec[]): string | null {
  if (runtime.status !== "drafting") return "Draft is not active";
  const expectedDrafter = nextEligibleDrafter(runtime);
  if (expectedDrafter !== drafterId) return `Expected drafter ${String(expectedDrafter)}`;
  if (!isPoolCard(state, cardId)) return `${cardId} is not in the draft pool`;
  const recipientId = runtime.assignmentByDrafter[drafterId] ?? drafterId;
  const target = runtime.targetHandSizes[recipientId];
  if (target === undefined) return `Missing target size for ${recipientId}`;
  if ((runtime.draftedFor[recipientId]?.length ?? 0) >= target) return `${recipientId} already reached target size`;
  ...
  if (totalPicks === 0 && drafterId === runtime.draftOrder[0] && !faceUp) return "The starting player's first pick cannot be face-down";
  const faceDownLimit = runtime.subMode === "mystery-mix" ? Number.POSITIVE_INFINITY : 2;
  if (!faceUp && (runtime.faceDownPicksByDrafter[drafterId] ?? 0) >= faceDownLimit) return `${drafterId} reached the face-down pick cap`;
  ...
  if (faceUp) runtime.faceUpPicksByDrafter[drafterId] = (runtime.faceUpPicksByDrafter[drafterId] ?? 0) + 1;
  else runtime.faceDownPicksByDrafter[drafterId] = (runtime.faceDownPicksByDrafter[drafterId] ?? 0) + 1;
...
function validateTargetSizes(state: EngineState, runtime: Phase14Runtime): string | null {
  for (const playerId of runtime.draftOrder) {
    const expected = runtime.targetHandSizes[playerId];
    if (!Number.isInteger(expected) || expected! < 1) return `Invalid target size for ${playerId}`;
    if (state.players[playerId]!.hand.length !== expected) return `${playerId} has ${state.players[playerId]!.hand.length}; expected ${expected}`;
  }
  if (runtime.subMode === "mystery-mix") {
    for (const drafter of runtime.draftOrder) if ((runtime.faceUpPicksByDrafter[drafter] ?? 0) < 1) return `${drafter} must take at least one face-up card`;
  }
  return null;
}

function configureRuntime(state: EngineState, action: Extract<Phase14Action, { kind: "configure-deffy" }>): Phase14Runtime | string {
  if (state.stack.length > 0 || state.pendingDeclaration !== null || state.priority?.open === true) return "Deffy setup cannot begin while gameplay objects are pending";
  if (action.turnOrder.length < 2 || action.turnOrder.length > 4 || new Set(action.turnOrder).size !== action.turnOrder.length || action.turnOrder.some((id) => !state.players[id])) return "Draft order must contain two to four unique existing players";
  try { deffyPoolShape(action.subMode, action.turnOrder.length); } catch (error) { return error instanceof Error ? error.message : String(error); }
  const targets = action.targetHandSizes ?? normalDeffyTargets(action.turnOrder);
  if (action.subMode === "soda") {
    for (const id of action.turnOrder) if (targets[id] !== 6) return "Soda gives each player exactly six cards";
  }
...
function initializePool(state: EngineState, runtime: Phase14Runtime, poolCardIds: CardId[], faceDownCardIds: CardId[]): string | null {
  const shape = deffyPoolShape(runtime.subMode!, runtime.draftOrder.length);
  if (new Set(poolCardIds).size !== poolCardIds.length || new Set(faceDownCardIds).size !== faceDownCardIds.length || faceDownCardIds.some((id) => !poolCardIds.includes(id))) return "Pool and face-down IDs must be unique and consistent";
  if (shape.total !== "all" && poolCardIds.length !== shape.total) return `${runtime.subMode} requires ${shape.total} pool cards`;
  if (shape.total === "all" && poolCardIds.length !== state.zones.dp.length) return "Deffy Moment must display every legal DP card";
  if (faceDownCardIds.length !== shape.faceDown) return `${runtime.subMode} requires ${shape.faceDown} face-down cards`;
  if (poolCardIds.some((id) => state.cards[id]?.zone !== "DP")) return "Every pool card must begin in DP";
...
function mirrorIdentity(identity: string): string | null {
  if (identity === "RJ") return "BJ";
  if (identity === "BJ") return "RJ";
  const rank = identity.match(/^(10|[A2-9JQK])/)?.[1];
  return rank ?? null;
}

export function resolvePhase14Action(input: EngineState, actorId: PlayerId, action: Phase14Action): Phase14Resolution {
  if (!input.players[actorId]) return fail("PHASE14_PLAYER", `Unknown actor ${actorId}`);
  const state = canonicalClone(input);
  let runtime = phase14Runtime(state);
  const events: Phase14EventSpec[] = [];

  switch (action.kind) {
    case "configure-deffy": {
      const configured = configureRuntime(state, action);
      if (typeof configured === "string") return fail("DEFFY_CONFIG", configured);
      runtime = configured;
      state.phase = "Setup";
    ...
    case "initialize-draft-pool": {
      if (!runtime.enabled || runtime.status !== "drafting") return fail("DEFFY_NOT_CONFIGURED", "Configure Deffy Mode before creating a pool");
      if (state.zones.staging.length > 0) return fail("DEFFY_POOL_EXISTS", "Draft pool already exists");
      const problem = initializePool(state, runtime, action.poolCardIds, action.faceDownCardIds);
      if (problem) return fail("DEFFY_POOL", problem);
    ...
    case "draft-pick": {
      if (!runtime.enabled) return fail("DEFFY_NOT_CONFIGURED", "Deffy Mode is not configured");
      const problem = pickCard(state, runtime, action.drafterId, action.cardId, events);
      if (problem) return fail("DEFFY_PICK", problem);
      runtime.lastResolution = { kind: action.kind, drafterId: action.drafterId, cardId: action.cardId };
      saveRuntime(state, runtime);
      return { ok: true, state, events };
    }
    case "speed-run-timeout": {
      if (!runtime.enabled || !runtime.addOns.speedRun || runtime.status !== "drafting") return fail("DEFFY_SPEED_RUN", "Speed Run is not active");
      const legal = state.zones.staging.filter((id) => runtime.poolFaceUpByCard[id] !== false);
      if (legal.length === 0) return fail("DEFFY_SPEED_RUN", "No legal face-up pool card exists");
      const cursorBefore = state.rng.cursor;
      ...
      if (problem) return fail("DEFFY_SPEED_RUN", problem);
    ...
    case "refill-pool": {
      if (!runtime.enabled || runtime.status !== "drafting" || state.zones.staging.length !== 0) return fail("DEFFY_REFILL", "Pool refill is legal only when the active pool is empty");
    ...
    case "complete-draft": {
      if (!runtime.enabled || runtime.status !== "drafting") return fail("DEFFY_COMPLETE", "No active draft exists");
      const targetProblem = validateTargetSizes(state, runtime);
      if (targetProblem) return fail("DEFFY_TARGETS", targetProblem);
      const leftovers = [...state.zones.staging];
      const disposition = action.leftoverDisposition ?? "shuffle";
      if (disposition === "scrap" && action.unanimous !== true) return fail("DEFFY_LEFTOVERS", "Scrapping leftovers requires unanimous agreement");
      if (disposition === "scrap") {
        for (const id of leftovers) moveCard(state, id, "GY");
      } else {
        ...
        for (const id of shuffled) moveCard(state, id, "DP");
      }
    ...
    case "mirror-me-pick": {
      if (!runtime.enabled || !runtime.addOns.mirrorMe || runtime.status !== "drafting") return fail("DEFFY_MIRROR", "Mirror Me is not active");
      if (!isPoolCard(state, action.cardId) || runtime.poolFaceUpByCard[action.cardId] !== false) return fail("DEFFY_MIRROR", "Mirror Me requires a face-down pool card");
      ...
      if (!mirror || mirror.zone !== "DP" || (identity === "RJ" || identity === "BJ" ? mirror.identity !== expectedMirror : !mirror.identity.startsWith(expectedMirror ?? "\u0000"))) return fail("DEFFY_MIRROR", "Chosen mirror is not a legal remaining DP mirror");
      ...
      const problem = pickCard(state, runtime, action.drafterId, action.cardId, events);
      if (problem) return fail("DEFFY_MIRROR", problem);
      ...
      for (const id of remaining) moveCard(state, id, "VOID");
    ...
    case "validate-assignment": {
      ...
      if (problem) return fail("DEFFY_ASSIGNMENT", problem);
