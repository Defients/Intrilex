...
export function enumerateAdvancedCoreCandidates(state:Readonly<EngineState>,actorId:PlayerId):AdvancedCoreCandidate[]{
  ...
  if(isUnrestricted(s)){
    ...
    for(const pair of combos(byRank("7"),2))out.push({family:"super",mode:"seven-topdeck",timingClass:"ACTION",sourceCardIds:[...pair],targetCardIds:[],advanced:{kind:"advanced-super-seven-topdeck",sourceCardIds:pair as [CardId,CardId],handCardIds:[],effectCardIds:[]},featureVector:{topdeck:true,reveal:4}});
