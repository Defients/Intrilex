...
function doThreeBounce(state: EngineState, actorId: PlayerId, sourceCardId: CardId, targetCardId: CardId): RankResolution | null {
...
function doFourRowClear(state: EngineState, actorId: PlayerId, sourceCardId: CardId, targetPlayerId: PlayerId, row: "pr" | "er"): RankResolution | null {
...
function doTotalClear(state: EngineState, sourceCardId: CardId): void {
  const targets = Object.values(state.players).flatMap((p) => [...p.pr, ...p.er]);
  moveCard(state, sourceCardId, "GY");
  for (const id of targets) moveCard(state, id, "GY");
}

function doRecycleFive(state: EngineState, actorId: PlayerId, sourceCardId: CardId, rummageCardId?: CardId): RankResolution | null {
...
function doDeepDrawSixSpade(state: EngineState, actorId: PlayerId, sourceCardId: CardId, discardCardIds: CardId[], keepCardIds: CardId[]): RankResolution | null {
...
function doTopdeckSeven(state: EngineState, actorId: PlayerId, sourceCardId: CardId, handCardId?: CardId, effectCardId?: CardId): RankResolution | null {
  const revealed = state.zones.dp.splice(0, Math.min(2, state.zones.dp.length));
  for (const id of revealed) state.cards[id]!.zone = "VOID";
  const chosen = [handCardId, effectCardId].filter((id): id is string => id !== undefined);
  ...
  switch (action.kind) {
    ...
    case "solo-wild-copy": {
      ...
      const sub = (() => {
        switch (copied.kind) {
          ...
          case "topdeck-seven": return doTopdeckSeven(state, actorId, action.sourceCardId, copied.handCardId, copied.effectCardId);
        }
      })();
    ...
    case "row-exchange": {
      ...
      for (const id of ownCards) {
        const value = state.cards[id]!; value.zone = `${action.targetPlayerId}_${suffix}`; value.controllerId = action.targetPlayerId;
    ...
    case "topdeck-seven": {
      ...
      const revealed = state.zones.dp.splice(0, Math.min(2, state.zones.dp.length));
      for (const id of revealed) state.cards[id]!.zone = "VOID";
      const chosen = [action.handCardId, action.effectCardId].filter((id): id is string => id !== undefined);
      if (new Set(chosen).size !== chosen.length || chosen.some((id) => !revealed.includes(id))) return fail("RANK_CHOICE", "Seven choices must be distinct revealed cards");
      for (const id of revealed) {
    ...
    case "mimic-ten-diamond": {
      ...
      switch (mimic.kind) {
        case "row-exchange": {
          ...
          const own = state.players[actorId]!;
          const other = state.players[mimic.targetPlayerId]!;
          const ownCards = [...own[mimic.row]];
          const otherCards = [...other[mimic.row]];
          own[mimic.row] = otherCards;
          other[mimic.row] = ownCards;
          const suffix = mimic.row === "pr" ? "PR" : "ER";
          for (const id of otherCards) {
            const value = state.cards[id]!; value.zone = `${actorId}_${suffix}`; value.controllerId = actorId;
            applyAegis(value, "10♦-mimic-⭐4", futureStart(state, actorId));
          }
          for (const id of ownCards) {
            const value = state.cards[id]!; value.zone = `${mimic.targetPlayerId}_${suffix}`; value.controllerId = mimic.targetPlayerId;
            applyAegis(value, "10♦-mimic-⭐4", futureStart(state, mimic.targetPlayerId));
          }
          events.push({ type: "MIMIC_ROW_EXCHANGE_RESOLVED", payload: { row: mimic.row, actorId, targetPlayerId: mimic.targetPlayerId, actorReceived: otherCards, opponentReceived: ownCards } });
          break;
        }
        ...
        case "topdeck-seven": {
          const revealed = state.zones.dp.splice(0, Math.min(2, state.zones.dp.length));
          for (const id of revealed) state.cards[id]!.zone = "VOID";
          const chosen = [mimic.handCardId, mimic.effectCardId].filter((id): id is string => id !== undefined);
          ...
          events.push({ type: "MIMIC_TOPDECK_SEVEN_RESOLVED", payload: { sourceCardId: action.sourceCardId, revealed, handCardId: mimic.handCardId ?? null, effectCardId: mimic.effectCardId ?? null } });
          break;
        }
        case "recycle-five": {
          const milled = state.zones.dp.splice(0, Math.min(2, state.zones.dp.length));
