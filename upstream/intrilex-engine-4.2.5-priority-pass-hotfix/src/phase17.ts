...
function markerSubset(state: Record<string, unknown>): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const key of PUBLIC_MARKER_KEYS) if (Object.prototype.hasOwnProperty.call(state, key)) out[key] = state[key];
  return out;
}

export function deriveJudgeMarkerChecklist(state: EngineState): JudgeMarkerEntry[] {
...
export function buildJudgePacket(state: EngineState, includePrivateFor: PlayerId[] = []): JudgePacket {
  ...
  if (includePrivateFor.length > 0) packet.privateFactsByViewer = Object.fromEntries(includePrivateFor.map((id) => [id, privateStateView(state, id)]));
  return packet;
}

export function classifyJudgeOutcome(command: EngineCommand, result: CommandResult): JudgeOutcomeClass {
  if (!result.accepted) return "illegal-declaration";
  const types = result.events.map((event) => event.type.toUpperCase());
  if (types.some((type) => type.includes("FIZZLE"))) return "fizzled";
  if (types.some((type) => type.includes("COUNTER") && !type.includes("COUNTER_DECLARED"))) return "countered";
  if (command.type === "NOOP" || result.events.length === 0) return "no-op";
  return "resolved";
}

export function explainIllegalVsFizzle(command: EngineCommand, result: CommandResult): { classification: JudgeOutcomeClass; explanation: string; rollbackRequired: boolean; sourceDisposition: string } {
  const classification = classifyJudgeOutcome(command, result);
  if (classification === "illegal-declaration") return { classification, explanation: result.error?.message ?? "The declaration failed public legality and never existed.", rollbackRequired: true, sourceDisposition: "Return every staged source/cost to the exact before-image; spend no Mini-Turn or limit." };
  if (classification === "fizzled") return { classification, explanation: "The declaration was legal, but a required target or condition failed revalidation at resolution.", rollbackRequired: false, sourceDisposition: "Committed sources follow failed-play destinations and destination replacements; paid limits remain spent." };
  if (classification === "countered") return { classification, explanation: "A legal pending object was negated before resolution.", rollbackRequired: false, sourceDisposition: "Committed sources follow counter destinations and replacements; costs and limits are not refunded." };
...
export function renderPrintableStateAid(state: EngineState): string {
  const lines: string[] = ["# Intrilex Judge State Aid", "", `- Revision: ${state.revision}`, `- Phase: ${state.phase}`, `- Active player: ${state.activePlayerId}`, `- Full Turn: ${state.fullTurnSequence}`, "", "## Scores and Goals", ""];
  for (const playerId of state.turnOrder) lines.push(`- ${playerId}: ${deriveSecuredPoints(state, playerId)} / Goal ${state.players[playerId]?.goal ?? "?"}`);
  ...
  if (markers.length === 0) lines.push("- None");
  for (const marker of markers) lines.push(`- ${marker.cardId} · ${marker.identity} · ${marker.zone} · ${canonicalize(marker.markers)}`);
...
export function judgeEventDigest(events: EngineEvent[]): string {
