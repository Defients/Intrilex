...
const immutableWork = await mkdtemp(path.join(tmpdir(), 'intrilex-421-immutable-'));
const executionWork = await mkdtemp(path.join(tmpdir(), 'intrilex-421-execution-'));
...
try {
  ...
  const stages = [];
  stages.push(run(immutableWork, 'node', ['scripts/core-effect-integrity.mjs', 'verify']));
  stages.push(run(executionWork, 'npm', ['ci', '--offline', '--ignore-scripts']));
  stages.push(run(executionWork, 'node', ['scripts/core-effect-integrity.mjs', 'verify']));
  ...
  stages.push(run(executionWork, 'node', ['--input-type=module', '-e', `
    ...
    const c=JSON.parse(fs.readFileSync('./reports/CORE_EFFECT_AUTHORITY_CERTIFICATION.json','utf8'));
    const s=JSON.parse(fs.readFileSync('./reports/core-effect-authority-stress-500.json','utf8'));
    if(c.verdict!=='PASS'||c.engineTests.passed!==160||c.conformance.passed!==121) throw new Error('cert mismatch');
    if(s.status!=='PASS'||s.matchCount!==500||s.terminations.NORMAL_VICTORY!==500||s.resultHash!=='a53bb2cf45a1e7397d7cd90448a6cd0a8f8f9e0d47229f9338defd2e50e2f44e') throw new Error('stress mismatch');
    const required=['ace','three','four','attachment','anchor','redJoker','boardLock'];
    for(const key of required) if(!(s.actions[key]>0)) throw new Error('unexercised effect:'+key);
    console.log(JSON.stringify({verdict:c.verdict,matches:s.matchCount,resultHash:s.resultHash,actions:s.actions}));
  `]));
  stages.push(run(executionWork, 'node', ['--input-type=module', '-e', `
    import {runCoreRandomLegalMatch,hashCanonical} from './dist/src/index.js';
    const rows=[];
    for(let i=0;i<25;i++){
      const seed=((0x42100000+Math.imul(i,0x9e3779b1))>>>0)||1;
      const r=runCoreRandomLegalMatch({profileId:'core-effect-declaration-authority',playerIds:['P1','P2'],enabledModules:[],eventApprovedModules:[],seed,seatOrder:['P1','P2']},1000);
      if(r.terminationReason!=='NORMAL_VICTORY') throw new Error(seed+':'+r.terminationReason);
      rows.push({seed,winner:r.state.winner,stateHash:hashCanonical(r.state),decisionHash:hashCanonical(r.decisions),effectCount:r.commands.filter(c=>c.action?.kind==='core-resolve-effect').length});
    }
    if(!rows.some(r=>r.effectCount>0)) throw new Error('operator cohort exercised no effects');
    console.log(JSON.stringify({matches:25,aggregateHash:hashCanonical(rows),effects:rows.reduce((n,r)=>n+r.effectCount,0)}));
  `]));
  stages.push(run(immutableWork, 'node', ['scripts/core-effect-integrity.mjs', 'verify']));
