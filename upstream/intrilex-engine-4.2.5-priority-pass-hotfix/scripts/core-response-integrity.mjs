...
const manifestPath = path.join(root, 'CORE_RESPONSE_AUTHORITY_MANIFEST.json');
const sumsPath = path.join(root, 'CORE_RESPONSE_AUTHORITY_SHA256SUMS');
const excludedTop = new Set(['node_modules', 'release', '.git']);
const excludedExact = new Set(['CORE_RESPONSE_AUTHORITY_MANIFEST.json', 'CORE_RESPONSE_AUTHORITY_SHA256SUMS']);
...
if(mode==='generate'){
 const manifest={schemaVersion:'1.0',artifact:'Intrilex Engine v4.2.2 — Core Response Authority',package:'@intrilex/headless-engine',version:'4.2.2',rulesVersion:'4.1',baseCertifiedVersion:'4.1.0',basePayloadHash:'707377ea9fa94f449c293b6a4dcd8dc4b40dd058bcd6f9e5dd874339859cf168',preservedConformanceAggregate:'05f67133eab3f8e92d526db17b25407842204e0eeea3a605bd417d0479805547',fileCount:actual.length,payloadHash:payloadHash(actual),files:actual,generatedAt:'2020-01-01T00:00:00.000Z'};
 await writeFile(manifestPath,JSON.stringify(manifest,null,2)+'\n'); await writeFile(sumsPath,actual.map(r=>`${r.sha256}  ${r.path}`).join('\n')+'\n'); console.log(`CORE RESPONSE MANIFEST GENERATED: files=${actual.length}; payload=${manifest.payloadHash}`);
}else{
 const manifest=JSON.parse(await readFile(manifestPath,'utf8'));if(manifest.version!=='4.2.2')throw new Error(`Manifest version mismatch: ${manifest.version}`);if(manifest.fileCount!==actual.length)throw new Error(`Manifest file count mismatch: expected ${manifest.fileCount}, actual ${actual.length}`);const hash=payloadHash(actual);if(manifest.payloadHash!==hash)throw new Error(`Manifest payload mismatch: expected ${manifest.payloadHash}, actual ${hash}`);if(JSON.stringify(manifest.files)!==JSON.stringify(actual))throw new Error('Manifest file records differ');const expected=actual.map(r=>`${r.sha256}  ${r.path}`).join('\n')+'\n';if(await readFile(sumsPath,'utf8')!==expected)throw new Error('SHA sums mismatch');console.log(`CORE RESPONSE MANIFEST PASS: files=${actual.length}; payload=${hash}`);
}
