...
export function isCorePrivateChoiceProfile(state) {
    return state.metadata.coreAuthority?.profileId === CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id || state.metadata.coreAuthority?.profileId === "core-advanced-authority";
}
    ...
    if (submission.kind === "core-rank7-assign") {
        ...
        if (revealed.length === 1) {
            ...
            if (submission.mode === "hand-only")
                ...
                return fail("CORE_PRIVATE_CHOICE_SELECTION", "Single-card Seven cannot use hand-and-effect");
        }
        else {
            if (submission.mode !== "hand-and-effect" || selected.length !== 2 || new Set(selected).size !== 2 || selected.some((id) => !revealed.includes(id)))
    ...
    if (submission.kind === "core-rank7-generated-effect") {
        ...
        if (!generatedEffect) {
            if (legalEffects.length > 0)
                return fail("CORE_PRIVATE_CHOICE_GENERATED", "A legal generated effect must be selected");
