...
function doThreeBounce(state, actorId, sourceCardId, targetCardId) {
...
function doFourRowClear(state, actorId, sourceCardId, targetPlayerId, row) {
...
function doTotalClear(state, sourceCardId) {
    const targets = Object.values(state.players).flatMap((p) => [...p.pr, ...p.er]);
    moveCard(state, sourceCardId, "GY");
    for (const id of targets)
        moveCard(state, id, "GY");
}
function doRecycleFive(state, actorId, sourceCardId, rummageCardId) {
...
function doDeepDrawSixSpade(state, actorId, sourceCardId, discardCardIds, keepCardIds) {
...
function doTopdeckSeven(state, actorId, sourceCardId, handCardId, effectCardId) {
    const revealed = state.zones.dp.splice(0, Math.min(2, state.zones.dp.length));
    for (const id of revealed)
        state.cards[id].zone = "VOID";
    const chosen = [handCardId, effectCardId].filter((id) => id !== undefined);
        ...
        case "mimic-ten-diamond": {
            ...
            switch (mimic.kind) {
                case "row-exchange": {
                    ...
                    for (const id of ownCards) {
                        const value = state.cards[id];
                        value.zone = `${mimic.targetPlayerId}_${suffix}`;
                        value.controllerId = mimic.targetPlayerId;
                ...
                case "topdeck-seven": {
                    ...
                    for (const id of revealed)
                        state.cards[id].zone = "VOID";
                    const chosen = [mimic.handCardId, mimic.effectCardId].filter((id) => id !== undefined);
                    if (new Set(chosen).size !== chosen.length || chosen.some((id) => !revealed.includes(id)))
