...
import { compareScuttle, hasOrdinaryScuttleImmunity, rankDefinition } from "./ranks.js";
...
const POLICIES = ["score-first", "balanced", "control-first"];
function deck54() {
...
function card(identity, zone) {
    return { id: identity, identity, originalOwnerId: "SIM", controllerId: "SIM", zone, state: { pointValue: rankDefinition(identity).prPoints } };
}
function points(pr) { return pr.reduce((sum, identity) => sum + rankDefinition(identity).prPoints, 0); }
function scuttleLegal(source, target) {
...
function chooseAction(policy, hand, ownPr, enemyPr, deckLength) {
    ...
    for (const source of hand)
        for (const target of enemyPr)
            if (scuttleLegal(source, target))
                scuttles.push({ source, target, swing: rankDefinition(target).prPoints - rankDefinition(source).prPoints });
    ...
    if (bestScore && own + rankDefinition(bestScore).prPoints >= 15)
        return { kind: "score", source: bestScore };
    if (policy === "control-first" && bestScuttle && bestScuttle.swing >= -1)
        return { kind: "scuttle", source: bestScuttle.source, target: bestScuttle.target };
    if (policy === "balanced" && bestScuttle && bestScuttle.swing >= 3 && points(enemyPr) >= own)
        return { kind: "scuttle", source: bestScuttle.source, target: bestScuttle.target };
    if (bestScore)
        return { kind: "score", source: bestScore };
    if (deckLength > 0)
...
function simulateMatch(seed, policyP1, policyP2, startingSeat) {
    const rng = { algorithm: "xorshift32", seed: seed >>> 0 || 1, cursor: 0 };
    const dp = shuffle(deck54(), rng);
    const hands = { P1: dp.splice(0, 5), P2: dp.splice(0, 6) };
    ...
    for (let turn = 1; turn <= 200; turn += 1) {
        ...
        if (choice.kind === "draw") {
            ...
            hands[active].push(...dp.splice(0, count));
            actions.draw += 1;
        }
        else if (choice.kind === "score" && choice.source) {
            hands[active].splice(hands[active].indexOf(choice.source), 1);
            pr[active].push(choice.source);
            actions.score += 1;
        }
        else if (choice.kind === "scuttle" && choice.source && choice.target) {
            hands[active].splice(hands[active].indexOf(choice.source), 1);
            pr[enemy].splice(pr[enemy].indexOf(choice.target), 1);
            actions.scuttle += 1;
        }
        else
            actions.pass += 1;
        if (points(pr[active]) >= 15)
            return { seed, policyP1, policyP2, startingSeat, winner: active, turns: turn, actions, finalPoints: { P1: points(pr.P1), P2: points(pr.P2) }, deckRemaining: dp.length, reason: "goal" };
        if (dp.length === 0 && hands.P1.length === 0 && hands.P2.length === 0) {
            const p1 = points(pr.P1), p2 = points(pr.P2);
...
function runCampaign(matchesPerPair = 1200) {
    const matches = [];
    let serial = 1;
    for (const p1 of POLICIES)
        for (const p2 of POLICIES)
            for (let i = 0; i < matchesPerPair; i += 1) {
                const starter = i % 2 === 0 ? "P1" : "P2";
                matches.push(simulateMatch((0x9e3779b9 ^ (serial * 0x45d9f3b)) >>> 0, p1, p2, starter));
                serial += 1;
            }
...
export async function runSimulationBaseline(projectRoot) {
    const scenarios = [];
    ...
    const campaignA = runCampaign();
    const campaignB = runCampaign();
    ...
    const core = { reportVersion: 2, rulesVersion: "4.1", engineVersion: "4.1.0", scenarioBaseline, fullMatchCampaign: campaignA, deterministicReproduction: { campaignHashA: hashA, campaignHashB: hashB, matched: hashA === hashB } };
    const report = { ...core, catalogHash: hashCanonical(core) };
