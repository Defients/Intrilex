...
test("4.2.1 every enumerated Core effect declaration is engine-accepted and command-vault isolated", async () => {
  ...
  for (const required of ["effect-ace","effect-three","effect-four","attachment","anchor","effect-red-joker","effect-board-lock"]) assert.equal(families.has(required), true, required);
...
test("4.2.1 Board Lock removes effects and Scuttle while preserving allowed Core actions and expires canonically", async () => {
  ...
  decision=advanceCoreToDecision(state); const lock=decision.legalActionFrame!.actions.find((entry)=>entry.family==="effect-board-lock")!;
...
test("4.2.5 Advanced profile declares a bounded public authority surface and fail-closed exclusions", async () => {
  ...
  assert.equal(CORE_ADVANCED_AUTHORITY_PROFILE.engineVersion,"4.2.5");
});
