...
import type { CardId, CoreAuthorityAction, CoreAuthorityProfileId, CoreEffectAction, CorePrimaryAction, CorePrivateChoiceState, CorePrivateChoiceSubmission, EngineCommand, EngineEvent, EngineState, PlayerId, RankAction, SoloWildCopiedAction } from "./types.js";

interface CoreRuntimeView {
  profileId?: string;
...
function readCoreRuntime(state: Readonly<EngineState>): CoreRuntimeView | null {
  const value = state.metadata.coreAuthority;
  return value && typeof value === "object" ? value as CoreRuntimeView : null;
}
function commandId(state: EngineState, actorId: PlayerId, suffix: string): string { return `CORE-${state.revision}-${state.fullTurnSequence}-${actorId}-${suffix}`; }
function command(state: EngineState, actorId: PlayerId, suffix: string, action: CoreAuthorityAction): Extract<EngineCommand, { type: "RESOLVE_CORE_AUTHORITY_ACTION" }> {
  return { id: commandId(state, actorId, suffix), type: "RESOLVE_CORE_AUTHORITY_ACTION", actorId, action };
}
function isPrimaryAction(value: CoreAuthorityAction): value is CorePrimaryAction {
...
function action(state: EngineState, actorId: PlayerId, family: string, mode: string, timingClass: CoreLegalAction["timingClass"], sourceCardIds: CardId[], targetCardIds: CardId[], coreAction: CoreAuthorityAction, featureVector: CoreLegalAction["featureVector"] = {}): CoreLegalAction {
  ...
  const submittedAction: CoreAuthorityAction = [CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(runtime?.profileId as any) && isPrimaryAction(coreAction) && !directVoltage
...
function isPhysicalSeven(state: EngineState, cardId: CardId): boolean {
...
export function canRecurseTopdeck(state: EngineState, sourceCardId: CardId): boolean {
  return isPhysicalSeven(state, sourceCardId);
}

function privateChoiceAction(state: EngineState, choice: CorePrivateChoiceState, mode: string, selectedCardIds: CardId[], submission: CorePrivateChoiceSubmission, featureVector: CoreLegalAction["featureVector"] = {}): CoreLegalAction {
...
export function enumerateCorePrivateChoiceActions(state: Readonly<EngineState>, actorId: PlayerId): CoreDecisionFrame {
  const engine = new IntrilexEngine();
  const choice = activeCorePrivateChoice(state);
  if (!choice || choice.chooserId !== actorId || ![CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes((state.metadata.coreAuthority as CoreRuntimeView | undefined)?.profileId as any)) return { stateRevision: state.revision, actorId, actions: [], frameHash: hashCanonical([]) };
  const candidates: CoreLegalAction[] = [];
  if (choice.kind === "core-rank3-present") for (const selected of combinations(choice.optionCardIds, choice.minSelections, choice.maxSelections)) candidates.push(privateChoiceAction(state as EngineState, choice, "rank3-present", selected, { kind: "core-rank3-present", selectedCardIds: selected }, { selectedCount: selected.length }));
  if (choice.kind === "core-rank3-take") for (const selected of combinations(choice.optionCardIds, choice.minSelections, choice.maxSelections)) candidates.push(privateChoiceAction(state as EngineState, choice, "rank3-take", selected, { kind: "core-rank3-take", selectedCardIds: selected }, { takeCount: selected.length }));
  if (choice.kind === "core-rank3-discard") for (const selected of combinations(choice.optionCardIds, choice.minSelections, choice.maxSelections)) candidates.push(privateChoiceAction(state as EngineState, choice, "rank3-discard", selected, { kind: "core-rank3-discard", selectedCardIds: selected }, { discardCount: selected.length }));
  if (choice.kind === "core-rank5-rummage") for (const id of choice.optionCardIds) candidates.push(privateChoiceAction(state as EngineState, choice, "rank5-rummage", [id], { kind: "core-rank5-rummage", selectedCardIds: [id] }, { rummage: true }));
  if (choice.kind === "core-rank6-dig") {
    const drawn = [...((choice.context.drawnCardIds as CardId[] | undefined) ?? [])];
    const discardOptions = [...((choice.context.discardOptionCardIds as CardId[] | undefined) ?? [])];
    const keepCount = Math.min(2, drawn.length);
    for (const kept of combinations(drawn, keepCount, keepCount)) {
      candidates.push(privateChoiceAction(state as EngineState, choice, "rank6-keep-return-top", kept, { kind: "core-rank6-dig", mode: "keep-return-top", selectedCardIds: kept }, { keepCount, returnTop: true }));
      candidates.push(privateChoiceAction(state as EngineState, choice, "rank6-keep-return-bottom", kept, { kind: "core-rank6-dig", mode: "keep-return-bottom", selectedCardIds: kept }, { keepCount, returnBottom: true }));
    }
    for (const discardId of discardOptions) candidates.push(privateChoiceAction(state as EngineState, choice, "rank6-keep-all-discard", [discardId], { kind: "core-rank6-dig", mode: "keep-all-discard", selectedCardIds: [discardId] }, { keepAll: true, discard: true }));
  }
  if (choice.kind === "core-rank7-assign") {
    if (choice.optionCardIds.length === 1) {
      const id = choice.optionCardIds[0]!;
      candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-hand-only", [id], { kind: "core-rank7-assign", mode: "hand-only", selectedCardIds: [id] }, { toHand: true }));
      candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-effect-only", [id], { kind: "core-rank7-assign", mode: "effect-only", selectedCardIds: [id] }, { toEffect: true }));
      candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-score-only", [id], { kind: "core-rank7-assign", mode: "score-only", selectedCardIds: [id] }, { toScore: true }));
    } else {
      for (const handId of choice.optionCardIds) for (const effectId of choice.optionCardIds) if (handId !== effectId) candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-hand-and-effect", [handId, effectId], { kind: "core-rank7-assign", mode: "hand-and-effect", selectedCardIds: [handId, effectId] }, { toHand: true, toEffect: true }));
      for (const handId of choice.optionCardIds) for (const scoreId of choice.optionCardIds) if (handId !== scoreId) candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-hand-and-score", [handId, scoreId], { kind: "core-rank7-assign", mode: "hand-and-score", selectedCardIds: [handId, scoreId] }, { toHand: true, toScore: true }));
    }
  }
  if (choice.kind === "core-rank7-generated-effect") {
    ...
    candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-generated-score", [generatedCardId], { kind: "core-rank7-generated-effect", selectedCardIds: [generatedCardId], scoreInstead: true }, { toScore: true }));
    if (effects.length === 0) candidates.push(privateChoiceAction(state as EngineState, choice, "rank7-generated-unavailable", [generatedCardId], { kind: "core-rank7-generated-effect", selectedCardIds: [generatedCardId] }, { fizzle: true }));
    for (const effect of effects) candidates.push(privateChoiceAction(state as EngineState, choice, `rank7-generated-${effect.kind}`, [generatedCardId], { kind: "core-rank7-generated-effect", selectedCardIds: [generatedCardId], generatedEffect: canonicalClone(effect) }, { generated: true, effectKind: effect.kind }));
  }
  if (choice.kind === "core-nine-anchor-discard") for (const id of choice.optionCardIds) candidates.push(privateChoiceAction(state as EngineState, choice, "nine-anchor-discard", [id], { kind: "core-nine-anchor-discard", selectedCardIds: [id] }, { discard: true }));
  const accepted = candidates.filter((entry) => engine.execute(state as EngineState, entry.command).accepted).sort((a, b) => a.actionId.localeCompare(b.actionId));
...
function enumerateSoloWildCopyCandidates(state: EngineState, actorId: PlayerId): CoreLegalAction[] {
  const player = state.players[actorId];
  if (!player) return [];
  const opponents = state.turnOrder.filter((id) => id !== actorId);
  const out: CoreLegalAction[] = [];
  for (const sourceCardId of player.hand) {
    const parsed = parseIdentity(state.cards[sourceCardId]!.identity);
    if (!parsed || parsed.rank !== "2" || !parsed.suit) continue;
    const twoSuit = parsed.suit;
    // Rank 3: three-bounce (bounce one OTT card to top of DP)
    for (const p of Object.values(state.players)) for (const t of [...p.pr, ...p.er]) {
      const copiedAction: SoloWildCopiedAction = { kind: "three-bounce", targetCardId: t };
      const rankAction: RankAction = { kind: "solo-wild-copy", sourceCardId, targetRank: "3", copiedAction };
    ...
    for (const oid of opponents) for (const row of ["pr", "er"] as const) {
      const copiedAction: SoloWildCopiedAction = { kind: "four-row-clear", targetPlayerId: oid, row };
      const rankAction: RankAction = { kind: "solo-wild-copy", sourceCardId, targetRank: "4", copiedAction };
    ...
    if (twoSuit === "♠") {
      const copiedAction: SoloWildCopiedAction = { kind: "total-clear" };
      const rankAction: RankAction = { kind: "solo-wild-copy", sourceCardId, targetRank: "4", copiedAction };
    ...
    {
      const copiedAction: SoloWildCopiedAction = { kind: "recycle-five" };
      const rankAction: RankAction = { kind: "solo-wild-copy", sourceCardId, targetRank: "5", copiedAction };
    ...
    if (twoSuit === "♠") {
      ...
      for (const d1 of otherHand) {
        const copiedAction: SoloWildCopiedAction = { kind: "deep-draw-six-spade", discardCardIds: [d1], keepCardIds: [] };
        const rankAction: RankAction = { kind: "solo-wild-copy", sourceCardId, targetRank: "6", copiedAction };
    ...
    {
      const copiedAction: SoloWildCopiedAction = { kind: "topdeck-seven" };
      const rankAction: RankAction = { kind: "solo-wild-copy", sourceCardId, targetRank: "7", copiedAction };
...
function enumerateWildSovereigntyCandidates(state: EngineState, actorId: PlayerId): CoreLegalAction[] {
  const player = state.players[actorId];
  if (!player) return [];
  const opponents = state.turnOrder.filter((id) => id !== actorId);
  const out: CoreLegalAction[] = [];
  for (const sourceCardId of player.hand) {
    const parsed = parseIdentity(state.cards[sourceCardId]!.identity);
    if (!parsed || parsed.rank !== "K" || parsed.suit !== "♠") continue;
    // Rank 3: three-bounce (bounce one OTT card to top of DP)
    for (const p of Object.values(state.players)) for (const t of [...p.pr, ...p.er]) {
      const copiedAction: SoloWildCopiedAction = { kind: "three-bounce", targetCardId: t };
      const rankAction: RankAction = { kind: "wild-sovereignty", sourceCardId, targetRank: "3", copiedAction };
    ...
    for (const oid of opponents) for (const row of ["pr", "er"] as const) {
      const copiedAction: SoloWildCopiedAction = { kind: "four-row-clear", targetPlayerId: oid, row };
      const rankAction: RankAction = { kind: "wild-sovereignty", sourceCardId, targetRank: "4", copiedAction };
    ...
    for (const costCardId of [...player.hand].filter((id) => id !== sourceCardId)) {
      const copiedAction: SoloWildCopiedAction = { kind: "total-clear" };
      const rankAction: RankAction = { kind: "wild-sovereignty", sourceCardId, targetRank: "4", copiedAction, discardCostCardId: costCardId };
    ...
    {
      const copiedAction: SoloWildCopiedAction = { kind: "recycle-five" };
      const rankAction: RankAction = { kind: "wild-sovereignty", sourceCardId, targetRank: "5", copiedAction };
    ...
    // Rank 6♠: deep-draw-six-spade (requires other hand cards to discard)
    {
      ...
      for (const d1 of otherHand) {
        const copiedAction: SoloWildCopiedAction = { kind: "deep-draw-six-spade", discardCardIds: [d1], keepCardIds: [] };
        const rankAction: RankAction = { kind: "wild-sovereignty", sourceCardId, targetRank: "6", copiedAction };
    ...
    {
      const copiedAction: SoloWildCopiedAction = { kind: "topdeck-seven" };
      const rankAction: RankAction = { kind: "wild-sovereignty", sourceCardId, targetRank: "7", copiedAction };
...
export function enumerateCoreLegalActions(state: Readonly<EngineState>, actorId: PlayerId): CoreDecisionFrame {
  const engine = new IntrilexEngine();
  const candidates: CoreLegalAction[] = [];
  const core = readCoreRuntime(state);
  if (!core || ![CORE_FOUNDATION_AUTHORITY_PROFILE.id, CORE_EFFECT_DECLARATION_PROFILE.id, CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any) || state.activePlayerId !== actorId || state.winner !== null || activeCorePrivateChoice(state) !== null) return { stateRevision: state.revision, actorId, actions: [], frameHash: hashCanonical([]) };
  if (state.phase === "Start" && core.startPreparedFullTurnSequence === state.fullTurnSequence) {
    ...
    if (!player.limits.swapBarUsedThisFT) {
      ...
      for (const handId of player.hand) for (const barId of facedown) candidates.push(action(state as EngineState, actorId, "swap-bar", "face-down", "SETUP", [handId], [barId], { kind: "core-face-down-swap", handCardId: handId, swapCardId: barId }));
    }
    ...
    if (!boardLockActive) {
      ...
      for (const effect of enumerateCoreEffectCandidates(state, actorId)) candidates.push(action(state as EngineState, actorId, effect.family, effect.mode, "ACTION", [...effect.sourceCardIds], [...effect.targetCardIds], { kind: "core-resolve-effect", effect: effect.effect }, effect.featureVector));
      if ([CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any)) for (const advanced of enumerateAdvancedCoreCandidates(state, actorId).filter((entry) => entry.timingClass === "ACTION" || (entry.timingClass === "INTERRUPT" && !("targetStackItemId" in (entry.advanced as any))))) candidates.push(action(state as EngineState, actorId, advanced.family, advanced.mode, advanced.timingClass, [...advanced.sourceCardIds], [...advanced.targetCardIds], { kind: "core-resolve-advanced", advanced: advanced.advanced }, advanced.featureVector));
      // Solo Wild Copy: a single 2 can copy a same-suit rank 3-7 Base effect
      for (const candidate of enumerateSoloWildCopyCandidates(state as EngineState, actorId)) candidates.push(candidate);
      // K♠ Wild Sovereignty: K♠ copies one Spade Base effect of rank 3-7 and is Exiled after.
      if ([CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any)) for (const candidate of enumerateWildSovereigntyCandidates(state as EngineState, actorId)) candidates.push(candidate);
    }
    if ([CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any)) {
      for (const sourceId of state.players[actorId]!.hand) {
        const rank = parseIdentity(state.cards[sourceId]!.identity)?.rank;
        if (rank === "8") candidates.push(action(state as EngineState, actorId, "quick", "eight-aegis-field", "QUICK", [sourceId], [], { kind: "core-declare-eight-aegis-field", sourceCardId: sourceId }, { quick: true, aegis: true }));
        if (rank === "Q") for (const targetId of [...state.players[actorId]!.pr, ...state.players[actorId]!.er]) candidates.push(action(state as EngineState, actorId, "quick", "queen-aegis", "QUICK", [sourceId], [targetId], { kind: "core-declare-queen-aegis-quick", sourceCardId: sourceId, targetCardId: targetId }, { quick: true, aegis: true }));
        // Black Joker Board Lock: Quick Effect, no Mini-Turn cost, open-state declaration only.
        if (rank === "BJ") candidates.push(action(state as EngineState, actorId, "quick", "board-lock", "QUICK", [sourceId], [], { kind: "core-declare-board-lock-quick", sourceCardId: sourceId }, { quick: true, boardLock: true, noMiniTurn: true }));
      }
    }
  ...
  let accepted = candidates.filter((candidate) => engine.execute(state as EngineState, candidate.command).accepted).sort((a, b) => a.actionId.localeCompare(b.actionId));
  if (state.phase === "Action" && state.players[actorId]!.limits.miniTurnsRemaining > 0 && core.exhausted && state.zones.dp.length === 0 && accepted.length === 0) {
    const exhaustedPass = action(state as EngineState, actorId, "exhausted-pass", "forced-mini-turn", "ACTION", [], [], { kind: "core-exhausted-pass" }, { progress: true, forced: true });
    if (engine.execute(state as EngineState, exhaustedPass.command).accepted) accepted.push(exhaustedPass);
  }
  if ([CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any) && state.phase === "Action") {
    const disrupted = new Set(core.disruptedActionTypesByPlayer?.[actorId] ?? []);
    const rootType = (entry: CoreLegalAction): string | null => entry.command.action.kind === "core-declare-primary" ? primaryDescriptor(entry.command.action.action).actionType : null;
    const availableRootTypes = new Set(accepted.map(rootType).filter((value): value is string => value !== null));
    accepted = accepted.filter((entry) => { const type = rootType(entry); if (!type || !disrupted.has(type)) return true; return ![...availableRootTypes].some((other) => other !== type); });
  }
...
export function enumerateCoreResponseActions(state: Readonly<EngineState>, actorId: PlayerId): CoreDecisionFrame {
  const engine = new IntrilexEngine();
  const runtime = readCoreRuntime(state);
  const priorityActor = currentPriorityActor(state);
  if (![CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(runtime?.profileId as any) || priorityActor !== actorId) return { stateRevision: state.revision, actorId, actions: [], frameHash: hashCanonical([]) };
  ...
  if (target) {
    for (const sourceId of [...state.players[actorId]!.hand].sort()) {
      ...
      if (parsed?.rank === "A") {
        candidates.push(action(state as EngineState, actorId, "counter", parsed.suit === "♠" ? "ace-spade" : "ace-base", "INSTANT", [sourceId], [], parsed.suit === "♠" ? { kind: "core-declare-spade-ace-counter", sourceCardId: sourceId, targetStackItemId: target.id } : { kind: "core-declare-base-ace-counter", sourceCardId: sourceId, targetStackItemId: target.id, sourceMode: "hand" }, { counter: true }));
      }
      if (parsed?.rank === "8") {
        candidates.push(action(state as EngineState, actorId, "counter", "eight-scuttle", "INSTANT", [sourceId], [], { kind: "core-declare-eight-scuttle-counter", sourceCardId: sourceId, targetStackItemId: target.id }, { counter: true, scuttle: true }));
        if (parsed.suit === "♠") for (const opponentId of state.turnOrder.filter((id) => id !== actorId)) for (const targetCardId of state.players[opponentId]!.pr) candidates.push(action(state as EngineState, actorId, "instant", "eight-spade-free-scuttle", "INSTANT", [sourceId], [targetCardId], { kind: "core-declare-eight-spade-free-scuttle", sourceCardId: sourceId, targetCardId }, { scuttle: true, free: true }));
        if (state.activePlayerId === actorId) candidates.push(action(state as EngineState, actorId, "quick", "eight-aegis-field", "QUICK", [sourceId], [], { kind: "core-declare-eight-aegis-field", sourceCardId: sourceId }, { quick: true, aegis: true }));
      }
      if (parsed?.rank === "9") for (const opponentId of state.turnOrder.filter((id) => id !== actorId)) for (const targetCardId of state.players[opponentId]!.pr) candidates.push(action(state as EngineState, actorId, "instant", "nine-tap", "INSTANT", [sourceId], [targetCardId], { kind: "core-declare-nine-tap", sourceCardId: sourceId, targetCardId }, { tap: true }));
      if (parsed?.rank === "J") candidates.push(action(state as EngineState, actorId, "disrupt", "jack", "INSTANT", [sourceId], [], { kind: "core-declare-jack-disrupt", sourceCardId: sourceId, targetStackItemId: target.id }, { disrupt: true }));
      if (parsed?.rank === "K") candidates.push(action(state as EngineState, actorId, "counter", "king-anchor", "INSTANT", [sourceId], [], { kind: "core-declare-king-counter", sourceCardId: sourceId, targetStackItemId: target.id }, { counter: true, specialized: true }));
      if ([CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(runtime?.profileId as any) && parsed?.rank === "K" && parsed.suit === "♠") candidates.push(action(state as EngineState, actorId, "counter", "king-spade", "INSTANT", [sourceId], [], { kind: "core-declare-king-spade-counter", sourceCardId: sourceId, targetStackItemId: target.id }, { counter: true, multiCard: true }));
      if (
        [CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(runtime?.profileId as any)
        && parsed?.rank === "10"
        && parsed.suit === "♠"
      ...
      ) candidates.push(action(state as EngineState, actorId, "interrupt", "rank10-stack-theft", "INTERRUPT", [sourceId], [], { kind: "core-declare-rank10-stack-theft", sourceCardId: sourceId, targetStackItemId: target.id }, { controlChange: true, printedFullTurnSkips: 2, genericInterruptTax: false }));
      if (parsed?.rank === "Q" && state.activePlayerId === actorId) for (const targetCardId of [...state.players[actorId]!.pr, ...state.players[actorId]!.er]) candidates.push(action(state as EngineState, actorId, "quick", "queen-aegis", "QUICK", [sourceId], [targetCardId], { kind: "core-declare-queen-aegis-quick", sourceCardId: sourceId, targetCardId }, { quick: true, aegis: true }));
    }
  ...
  const accepted = candidates.filter((entry) => engine.execute(state as EngineState, entry.command).accepted).sort((a,b) => a.actionId.localeCompare(b.actionId));
  if (accepted.length > 0) accepted.push(action(state as EngineState, actorId, "response-decline", "decline", "INSTANT", [], target ? [target.id] : [], { kind: "core-pass-priority", semantic: "DECLINE_RESPONSE" }, { progress: false, hadLawfulResponse: true }));
  accepted.sort((a,b) => a.actionId.localeCompare(b.actionId));
  return { stateRevision: state.revision, actorId, actions: accepted, frameHash: hashCanonical(accepted.map((entry) => ({ actionId: entry.actionId, commandHash: entry.commandHash }))) };
}

export function toAuthorizedCoreAction(value: CoreLegalAction): AuthorizedCoreAction {
...
export function createCoreMatchState(setup: CoreMatchSetup): EngineState {
  if (![CORE_FOUNDATION_AUTHORITY_PROFILE.id, CORE_EFFECT_DECLARATION_PROFILE.id, CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(setup.profileId as any)) throw new Error(`Unsupported Core profile ${setup.profileId}`);
  if (setup.enabledModules.length > 0) throw new Error("Core Foundation Authority does not permit optional modules");
  ...
  const cmd: Extract<EngineCommand, { type: "RESOLVE_CORE_AUTHORITY_ACTION" }> = { id: `CORE-SETUP-${setup.seed >>> 0}`, type: "RESOLVE_CORE_AUTHORITY_ACTION", actorId: setup.seatOrder[0], action: { kind: "core-apply-setup", playerIds: [...setup.seatOrder], profileId: setup.profileId as CoreAuthorityProfileId } };
  ...
  assertValidState(result.state); return result.state;
}
...
export function advanceCoreToDecision(input: Readonly<EngineState>, maxCommands = 16): CoreOrchestrationResult {
  ...
  for (let index = 0; index < maxCommands; index += 1) {
    ...
    if (pendingChoice) {
      ...
      if (frame.actions.length === 0) return { status: "UNSUPPORTED_CONFIGURATION", state, events, executedCommands, reasonCode: "CORE_NO_PRIVATE_CHOICE_ACTION" };
    ...
    if ([CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any) && state.priority?.open === true) {
      const responseActorId = currentPriorityActor(state)!;
      const frame = enumerateCoreResponseActions(state, responseActorId);
      if (frame.actions.length > 0) return { status: "PLAYER_DECISION_REQUIRED", state, events, executedCommands, decisionActorId: responseActorId, legalActionFrame: frame };
      ...
      const result = engine.execute(state, cmd); if (!result.accepted) return { status: "UNSUPPORTED_CONFIGURATION", state, events, executedCommands, reasonCode: `CORE_ORCHESTRATION_REJECTED:${result.error?.code}` };
      state = result.state; events.push(...result.events); executedCommands.push(cmd); continue;
    }
    if ([CORE_RESPONSE_AUTHORITY_PROFILE.id, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, CORE_ADVANCED_AUTHORITY_PROFILE.id, CORE_UNRESTRICTED_AUTHORITY_PROFILE.id].includes(core.profileId as any) && state.stack.length > 0 && state.priority?.open !== true) {
      const cmd = command(state, state.activePlayerId, `ORCH-${index}-RESOLVE`, { kind: "core-resolve-response-top" });
      const result = engine.execute(state, cmd); if (!result.accepted) return { status: "UNSUPPORTED_CONFIGURATION", state, events, executedCommands, reasonCode: `CORE_ORCHESTRATION_REJECTED:${result.error?.code}` };
      state = result.state; events.push(...result.events); executedCommands.push(cmd); continue;
    }
    if (state.stack.length > 0 || state.priority?.open === true) return { status: "UNSUPPORTED_CONFIGURATION", state, events, executedCommands, reasonCode: "CORE_STACK_STATE_UNEXPECTED" };
    if (state.phase === "Start" && core.startPreparedFullTurnSequence !== state.fullTurnSequence) {
    ...
    if (state.phase === "Start" || state.phase === "Action") {
      ...
      if (frame.actions.length === 0) return { status: "UNSUPPORTED_CONFIGURATION", state, events, executedCommands, reasonCode: "CORE_NO_LEGAL_ACTION" };
...
export interface CoreMatchResult { state: EngineState; commands: EngineCommand[]; events: EngineEvent[]; decisions: { actorId: PlayerId; actionId: string; frameHash: string }[]; terminationReason: "NORMAL_VICTORY" | "EXHAUSTED_RESOLUTION" | "CANONICAL_DRAW" | "DECISION_LIMIT" | "ENGINE_REJECTION" | "UNSUPPORTED_CONFIGURATION"; }

export function runCoreRandomLegalMatch(setup: CoreMatchSetup, decisionLimit = 800): CoreMatchResult {
  let state = createCoreMatchState(setup); const commands: EngineCommand[] = []; const events: EngineEvent[] = []; const decisions: CoreMatchResult["decisions"] = [];
  const policyRng: EngineState["rng"] = { algorithm: "xorshift32", seed: ((setup.seed ^ 0x42c0ffee) >>> 0) || 1, cursor: 0 }; const engine = new IntrilexEngine();
  for (let index = 0; index < decisionLimit; index += 1) {
    const advanced = advanceCoreToDecision(state); state = advanced.state; commands.push(...advanced.executedCommands); events.push(...advanced.events);
    ...
    const result = engine.execute(state, selected.command); commands.push(selected.command); events.push(...result.events);
