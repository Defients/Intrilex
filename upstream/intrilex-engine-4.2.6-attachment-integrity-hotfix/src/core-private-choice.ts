...
import type {
  CardId,
  CoreEffectAction,
  CorePrivateChoiceState,
  CorePrivateChoiceSubmission,
  CorePrimaryAction,
  EngineState,
  PlayerId,
  Visibility
...
export const CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE = Object.freeze({
  id: "core-private-choice-authority" as const,
...
const fail = (code: string, message: string, details?: unknown): CorePrivateChoiceFailure =>
  details === undefined ? { ok: false, code, message } : { ok: false, code, message, details };

function runtime(state: EngineState): CoreRuntimeWithChoice | null {
  const value = state.metadata.coreAuthority;
  return value && typeof value === "object" ? value as CoreRuntimeWithChoice : null;
}

export function isCorePrivateChoiceProfile(state: Readonly<EngineState>): boolean {
  const profileId = (state.metadata.coreAuthority as CoreRuntimeWithChoice | undefined)?.profileId;
...
export function activeCorePrivateChoice(state: Readonly<EngineState>): CorePrivateChoiceState | null {
  const value = (state.metadata.coreAuthority as CoreRuntimeWithChoice | undefined)?.privateChoice;
  return value && typeof value === "object" ? value : null;
}

function tokenCore(choice: Omit<CorePrivateChoiceState, "token">): unknown {
  return {
...
function createChoice(
  ...
  return { ...base, token: hashCanonical(tokenCore({ ...base, token: "" } as CorePrivateChoiceState)) };
}

function setChoice(state: EngineState, choice: CorePrivateChoiceState): void {
  const core = runtime(state);
  if (!core) throw new Error("Core runtime missing");
  core.privateChoice = choice;
}

function clearChoice(state: EngineState): void {
  const core = runtime(state);
...
function stageSource(state: EngineState, sourceCardId: CardId, actorId: PlayerId): void {
...
function completeSource(state: EngineState, sourceCardId: CardId): void {
  const card = state.cards[sourceCardId];
  if (!card) return;
  delete card.state.privateChoiceSource;
  delete card.state.draftFaceUp;
  if (card.zone === "STAGING" || card.zone === "VOID") moveCard(state, sourceCardId, "GY");
  clearChoice(state);
}

function holdPrivate(state: EngineState, cardId: CardId, chooserId: PlayerId, publicReveal = false): void {
  moveCard(state, cardId, "VOID");
  state.cards[cardId]!.state.privateChoiceHeldBy = chooserId;
  if (publicReveal) state.cards[cardId]!.state.privateChoicePublicReveal = true;
}

function releaseHeld(state: EngineState, cardId: CardId): void {
...
function validateSubmission(
  ...
  actorId: PlayerId,
  token: string,
  submission: CorePrivateChoiceSubmission
): CorePrivateChoiceState | string {
  if (!isCorePrivateChoiceProfile(state)) return "Core Private Choice Authority profile is not active";
  const choice = activeCorePrivateChoice(state);
  if (!choice) return "No Core private choice is pending";
  if (choice.chooserId !== actorId) return `${actorId} is not authorized for ${choice.choiceId}`;
  if (choice.token !== token) return "Core private choice token is stale or invalid";
  const expected = hashCanonical(tokenCore({ ...choice, token: "" } as CorePrivateChoiceState));
  if (expected !== choice.token || hashCanonical([...choice.optionCardIds].sort()) !== choice.optionsHash) return "Core private choice seal verification failed";
  if (choice.kind !== submission.kind) return `Core choice kind mismatch: expected ${choice.kind}`;
  const selected = submission.selectedCardIds;
  if (new Set(selected).size !== selected.length) return "Core private choice selections must be unique";
  if (selected.length < choice.minSelections || selected.length > choice.maxSelections) return `Core private choice requires ${choice.minSelections}-${choice.maxSelections} selections`;
  if (selected.some((id) => !choice.optionCardIds.includes(id))) return "Core private choice selected an unavailable card";
  return choice;
}

function rank(state: EngineState, cardId: CardId): string | null {
  return parseIdentity(state.cards[cardId]?.identity ?? "")?.rank ?? null;
}

function requireSource(state: EngineState, actorId: PlayerId, sourceCardId: CardId, expectedRank: string): string | null {
  const card = state.cards[sourceCardId];
  if (!card || card.controllerId !== actorId || card.zone !== `${actorId}_HAND`) return "Effect source must be controlled in hand";
  if (rank(state, sourceCardId) !== expectedRank) return `Effect source must be rank ${expectedRank}`;
  return null;
}

export function isCorePrivateChoiceEffect(effect: CoreEffectAction): boolean {
...
export function resolveCorePrivateChoiceRoot(
  ...
  if (effect.kind === "nine-anchor") {
    ...
    for (const existingId of [...state.players[actorId]!.er]) {
      if (existingId !== effect.sourceCardId && rank(state, existingId) === "9" && state.cards[existingId]?.state.playedForEffect === true) moveCard(state, existingId, "GY");
    }
    ...
    if (options.length === 0) return { ok: true, state, events };
    beginChoice(state, {
...
export function generatedCoreEffectCandidates(state: Readonly<EngineState>, actorId: PlayerId, cardId: CardId): CoreEffectAction[] {
  const card = state.cards[cardId];
...
export function resolveCorePrivateChoiceSubmission(
  ...
  if (typeof validation === "string") return fail("CORE_PRIVATE_CHOICE_INVALID", validation);
  ...
  if (submission.kind === "core-rank3-present") {
    ...
    if (selected.some((id) => state.cards[id]?.zone !== `${targetPlayerId}_HAND`)) return fail("CORE_PRIVATE_CHOICE_STALE", "Presented cards must remain in the opponent hand");
  ...
  if (submission.kind === "core-rank3-take") {
    const targetPlayerId = String(context.targetPlayerId);
    if (selected.some((cardId) => state.cards[cardId]?.zone !== `${targetPlayerId}_HAND`)) return fail("CORE_PRIVATE_CHOICE_STALE", "Taken cards must remain in the presenting opponent hand");
  ...
  if (submission.kind === "core-rank3-discard") {
    const targetPlayerId = String(context.targetPlayerId);
    if (selected.some((id) => state.cards[id]?.zone !== `${targetPlayerId}_HAND`)) return fail("CORE_PRIVATE_CHOICE_STALE", "Discarded cards must remain in the opponent hand");
  ...
  if (submission.kind === "core-rank5-rummage") {
    ...
    if (state.cards[cardId]?.zone !== "GY") return fail("CORE_PRIVATE_CHOICE_STALE", "Rummage choice must remain in GY");
    moveCard(state, cardId, `${choice.controllerId}_HAND`, choice.controllerId);
    revealUntilStart(state.cards[cardId]!, { playerId: choice.controllerId, startSequence: (state.startPhaseSequenceByPlayer[choice.controllerId] ?? 0) + 1 });
    let bottomDrawCardId: CardId | null = null;
  ...
  if (submission.kind === "core-rank6-dig") {
    ...
    if (drawn.some((id) => state.cards[id]?.state.privateChoiceHeldBy !== choice.controllerId)) return fail("CORE_PRIVATE_CHOICE_STALE", "Drawn Six cards are no longer sealed");
    if (submission.mode === "keep-all-discard") {
      if (selected.length !== 1 || !discardOptions.includes(selected[0]!)) return fail("CORE_PRIVATE_CHOICE_SELECTION", "Keep-all requires one legal discard");
      for (const id of drawn) { releaseHeld(state, id); moveCard(state, id, `${choice.controllerId}_HAND`, choice.controllerId); }
      const discardId = selected[0]!;
      if (state.cards[discardId]?.zone !== `${choice.controllerId}_HAND`) return fail("CORE_PRIVATE_CHOICE_STALE", "Six discard is no longer in hand");
      ...
      if (selected.length !== keepCount || selected.some((id) => !drawn.includes(id))) return fail("CORE_PRIVATE_CHOICE_SELECTION", `Six keep-return requires exactly ${keepCount} drawn cards`);
      const keep = new Set(selected);
      const returned = drawn.filter((id) => !keep.has(id));
      for (const id of selected) { releaseHeld(state, id); moveCard(state, id, `${choice.controllerId}_HAND`, choice.controllerId); }
      for (const id of returned) {
        releaseHeld(state, id);
        moveCard(state, id, "DP");
        const index = state.zones.dp.indexOf(id);
        state.zones.dp.splice(index, 1);
        if (submission.mode === "keep-return-top") state.zones.dp.unshift(id); else state.zones.dp.push(id);
      }
  ...
  if (submission.kind === "core-rank7-assign") {
    ...
    if (revealed.some((id) => state.cards[id]?.state.privateChoiceHeldBy !== choice.controllerId || state.cards[id]?.state.privateChoicePublicReveal !== true)) return fail("CORE_PRIVATE_CHOICE_STALE", "Revealed Seven cards are no longer sealed");
    let handCardId: CardId | null = null;
    let effectCardId: CardId | null = null;
    let scoreCardId: CardId | null = null;
    if (revealed.length === 1) {
      if (selected.length !== 1 || selected[0] !== revealed[0]) return fail("CORE_PRIVATE_CHOICE_SELECTION", "Single-card Seven must select the revealed card");
      if (submission.mode === "hand-only") handCardId = selected[0]!;
      else if (submission.mode === "effect-only") effectCardId = selected[0]!;
      else if (submission.mode === "score-only") scoreCardId = selected[0]!;
      else return fail("CORE_PRIVATE_CHOICE_SELECTION", "Single-card Seven cannot use hand-and-effect or hand-and-score");
    } else {
      if (submission.mode === "hand-and-effect") {
        if (selected.length !== 2 || new Set(selected).size !== 2 || selected.some((id) => !revealed.includes(id))) return fail("CORE_PRIVATE_CHOICE_SELECTION", "Two-card Seven requires ordered [hand,effect] assignment");
        handCardId = selected[0]!;
        effectCardId = selected[1]!;
      } else if (submission.mode === "hand-and-score") {
        if (selected.length !== 2 || new Set(selected).size !== 2 || selected.some((id) => !revealed.includes(id))) return fail("CORE_PRIVATE_CHOICE_SELECTION", "Two-card Seven requires ordered [hand,score] assignment");
        handCardId = selected[0]!;
    ...
    if (scoreCardId) {
      ...
      for (const id of revealed.filter((entry) => entry !== handCardId && entry !== scoreCardId)) { releaseHeld(state, id); moveCard(state, id, "DP"); }
    ...
    if (!effectCardId) {
      for (const id of revealed.filter((entry) => entry !== handCardId)) { releaseHeld(state, id); moveCard(state, id, "DP"); }
  ...
  if (submission.kind === "core-rank7-generated-effect") {
    ...
    if (selected.length !== 1 || selected[0] !== generatedCardId || state.cards[generatedCardId]?.state.privateChoiceHeldBy !== choice.controllerId) return fail("CORE_PRIVATE_CHOICE_STALE", "Generated Seven effect card is unavailable");
    ...
    if (!generatedEffect) {
      if (legalEffects.length > 0) return fail("CORE_PRIVATE_CHOICE_GENERATED", "A legal generated effect or score must be selected");
      releaseHeld(state, generatedCardId);
    ...
    if (!legalEffects.some((candidate) => hashCanonical(candidate) === effectHash)) return fail("CORE_PRIVATE_CHOICE_GENERATED", "Generated Seven effect is not legal from the sealed frame");
    releaseHeld(state, generatedCardId);
  ...
  if (submission.kind === "core-nine-anchor-discard") {
    const targetPlayerId = String(context.targetPlayerId);
    const cardId = selected[0]!;
    if (state.cards[cardId]?.zone !== `${targetPlayerId}_HAND`) return fail("CORE_PRIVATE_CHOICE_STALE", "Nine Anchor discard must remain in the opponent hand");
    moveCard(state, cardId, "GY");
    clearChoice(state);
