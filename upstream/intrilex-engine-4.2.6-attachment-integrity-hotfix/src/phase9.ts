...
function normalizeSetup(state: EngineState, playerIds: [PlayerId, PlayerId], teachingOverrideId?: string): string | null {
  ...
  for (const playerId of playerIds) {
    ...
    player.limits.queensCourtPlayedThisFT = false;
  }
...
export function resolvePhase9Action(input: EngineState, actorId: PlayerId, action: FirstContactAction): Phase9Resolution {
  ...
  switch (action.kind) {
    ...
    case "autonomy-exhausted-pass": {
      ...
      if (problem) return fail("AUTONOMY_ACTION", problem);
      const runtime = autonomyRuntime(state)!;
      if (!runtime.exhausted || state.zones.dp.length !== 0) return fail("AUTONOMY_EXHAUSTED_PASS", "Exhausted Pass requires active Exhausted with an empty Draw Pile");
      state.players[actorId]!.limits.miniTurnsUsed = 1;
      state.players[actorId]!.limits.miniTurnsRemaining = 0;
      state.phase = "End";
