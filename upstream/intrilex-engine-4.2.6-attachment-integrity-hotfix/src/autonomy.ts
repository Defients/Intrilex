...
export const FIRST_CONTACT_AUTONOMY_BASELINE_PROFILE = Object.freeze({
  ...
  allowedActions: ["draw", "play-for-points", "scuttle", "exhausted-pass"] as const,
...
export const FIRST_CONTACT_AUTONOMY_ESSENTIALS_PROFILE = Object.freeze({
  ...
  allowedActions: ["draw", "play-for-points", "play-for-effect", "scuttle", "exhausted-pass"] as const,
...
export function enumerateLegalActions(state: Readonly<EngineState>, query: LegalActionQuery): LegalActionFrame {
  ...
  const ordinaryActionsBeforePass = actions.length;
  const runtime = state.metadata.autonomy as { exhausted?: { remaining: number; startedFullTurnSequence: number } | null } | undefined;
  if (runtime?.exhausted && state.zones.dp.length === 0 && ordinaryActionsBeforePass === 0) actions.push(makeAction(state as EngineState, query.actorId, "exhausted-pass", "forced-mini-turn", [], [], "EXHAUSTED_PASS", { progress: true, forced: true }, { kind: "autonomy-exhausted-pass" }));
...
export function enumerateResponseActions(state: Readonly<EngineState>, query: LegalActionQuery): LegalActionFrame {
  ...
  const acceptedResponses = actions.filter((entry) => new IntrilexEngine().execute(state as EngineState, entry.command).accepted).sort((a, b) => a.actionId.localeCompare(b.actionId));
  if (acceptedResponses.length > 0) acceptedResponses.push(makeResponseAction(state as EngineState, query.actorId, "response-decline", "decline", [], target, "DECLINE_RESPONSE", { progress: false, hadLawfulResponse: true }, { kind: "autonomy-pass-priority", semantic: "DECLINE_RESPONSE" }));
  acceptedResponses.sort((a, b) => a.actionId.localeCompare(b.actionId));
  const frameCore = { stateRevision: state.revision, actorId: query.actorId, priorityIndex: priority.index, stackTopId: target?.id ?? null, actionHashes: acceptedResponses.map((entry) => ({ actionId: entry.actionId, commandHash: entry.commandHash })) };
  return { stateRevision: state.revision, actorId: query.actorId, actions: acceptedResponses, frameHash: hashCanonical(frameCore) };
}
...
export function advanceToDecision(input: Readonly<EngineState>, limits: OrchestrationLimits = {}): OrchestrationResult {
  ...
  for (let count = 0; count < maxCommands; count += 1) {
    ...
    if (isResponseProfile(state) && state.priority?.open === true) {
      ...
      if (legalActionFrame.actions.length > 0) return { status: "PLAYER_DECISION_REQUIRED", state, events, executedCommands, decisionActorId: priorityActorId, legalActionFrame };
      const command: EngineCommand = { id: commandId(state, priorityActorId, `ORCH-${count}-AUTO-PRIORITY`), type: "RESOLVE_PHASE9_ACTION", actorId: priorityActorId, action: { kind: "autonomy-pass-priority", semantic: "AUTOMATIC_PRIORITY_ADVANCE" } };
      const result = engine.execute(state, command);
      if (!result.accepted) return { status: "UNSUPPORTED_CONFIGURATION", state, events, executedCommands, reasonCode: `ORCHESTRATION_REJECTED:${result.error?.code ?? "UNKNOWN"}` };
      state = result.state; events.push(...result.events); executedCommands.push(command); continue;
    }
