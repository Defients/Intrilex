...
function isDrawPileZone(zone: string | undefined): boolean {
...
export function publicStateView(state: EngineState): unknown {
  ...
  for (const card of Object.values(clone.cards)) {
    ...
    if (isDrawPileZone(card.zone) || card.zone.endsWith("_HAND") || card.state.faceDownTrap === true || card.state.swapBarFaceDown === true || (card.zone === "STAGING" && card.state.draftFaceUp === false) || privateChoiceHidden) card.identity = "HIDDEN";
    delete card.state.privateChoiceHeldBy;
    delete card.state.privateChoicePublicReveal;
    delete card.state.privateChoiceSource;
  }
  delete (clone as unknown as Record<string, unknown>).rng;
...
export function privateStateView(state: EngineState, viewerId: PlayerId): unknown {
  ...
  for (const card of Object.values(clone.cards)) {
    ...
    if (isDrawPileZone(card.zone) || ((card.zone.endsWith("_HAND") || card.state.faceDownTrap === true || card.state.swapBarFaceDown === true) && card.controllerId !== viewerId) || (card.zone === "STAGING" && card.state.draftFaceUp === false) || privateChoiceHidden) card.identity = "HIDDEN";
    delete card.state.privateChoiceHeldBy;
    delete card.state.privateChoicePublicReveal;
    delete card.state.privateChoiceSource;
  }
