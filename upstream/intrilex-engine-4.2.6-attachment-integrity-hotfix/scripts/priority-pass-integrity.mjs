...
async function collect(dir=root,prefix=''){const rows=[];const entries=await readdir(dir,{withFileTypes:true});entries.sort((a,b)=>a.name<b.name?-1:a.name>b.name?1:0);for(const entry of entries){const rel=prefix?`${prefix}/${entry.name}`:entry.name;if(!prefix&&excludedTop.has(entry.name))continue;if(entry.name==='__pycache__')continue;if(excludedExact.has(rel)||rel.endsWith('.log')||rel.endsWith('.tmp')||rel.endsWith('.pyc')||/^reports\/core-(advanced|private|response)-segment-/.test(rel))continue;const abs=path.join(dir,entry.name);const st=await lstat(abs);if(st.isSymbolicLink())throw new Error(`Manifest refuses symlink: ${rel}`);if(st.isDirectory())rows.push(...await collect(abs,rel));else if(st.isFile()){const bytes=await readFile(abs);rows.push({path:rel,size:bytes.length,sha256:sha256(bytes)});}}return rows;}
