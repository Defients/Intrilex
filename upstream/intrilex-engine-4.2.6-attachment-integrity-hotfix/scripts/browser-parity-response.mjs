...
const reportPath = path.join(root, 'reports/browser-response-parity.json');
const writeReports = process.env.INTRILEX_WRITE_REPORTS !== '0';
const shimPath = '/mnt/data/intrilex-simulation-lab-v0.3.0/packages/browser-crypto-shim/src/hash.js';

const built = spawnSync('npm', ['run', 'build'], { cwd: root, stdio: 'inherit' });
if (built.status !== 0) process.exit(built.status ?? 1);

const work = await mkdtemp(path.join(tmpdir(), 'intrilex-413-browser-'));
...
await writeFile(path.join(engineDir, 'browser-entry.js'), [
  ...
  "export { FIRST_CONTACT_RESPONSE_AUTHORITY_PROFILE, createMatchState, enumerateLegalActions, authorizedLegalActionView, advanceToDecision, autonomousCapabilities, runRandomLegalMatch } from './autonomy.js';",
  ''
].join('\n'));
...
const setup = { profileId: 'first-contact-response', playerIds: ['P1', 'P2'], enabledModules: [], eventApprovedModules: [], seed: 0x413c0de, seatOrder: ['P1', 'P2'] };
...
const profileDir = await mkdtemp(path.join(tmpdir(), 'intrilex-413-chromium-'));
...
try {
  ...
  const browserProgram = `async () => {
    ...
    const match = __engine.runRandomLegalMatch({profileId:'first-contact-response',playerIds:['P1','P2'],enabledModules:[],eventApprovedModules:[],seed:0x413c0de,seatOrder:['P1','P2']},1600);
    return {
      ...
      match:{terminationReason:match.terminationReason,winner:match.state.winner,decisionCount:match.decisions.length,commandCount:match.commands.length,finalStateHash:__engine.hashCanonical(match.state),decisionHash:__engine.hashCanonical(match.decisions)},
      vectors:vectors.length,
      userAgent:navigator.userAgent
    };
  }`;
  const main = await cdp.evaluate(`(${browserProgram})()`, true);
  const workerSource = `self.onmessage = async ({data}) => { try { const e=await import(data.entry); const summaries=data.replays.map((replay)=>{const v=e.verifyCertifiedReplay(replay);return {fixtureId:replay.fixtureId,contentHash:replay.contentHash,finalStateHash:replay.finalStateHash,accepted:v.accepted.length,events:v.events.length};}); const match=e.runRandomLegalMatch({profileId:'first-contact-response',playerIds:['P1','P2'],enabledModules:[],eventApprovedModules:[],seed:0x413c0de,seatOrder:['P1','P2']},1600); self.postMessage({ok:true,result:{replay:{replayCount:summaries.length,commandCount:summaries.reduce((s,x)=>s+x.accepted,0),eventCount:summaries.reduce((s,x)=>s+x.events,0),aggregateHash:e.hashCanonical(summaries)},match:{terminationReason:match.terminationReason,winner:match.state.winner,decisionCount:match.decisions.length,commandCount:match.commands.length,finalStateHash:e.hashCanonical(match.state),decisionHash:e.hashCanonical(match.decisions)}}}); } catch(error){self.postMessage({ok:false,error:error?.stack??String(error)});} };`;
  ...
  const parity = {
    ...
    normalTermination: main.match.terminationReason === 'NORMAL_VICTORY',
  ...
  const report = {
    schemaVersion: '1.0', status: 'PASS', engineVersion: '4.1.3', profileId: 'first-contact-response',
    proofKind: 'REAL_CHROMIUM_MAIN_THREAD_AND_WEB_WORKER', browser: main.userAgent,
    certifiedReplayCorpus: nodeReplay, responseAuthorityMatch: nodeMatch,
  ...
  console.log(`BROWSER RESPONSE PARITY PASS: replays=${nodeReplay.replayCount}; replayHash=${nodeReplay.aggregateHash}; matchHash=${nodeMatch.finalStateHash}`);
} finally {
