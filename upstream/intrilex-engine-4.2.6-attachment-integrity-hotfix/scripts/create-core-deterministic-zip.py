...
root=Path(sys.argv[1]).resolve(); out=Path(sys.argv[2]).resolve(); manifest=json.loads((root/'CORE_AUTHORITY_MANIFEST.json').read_text())
paths=sorted(set([r['path'] for r in manifest['files']]+['CORE_AUTHORITY_MANIFEST.json','CORE_AUTHORITY_SHA256SUMS']))
