...
async function prepareSevenScoringTrigger() {
    ...
    for (let guard = 0; guard < 12; guard += 1) {
        ...
        const choice = triggerPriority.state.metadata.autonomy.privateChoice;
        if (choice?.kind === "rank7-scoring-trigger")
            break;
...
test("4.1.5 scoring Seven queues and flushes an engine-owned non-play trigger", async () => {
    ...
    assert.equal(triggerPriority.state.metadata.autonomy.privateChoice.kind, "rank7-scoring-trigger");
...
test("4.1.5 Seven scoring trigger auto-closes when no response family is legal", async () => {
    ...
    const top = { id: state.metadata.autonomy.privateChoice.sourceStackItemId ?? "closed-trigger" };
    const before = hashCanonical(state);
...
test("4.2.1 every enumerated Core effect declaration is engine-accepted and command-vault isolated", async () => {
    ...
    for (const required of ["effect-ace", "effect-three", "effect-four", "attachment", "anchor", "effect-red-joker"])
...
test("4.2.1 Board Lock removes effects and Scuttle while preserving allowed Core actions and expires canonically", async () => {
    ...
    decision = advanceCoreToDecision(state);
    const lock = decision.legalActionFrame.actions.find((entry) => entry.mode === "board-lock");
...
test("4.2.2 root effects are sealed on stack and every enumerated response is engine-accepted without command-vault leakage", async () => {
    ...
    for (const expected of ["response-decline:decline", "counter:ace-base", "instant:eight-spade-free-scuttle", "instant:nine-tap", "disrupt:jack"])
...
test("4.2.5 3 Red Ultra counters a root and preserves its bottom-GY draw rider", async () => {
    ...
    assert.ok(["Action", "End", "Start"].includes(state.phase));
});
test("4.2.5 3 Red Ultra inherits Super Ace's two-Queen declaration prohibition", async () => {
    const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
    const engine = new IntrilexEngine();
    let state = createCoreMatchState({ profileId: CORE_ADVANCED_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42413 });
    let d = advanceCoreToDecision(state);
    state = engine.execute(d.state, d.legalActionFrame.actions.find((x) => x.mode === "enter-action").command).state;
    const by = Object.fromEntries(Object.values(state.cards).map((c) => [c.identity, c.id]));
    for (const x of ["Q♣", "Q♠"])
        moveCard(state, by[x], "P1_ER", "P1");
    for (const x of ["A♦", "4♥", "6♦"])
        moveCard(state, by[x], "P2_HAND", "P2");
    moveCard(state, by["3♣"], "P1_HAND", "P1");
    moveCard(state, by["2♣"], "P2_PR", "P2");
...
test("Solo Wild Copy: 2♥ copying 3♥ bounce resolves correctly and 2 goes to GY", async () => {
    ...
    const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
    moveCard(state, by["2♥"], "P1_HAND", "P1");
    moveCard(state, by["5♣"], "P2_PR", "P2");
    state.cards[by["5♣"]].state.pointValue = 5;
    const before = hashCanonical(state);
    const r = engine.execute(state, { id: "SWC-BOUNCE", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"], targetRank: "3", copiedAction: { kind: "three-bounce", targetCardId: by["5♣"] } } });
    assert.equal(r.accepted, true);
...
test("Solo Wild Copy: 2♠ copying 4♠ total-clear resolves correctly", async () => {
    ...
    moveCard(state, by["5♣"], "P2_PR", "P2");
    state.cards[by["5♣"]].state.pointValue = 5;
    moveCard(state, by["6♦"], "P1_PR", "P1");
    state.cards[by["6♦"]].state.pointValue = 6;
    const r = engine.execute(state, { id: "SWC-TOTAL-CLEAR", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♠"], targetRank: "4", copiedAction: { kind: "total-clear" } } });
    assert.equal(r.accepted, true);
    assert.equal(r.state.cards[by["2♠"]]?.zone, "GY");
...
test("Solo Wild Copy: 2♥ cannot copy 4♠ total-clear (suit mismatch)", async () => {
    ...
    moveCard(state, by["2♥"], "P1_HAND", "P1");
    const before = hashCanonical(state);
    const r = engine.execute(state, { id: "SWC-SUIT-FAIL", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"], targetRank: "4", copiedAction: { kind: "total-clear" } } });
    assert.equal(r.accepted, false);
    assert.equal(hashCanonical(r.state), before);
});
test("Solo Wild Copy: 2♥ copying 5♥ recycle resolves and 2 goes to GY", async () => {
    ...
    const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
    moveCard(state, by["2♥"], "P1_HAND", "P1");
    const r = engine.execute(state, { id: "SWC-RECYCLE", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"], targetRank: "5", copiedAction: { kind: "recycle-five" } } });
    assert.equal(r.accepted, true);
    assert.equal(r.state.cards[by["2♥"]]?.zone, "GY");
    assert.ok(r.events.some((e) => e.type === "SOLO_WILD_COPY_RESOLVED"));
});
test("Solo Wild Copy: 2♥ copying 7♥ topdeck resolves and 2 goes to GY", async () => {
    ...
    const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
    moveCard(state, by["2♥"], "P1_HAND", "P1");
    const r = engine.execute(state, { id: "SWC-TOPDECK", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"], targetRank: "7", copiedAction: { kind: "topdeck-seven" } } });
    assert.equal(r.accepted, true);
    assert.equal(r.state.cards[by["2♥"]]?.zone, "GY");
    assert.ok(r.events.some((e) => e.type === "SOLO_WILD_COPY_RESOLVED"));
});
test("Solo Wild Copy: does not create ⭐2 (not a commandeer play)", async () => {
    ...
    moveCard(state, by["5♣"], "P2_PR", "P2");
    state.cards[by["5♣"]].state.pointValue = 5;
    const r = engine.execute(state, { id: "SWC-NOT-STAR-2", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"], targetRank: "3", copiedAction: { kind: "three-bounce", targetCardId: by["5♣"] } } });
    assert.equal(r.accepted, true);
...
test("Solo Wild Copy: 2♥ copying 4♥ row-clear clears opponent PR cards", async () => {
    ...
    moveCard(state, by["6♦"], "P2_PR", "P2");
    state.cards[by["6♦"]].state.pointValue = 6;
    const r = engine.execute(state, { id: "SWC-ROW-CLEAR", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"], targetRank: "4", copiedAction: { kind: "four-row-clear", targetPlayerId: "P2", row: "pr" } } });
    assert.equal(r.accepted, true);
    assert.equal(r.state.cards[by["2♥"]]?.zone, "GY");
