...
function checkpointRngTrace(checkpoints) {
...
function certifiedContent(envelope) {
    return envelope;
}
export function createCertifiedReplay(fixtureId, initialState, commands, engineVersion = PHASE16_ENGINE_VERSION) {
...
export function verifyCertifiedReplay(replay) {
    ...
    if (contentHash !== expectedContentHash)
        throw new EngineError("REPLAY_CONTENT_HASH", "Certified replay content hash mismatch");
    if (integrityHash !== sha256Text(`intrilex-certified-replay-v2:${contentHash}`))
        throw new EngineError("REPLAY_INTEGRITY_HASH", "Certified replay integrity hash mismatch");
    ...
    for (const field of exactFields) {
        if (canonicalize(rebuilt[field]) !== canonicalize(replay[field]))
            throw new EngineError("REPLAY_MISMATCH", `Certified replay mismatch at ${String(field)}`);
    }
    return { state: runFinalState(replay.initialState, replay.commands), events: rebuilt.events, accepted: rebuilt.accepted };
}
function runFinalState(initialState, commands) {
    const engine = new IntrilexEngine();
    let state = canonicalClone(initialState);
    for (const command of commands)
        state = engine.execute(state, command).state;
    return state;
}
...
export function publicCertifiedReplayView(replay) {
...
export function serializeCertifiedReplay(replay) {
    return canonicalize(replay);
}
export function parseCertifiedReplay(text) {
    let parsed;
    try {
        parsed = JSON.parse(text);
    }
    catch (error) {
        throw new EngineError("REPLAY_PARSE", "Replay is not valid JSON", error instanceof Error ? error.message : String(error));
    }
    if (typeof parsed !== "object" || parsed === null)
        throw new EngineError("REPLAY_PARSE", "Replay root must be an object");
    const replay = parsed;
    verifyCertifiedReplay(replay);
    if (serializeCertifiedReplay(replay) !== canonicalize(JSON.parse(text)))
        throw new EngineError("REPLAY_CANONICAL", "Replay does not round-trip canonically");
    return replay;
}
export function runRngVector(vector) {
    // Imported lazily by static code to keep this module observational.
    const state = { algorithm: "xorshift32", seed: vector.seed >>> 0, cursor: vector.cursor };
    const uint32 = [];
    const indices = [];
    ...
    for (const length of vector.lengths) {
        ...
        state.seed = x >>> 0;
        state.cursor += 1;
        indices.push(state.seed % length);
    }
    return { uint32, indices, final: state };
}
//# sourceMappingURL=phase16.js.map