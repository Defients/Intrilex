...
export function phase13Runtime(state) {
    const current = state.metadata.phase13;
    return {
...
function suitOfQueen(card) {
    const match = /^Q([♣♦♥♠])$/.exec(card.identity);
    return match ? match[1] : null;
}
export function bombMarker(card) {
    const value = card.state.timeBomb;
    if (typeof value !== "object" || value === null)
        return null;
    const marker = value;
    if (!["♣", "♦", "♥", "♠"].includes(marker.suit) || !Number.isInteger(marker.stage) || !Number.isInteger(marker.peak))
        return null;
    return { suit: marker.suit, stage: marker.stage, peak: marker.peak };
}
export function isTimeBomb(card) {
    return card.zone.endsWith("_PR") && card.state.faceDownTrap !== true && bombMarker(card) !== null;
}
export function stageValue(suit, stage) {
    const track = TRACKS[suit];
    const bounded = Math.max(0, Math.min(stage, track.length - 1));
    return track[bounded] ?? 0;
}
export function createTimeBomb(card) {
    const suit = suitOfQueen(card);
    if (!suit)
        throw new Error(`${card.identity} is not a suited Queen`);
    const marker = { suit, stage: 0, peak: TRACKS[suit].length - 1 };
    card.state.timeBomb = marker;
    card.state.timeBombStage = 0;
    card.state.pointValue = 0;
    return marker;
}
export function clearTimeBomb(card) {
    delete card.state.timeBomb;
    delete card.state.timeBombStage;
    delete card.state.forcedDrawSource;
}
function nextEnemy(state, controllerId) {
    ...
    for (let offset = 1; offset < state.turnOrder.length; offset += 1) {
        ...
        if (candidate && relationBetween(state, controllerId, candidate) === "enemy")
            return candidate;
    }
    return null;
}
function removeChosenEnemyCards(state, controllerId, choices, events) {
    ...
    for (const enemyId of enemies) {
        ...
        if (!chosen || !player.hand.includes(chosen))
            return fail("TIME_BOMB_CHOICE", `Q♥ Peak requires a legal discard choice for ${enemyId}`);
    }
    for (const enemyId of enemies) {
        const chosen = choices?.[enemyId];
        if (!chosen)
...
function resolveClubPeak(state, controllerId, events) {
    const newest = state.zones.gy[0];
    const oldest = state.zones.gy[state.zones.gy.length - 1];
    const selected = [...new Set([newest, oldest].filter((id) => id !== undefined))];
...
export function resolvePhase13Action(input, actorId, action) {
    ...
    switch (action.kind) {
        ...
        case "score-queen-as-bomb": {
            if (!runtime.enabled)
                return fail("TIME_BOMB_DISABLED", "Time Bomb Mode is not enabled");
            const card = state.cards[action.cardId];
            if (!card || card.controllerId !== action.playerId || card.zone !== `${action.playerId}_HAND`)
                return fail("TIME_BOMB_SCORE", "The Queen must be controlled in the scoring player's hand");
            if (!suitOfQueen(card))
                return fail("TIME_BOMB_SCORE", "Only a suited Queen can become a Time Bomb");
        ...
        case "queue-fuse-triggers": {
            ...
            for (const cardId of ids)
                events.push({ type: "TIME_BOMB_FUSE_QUEUED", payload: { cardId, controllerId: action.playerId } });
            return { ok: true, state, events };
        }
        case "resolve-fuse": {
            if (!runtime.enabled)
                return fail("TIME_BOMB_DISABLED", "Time Bomb Mode is not enabled");
            const card = state.cards[action.cardId];
            if (!card || !isTimeBomb(card))
                return fail("TIME_BOMB_TARGET", "Fuse target is not a legal Time Bomb");
            ...
            if (nextStage === marker.peak) {
                if (marker.suit === "♣")
                    resolveClubPeak(state, card.controllerId, events);
                else if (marker.suit === "♦") {
        ...
        case "declare-defuse": {
            if (!runtime.enabled)
                return fail("TIME_BOMB_DISABLED", "Time Bomb Mode is not enabled");
            if (!action.responseWindow)
                return fail("DEFUSE_TIMING", "Defuse requires a legal response window");
            const target = state.cards[action.targetCardId];
            if (!target || !isTimeBomb(target))
                return fail("DEFUSE_TARGET", "Defuse requires a legal Time Bomb target");
            if (hasAegis(target))
                return fail("DEFUSE_AEGIS", "Aegis blocks Defuse");
            const actor = state.players[actorId];
            if (!actor)
                return fail("PLAYER_UNKNOWN", `Unknown player ${actorId}`);
            const marker = bombMarker(target);
            const required = marker.stage === marker.peak ? 1 : 2;
            if (action.costCardIds.length !== required || action.costCardIds.some((id) => !actor.hand.includes(id)))
                return fail("DEFUSE_COST", `Defuse requires exactly ${required} legal hand card(s)`);
            for (const cardId of action.costCardIds)
        ...
        case "consume-action-phase-skip": {
            ...
            if (!player)
                return fail("PLAYER_UNKNOWN", `Unknown player ${action.playerId}`);
            if (player.limits.pendingActionPhaseSkips < 1)
        ...
        case "enforce-forced-draw": {
            ...
            if (!pending)
                return fail("FORCED_DRAW", "No pending Q♦ forced-Draw requirement exists");
            if (action.actionPhaseSkipped)
                return fail("FORCED_DRAW_WAITING", "A skipped Action Phase does not consume the requirement");
            if (action.drawLegal) {
                if (action.declaredAction !== "draw")
                    return fail("FORCED_DRAW_REQUIRED", "Draw must be the first Action");
            }
            else if (action.declaredAction !== "pass")
                return fail("FORCED_PASS_REQUIRED", "When Draw is illegal, Pass is required");
        ...
        case "change-bomb-controller": {
            const card = state.cards[action.cardId];
            if (!card || !isTimeBomb(card) || !state.players[action.controllerId])
                return fail("TIME_BOMB_CONTROL", "Invalid Time Bomb control change");
            ...
            saveRuntime(state, runtime);
            events.push({ type: "TIME_BOMB_CONTROLLER_CHANGED", payload: runtime.lastResolution });
            return { ok: true, state, events };
        }
        case "move-time-bomb": {
            const card = state.cards[action.cardId];
            ...
            if (wasBomb && !action.destination.endsWith("_PR"))
                clearTimeBomb(card);
            ...
            saveRuntime(state, runtime);
            events.push({ type: "TIME_BOMB_MOVED", payload: runtime.lastResolution });
            return { ok: true, state, events };
        }
    }
}
//# sourceMappingURL=phase13.js.map