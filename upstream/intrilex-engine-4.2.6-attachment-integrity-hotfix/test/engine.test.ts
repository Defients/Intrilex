...
import type { ConformanceFixture } from "../src/types.js";
...
async function fixtures(): Promise<ConformanceFixture[]> {
...
async function lifecycleFixtures(): Promise<ConformanceFixture[]> {
  return loadFixtures(lifecycleFixturePath);
}

function finalOf(fixture: ConformanceFixture) {
...
async function rankFixtures(): Promise<ConformanceFixture[]> {
...
async function interactionFixtures(): Promise<ConformanceFixture[]> {
...
async function phase8Fixtures(): Promise<ConformanceFixture[]> {
...
test("Voltage eligibility is fixed at snapshot and each rank resolves once", async () => {
  ...
  const phase8 = result.state.metadata.phase8 as { voltageUsedThisFT?: Record<string, Record<string, boolean>> };
...
test("End Phase timer order short-circuits before Exhausted after Sudden Death", async () => {
  ...
  for (const id of ["CT-015","CT-086"]) {
    ...
    const phase8 = result.state.metadata.phase8 as { exhausted?: { remaining: number } };
...
async function phase9Fixtures(): Promise<ConformanceFixture[]> {
...
test("First Contact setup is a profile over the same state kernel", () => {
  ...
  assert.equal((result.state.metadata.firstContact as { active?: boolean }).active, true);
  assert.deepEqual((result.state.metadata.firstContact as { allowedActions?: string[] }).allowedActions, [...FIRST_CONTACT_PROFILE.allowedActions]);
});
...
async function trapFixtures(): Promise<ConformanceFixture[]> {
...
test("CT-097 Board Lock suppresses rather than spends Trap trigger", async () => {
  ...
  assert.equal((result.state.metadata.phase10 as { triggerUsedThisActiveFT?: Record<string, boolean> } | undefined)?.triggerUsedThisActiveFT?.P2, undefined);
});
...
test("public and opponent views redact face-down Trap identities", async () => {
  ...
  const fixture = (await trapFixtures()).find((entry) => entry.id === "CT-095");
  assert.ok(fixture);
  const publicView = publicStateView(fixture.initialState) as typeof fixture.initialState;
  const ownerView = privateStateView(fixture.initialState, "P2") as typeof fixture.initialState;
  const opponentView = privateStateView(fixture.initialState, "P1") as typeof fixture.initialState;
...
async function multiplayerFixtures(): Promise<ConformanceFixture[]> {
...
test("multiplayer relation helpers separate Ally, Enemy, and self", async () => {
  ...
  state.players.P1!.teamId = "A"; state.players.P3!.teamId = "A";
  state.players.P2!.teamId = "B"; state.players.P4!.teamId = "B";
...
test("CT-105 closes four-player priority only after all four consecutive passes", async () => {
  ...
  for (let index = 0; index < 4; index += 1) state = new IntrilexEngine().execute(state, fixture.commands[index]!).state;
...
test("CT-114 aggregates team Anchors before signed Points", async () => {
  ...
  assert.ok(fixture);
  const state = runCommands(fixture.initialState, fixture.commands).state;
  const phase11 = state.metadata.phase11 as { winningTeamId?: string; lastTeamTotals?: Record<string,{anchors:number;points:number}> };
...
async function battleRealmFixtures(): Promise<ConformanceFixture[]> {
...
test("Mastermind logs private order as authorized rather than public", () => {
  const state = createEmptyState();
  for (const [id,identity] of [["M1","A♣"],["M2","2♣"],["M3","3♣"],["M4","4♣"],["M5","5♣"]] as const) addCard(state,{id,identity,originalOwnerId:"P1",zone:"DP"});
  const engine = new IntrilexEngine();
...
async function timeBombFixtures(): Promise<ConformanceFixture[]> {
...
test("Q♦ Peak targets the next Enemy rather than the next Ally", () => {
  ...
  const runtime = result.state.metadata.phase13 as {forcedDrawByPlayer:Record<string,unknown>};
  assert.equal(result.accepted,true);
...
async function deffyFixtures(): Promise<ConformanceFixture[]> {
...
test("CT-115 Classic draft reaches exact 5/6 hands without gameplay Stack use", async () => {
  ...
  const runtime = result.state.metadata.phase14 as { status:string; specsMayBeRevealed:boolean; faceDownPicksByDrafter:Record<string,number> };
...
test("Deffy sub-mode shapes are canonical and Soda is two-player only", async () => {
  ...
  let sodaError = "";
  try { deffyPoolShape("soda",3); } catch (error) { sodaError = error instanceof Error ? error.message : String(error); }
...
test("starting player's first face-down pick is a total rollback", () => {
  const state = createEmptyState();
  for (let index=0; index<21; index+=1) addCard(state,{id:`DF-${index}`,identity:index===0?"A♣":"2♣",originalOwnerId:"P1",zone:"DP"});
  const engine = new IntrilexEngine();
...
test("Speed Run timeout consumes serialized RNG and selects only face-up pool cards", () => {
  const state=createEmptyState();
  for(let i=0;i<12;i+=1) addCard(state,{id:`SR-${i}`,identity:`${(i%9)+1}♣`,originalOwnerId:"P1",zone:"DP"});
  const engine=new IntrilexEngine();
  ...
  const runtime=result.state.metadata.phase14 as {rngAudit:Array<{operation:string}>};
...
test("pool exhaustion refills exactly three face-up cards when available", () => {
  const state=createEmptyState();
  for(let i=0;i<4;i+=1) addCard(state,{id:`RF-${i}`,identity:`A♣`,originalOwnerId:"P1",zone:"DP"});
  state.phase="Setup";
...
async function tournamentSeedFixtures(): Promise<ConformanceFixture[]> {
...
test("CT-004 constructs the exact Ban Pile, six-card hands, and Swap Bar", async () => {
  ...
  const runtime = result.state.metadata.phase15 as { banPileCardIds:string[]; activeHighImpactPool:string[]; status:string };
...
test("CT-005 resolves a High-Impact collision through ranked backup", async () => {
  ...
  assert.ok(fixture);
  const result = runCommands(fixture.initialState, fixture.commands);
  const runtime = result.state.metadata.phase15 as { highImpactAssignments:Record<string,string> };
...
test("banned Tournament Seed cards are outside all gameplay zones", async () => {
  ...
  const runtime = result.state.metadata.phase15 as { banPileCardIds:string[] };
...
test("certified replay v2 verifies checkpoints and detects tampering", async () => {
  ...
  let rejected = false;
  try { verifyCertifiedReplay(tampered); } catch { rejected = true; }
...
test("Phase 16 RNG vectors freeze unsigned xorshift behavior", async () => {
  ...
  const vectors = JSON.parse(await readFile(path.join(root,"fixtures","phase16-rng-vectors.json"),"utf8")) as Array<{name:string;seed:number;cursor:number;draws:number;lengths:number[];expectedUint32:number[];expectedIndices:number[]}>;
...
test("judge assistant distinguishes illegal declaration from later fizzle", async () => {
  ...
  const synthetic = { accepted: true, state: fizzleResult.state, events: [...fizzleResult.events, { id:"J-FIZZLE", sequence:999, commandId:"J", type:"PLAY_FIZZLED", visibility:"public" as const, payload:{}, previousStateHash:"x", stateHash:"y" }] };
...
async function canonicalClosureFixtures(): Promise<ConformanceFixture[]> { return loadFixtures(canonicalClosurePath); }
...
test("CT-013 transfers both cards but Nine rejects fresh Aegis", async () => {
  const fixture=(await canonicalClosureFixtures()).find((entry)=>entry.id==="CT-013"); assert.ok(fixture);
  const state=runCommands(fixture.initialState,fixture.commands).state;
...
test("CT-034 consumes only the pending skip and performs no Full Turn reset", async () => {
  const fixture=(await canonicalClosureFixtures()).find((entry)=>entry.id==="CT-034"); assert.ok(fixture);
  const state=runCommands(fixture.initialState,fixture.commands).state;
  assert.equal(state.fullTurnSequence,12); assert.equal(state.startPhaseSequenceByPlayer.P1,5);
  assert.equal(state.players.P1!.limits.rank10PlayedThisFT,true); assert.equal(state.players.P1!.limits.pendingFullTurnSkips,0);
  assert.equal((state.metadata.phase8 as any).boardLock.remaining,2);
});

test("CT-035 changes controller and row without changing original owner", async () => {
  const fixture=(await canonicalClosureFixtures()).find((entry)=>entry.id==="CT-035"); assert.ok(fixture);
  const state=runCommands(fixture.initialState,fixture.commands).state;
...
test("Phase 19 simulation campaign is deterministic and uses complete matches", async () => {
  ...
  assert.equal((report.fullMatchCampaign as Record<string,unknown>).matchCount,10800);
  assert.equal((report.scenarioBaseline as Record<string,unknown>).scenarioCount,121);
});
...
test("4.1.1 legal action enumeration is sound and hides command payloads from policy view", async () => {
  ...
  assert.ok(advanced.legalActionFrame.actions.length >= 6);
  assert.equal(advanced.legalActionFrame.actions.some((entry) => entry.family === "pass"), false);
  const engine = new IntrilexEngine();
...
test("4.1.1 action frame does not expose opponent hand identities or ids", async () => {
  ...
  for (const id of opponentHand) assert.doesNotMatch(text, new RegExp(id));
  for (const id of opponentHand) assert.doesNotMatch(text, new RegExp(state.cards[id]!.identity.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
...
const essentialsSetup = { ...autonomySetup, profileId: "first-contact-essentials", seed: 0x51e77a1 } as const;

async function placeIdentity(state: ReturnType<typeof createEmptyState>, identity: string, destination: string, controllerId: string): Promise<string> {
  ...
  moveCard(state, card.id, destination as `${string}_HAND` | `${string}_PR` | `${string}_ER`, controllerId);
  return card.id;
}
...
test("4.1.2 Essentials enumerates only engine-accepted effects and Anchors", async () => {
  ...
  state = initial.state;
  for (const identity of ["4♣", "9♣", "J♣", "Q♣", "K♣", "RJ", "BJ"]) await placeIdentity(state, identity, "P1_HAND", "P1");
  ...
  for (const family of ["effect-row-clear", "effect-tap", "effect-goal-shift", "effect-jack-control", "anchor-guard", "anchor", "effect-red-joker", "effect-board-lock"]) assert.ok(families.has(family), family);
  const engine = new IntrilexEngine();
  for (const action of advanced.legalActionFrame.actions) {
...
test("4.1.2 Board Lock fail-closes Scuttle, effects, and Anchor plays", async () => {
  ...
  const frame = enumerateLegalActions(state, { actorId: "P1", visibility: "PLAYER_AUTHORIZED", profileId: "first-contact-essentials", enabledModules: [] });
  assert.deepEqual([...new Set(frame.actions.map((entry) => entry.family))].sort(), ["draw", "play-for-points"]);
});
...
test("4.1.2 autonomy capabilities are explicit and fail closed outside teaching overrides", async () => {
  ...
  assert.deepEqual(autonomousCapabilities(), [{
    ...
    playerCounts: [2],
    moduleSets: [[]],
    completeActionFamilies: ["draw", "play-for-points", "scuttle", "exhausted-pass"],
    status: "SUPPORTED",
    ...
    playerCounts: [2],
    moduleSets: [[]],
    completeActionFamilies: ["draw", "play-for-points", "play-for-effect", "scuttle", "exhausted-pass", "A-base-counter", "8-scuttle-counter", "J-disrupt", "K-anchor-goal-counter"],
    status: "SUPPORTED",
    ...
    playerCounts: [2],
    moduleSets: [[]],
    completeActionFamilies: ["draw", "play-for-points", "play-for-effect", "scuttle", "exhausted-pass", "A-base-counter", "8-scuttle-counter", "J-disrupt", "K-anchor-goal-counter", "3-opponent-presentation", "3-opponent-discard", "3-caster-take", "5-rummage", "6-private-draw", "7-topdeck-assignment", "7-generated-effect", "9-anchor-opponent-discard"],
    status: "SUPPORTED",
    ...
    playerCounts: [2],
    moduleSets: [[]],
    completeActionFamilies: ["draw", "play-for-points", "play-for-effect", "scuttle", "exhausted-pass", "A-base-counter", "8-scuttle-counter", "J-disrupt", "K-anchor-goal-counter", "3-opponent-presentation", "3-opponent-discard", "3-caster-take", "5-rummage", "6-private-draw", "7-topdeck-assignment", "7-generated-effect", "7-scoring-trigger-take-return", "9-anchor-opponent-discard"],
    status: "SUPPORTED",
    ...
    playerCounts: [2],
    moduleSets: [[]],
    completeActionFamilies: ["draw", "play-for-points", "play-for-effect", "scuttle", "exhausted-pass"],
    status: "SUPPORTED",
    reasonCodes: ["FORMAL_FIRST_CONTACT_TEACHING_OVERRIDE", "EXPLICIT_GENERIC_EFFECT_ALLOWLIST", "RESPONSE_WINDOW_EFFECTS_EXCLUDED"]
  }]);
  let blocked = false;
  try { createMatchState({ ...autonomySetup, profileId: "first-contact" }); }
  catch (error) { blocked = /Unsupported autonomous profile/.test(String(error)); }
  assert.equal(blocked, true);
});


const responseSetup = { ...autonomySetup, profileId: "first-contact-response", seed: 0x413c0de } as const;

async function executeActionBy(engine: IntrilexEngine, state: ReturnType<typeof createEmptyState>, frame: { actions: readonly { actionId: string; command: import("../src/types.js").EngineCommand }[] }, predicate: (actionId: string) => boolean) {
...
async function passCurrentPriority(engine: IntrilexEngine, state: ReturnType<typeof createEmptyState>) {
  const { advanceToDecision } = await import("../src/index.js");
  const advanced = advanceToDecision(state);
  if (advanced.status !== "PLAYER_DECISION_REQUIRED" || !advanced.legalActionFrame) return advanced.state;
  const decline = advanced.legalActionFrame.actions.find((entry) => entry.actionId.startsWith("response-decline:"));
  return decline ? executeActionBy(engine, advanced.state, advanced.legalActionFrame, (id) => id === decline.actionId) : advanced.state;
}
...
test("4.1.3 Response Authority seals every root action behind engine-owned priority", async () => {
  ...
  for (const action of initial.legalActionFrame.actions) {
    ...
    if (action.command.type !== "RESOLVE_PHASE9_ACTION") throw new Error("unexpected command type");
  ...
  const responseFamilies = response.legalActionFrame.actions.map((entry) => entry.family);
  if (responseFamilies.includes("response-decline")) {
    assert.equal(response.decisionActorId, "P2");
    assert.ok(responseFamilies.some((family) => family !== "response-decline"), "decline is exposed only alongside a lawful response");
  } else {
    assert.ok(response.events.some((entry) => entry.type === "AUTONOMY_AUTOMATIC_PRIORITY_ADVANCE"));
    assert.ok(response.events.some((entry) => entry.type === "AUTONOMY_RESPONSE_WINDOW_CLOSED"));
  }
});
...
test("4.1.3 Base Ace counter chains preserve the root until the winning counter resolves", async () => {
  ...
  assert.equal(state.stack.length, 0, "automatic no-response orchestration resolves the surviving root");
...
test("4.1.3 Jack Disrupt records the action type, draws one, and never counters the root", async () => {
  ...
  response = advanceToDecision(state);
  state = response.state;
  assert.equal(state.stack.length, 0, "automatic orchestration resolves Jack then the original Draw");
  assert.equal(state.cards[jack]!.zone, "GY");
  assert.equal(state.players.P2!.hand.length, beforeHandCount, "Jack leaves hand and its draw replaces it");
  assert.deepEqual((state.metadata.autonomy as { disruptedActionTypesByPlayer?: Record<string, string[]> }).disruptedActionTypesByPlayer?.P1, []);
  assert.deepEqual(validateState(state), []);
});
...
test("4.1.3 Response Authority Random Legal is deterministic and terminal", async () => {
  ...
  assert.equal(hashCanonical({ state: first.state, commands: first.commands, decisions: first.decisions }), hashCanonical({ state: second.state, commands: second.commands, decisions: second.decisions }));
  assert.ok(first.decisions.some((entry) => entry.actionId.startsWith("response-decline:")) || first.events.some((entry) => entry.type === "AUTONOMY_AUTOMATIC_PRIORITY_ADVANCE"));
});
...
const privateChoiceSetup = { ...autonomySetup, profileId: "first-contact-private-choice", seed: 0x414c001 } as const;

async function resolveRootToNextDecision(engine: IntrilexEngine, state: ReturnType<typeof createEmptyState>) {
  state = await passCurrentPriority(engine, state);
  state = await passCurrentPriority(engine, state);
...
test("4.1.4 Private Choice profile enumerates engine-accepted Three, Five, Six, Seven, and Nine Anchor roots", async () => {
  ...
  for (const identity of ["3♣", "5♣", "6♣", "7♣", "9♣"]) await placeIdentity(state, identity, "P1_HAND", "P1");
  ...
  for (const expected of ["effect-private-choice:three-present-take", "effect-private-choice:three-force-discard", "effect-private-choice:five-recycle", "effect-private-choice:six-dig", "effect-private-choice:seven-topdeck", "anchor-private-choice:nine"]) assert.ok(modes.has(expected), expected);
  const engine = new IntrilexEngine();
...
test("4.1.4 every enumerated private-choice action is engine-accepted from its exact frame", async () => {
  ...
  for (let ordinal = 0; ordinal <= 10 && seen.size < required.size; ordinal += 1) {
    ...
    const policyRng = { algorithm: "xorshift32" as const, seed: ((seed ^ 0xa5a5a5a5) >>> 0) || 1, cursor: 0 };
    for (let decisionIndex = 0; decisionIndex < 5000; decisionIndex += 1) {
      const advanced = advanceToDecision(state);
      state = advanced.state;
      if (advanced.status === "TERMINAL") break;
      assert.equal(advanced.status, "PLAYER_DECISION_REQUIRED", advanced.reasonCode);
      assert.ok(advanced.legalActionFrame);
      const choice = (state.metadata.autonomy as any).privateChoice;
...
test("4.1.4 Three presentation is a sealed opponent choice followed by a caster-only take", async () => {
  ...
  const runtimeChoice = (decision.state.metadata.autonomy as any).privateChoice;
  ...
  assert.equal((result.state.metadata.autonomy as any).privateChoice, null);
...
test("4.1.4 Six private draw identities are visible only to the authorized chooser", async () => {
  ...
  assert.equal(decision.decisionActorId, "P1");
  const choice = (decision.state.metadata.autonomy as any).privateChoice;
  assert.equal(choice.kind, "rank6-dig");
  const secretIdentities = choice.context.drawnCardIds.map((id: string) => decision.state.cards[id]!.identity);
  const publicText = JSON.stringify(publicStateView(decision.state));
...
test("4.1.4 private-choice secrets never enter public state, events, or replay commands", async () => {
  ...
  const secretIdentities = secretIds.map((id: string) => decision.state.cards[id]!.identity);
...
test("4.1.4 Seven generated effect becomes a response-stack child play", async () => {
  ...
  const index = state.zones.dp.indexOf(generatedFive.id); state.zones.dp.splice(index, 1); state.zones.dp.unshift(generatedFive.id);
  ...
  let decision = await resolveRootToNextDecision(engine, state);
  assert.equal((decision.state.metadata.autonomy as any).privateChoice.kind, "rank7-assign");
  ...
  assert.equal(result.accepted, true);
  decision = advanceToDecision(result.state);
  assert.equal((decision.state.metadata.autonomy as any).privateChoice.kind, "rank7-generated-effect");
...
test("4.1.4 stale, tampered, and wrong-viewer private choice commands rewind exactly", async () => {
  ...
  if (valid.type !== "RESOLVE_PHASE9_ACTION" || valid.action.kind !== "autonomy-submit-private-choice") throw new Error("unexpected choice command");
  ...
  for (const command of [{ ...valid, id: "WRONG-VIEWER", actorId: "P1" }, { ...valid, id: "BAD-TOKEN", action: { ...valid.action, token: `00${valid.action.token.slice(2)}` } }]) {
    const result = engine.execute(decision.state, command as any);
    assert.equal(result.accepted, false);
    assert.equal(hashCanonical(result.state), before);
    assert.equal(result.events.length, 0);
  }
...
const triggerClosureSetup = { ...autonomySetup, profileId: "first-contact-trigger-closure", seed: 9 } as const;
...
async function prepareSevenScoringTrigger() {
  ...
  for (const identity of ["A♦", "8♦", "J♦", "K♦"]) await placeIdentity(state, identity, "P2_HAND", "P2");
  ...
  let triggerPriority = advanceToDecision(state);
  for (let guard = 0; guard < 12; guard += 1) {
    events.push(...triggerPriority.events);
    executedCommands.push(...triggerPriority.executedCommands);
    const choice = (triggerPriority.state.metadata.autonomy as any).privateChoice;
    if (choice?.kind === "rank7-scoring-trigger") break;
    const decline = triggerPriority.legalActionFrame?.actions.find((entry) => entry.family === "response-decline");
    if (!decline) break;
    const result = engine.execute(triggerPriority.state, decline.command);
    assert.equal(result.accepted, true);
    state = result.state;
    events.push(...result.events);
    executedCommands.push(decline.command);
    triggerPriority = advanceToDecision(state);
  }
  triggerPriority = { ...triggerPriority, events, executedCommands };
...
test("4.1.5 scoring Seven queues and flushes an engine-owned non-play trigger", async () => {
  ...
  assert.equal(triggerPriority.state.stack.length, 0);
  assert.equal((triggerPriority.state.metadata.autonomy as any).privateChoice.kind, "rank7-scoring-trigger");
...
test("4.1.5 Seven scoring trigger auto-closes when no response family is legal", async () => {
  const { engine, state, triggerPriority } = await prepareSevenScoringTrigger();
  assert.ok(triggerPriority.events.some((entry) => entry.type === "AUTONOMY_AUTOMATIC_PRIORITY_ADVANCE"));
  assert.ok(triggerPriority.events.some((entry) => entry.type === "AUTONOMY_RESPONSE_WINDOW_CLOSED"));
  const top = { id: (state.metadata.autonomy as any).privateChoice.sourceStackItemId ?? "closed-trigger" };
  const before = hashCanonical(state);
  ...
  for (const command of commands) {
    const result = engine.execute(state, command as any);
...
test("4.1.5 Seven trigger seals take/return choice and applies First Contact hidden-hand override", async () => {
  ...
  const decision = advanceToDecision(prepared.state);
  assert.equal(decision.decisionActorId, "P1");
  assert.equal((decision.state.metadata.autonomy as any).privateChoice.kind, "rank7-scoring-trigger");
...
test("4.1.5 stale, tampered, and wrong-viewer Seven trigger choices exact-rewind", async () => {
  ...
  const valid = decision.legalActionFrame!.actions[0]!.command;
  assert.equal(valid.type, "RESOLVE_PHASE9_ACTION");
  if (valid.type !== "RESOLVE_PHASE9_ACTION" || valid.action.kind !== "autonomy-submit-private-choice") throw new Error("unexpected Seven choice command");
  const before = hashCanonical(decision.state);
  const badCommands = [
  ...
  for (const command of badCommands) {
    const result = prepared.engine.execute(decision.state, command as any);
    assert.equal(result.accepted, false, command.id);
    assert.equal(hashCanonical(result.state), before, command.id);
    assert.equal(result.events.length, 0, command.id);
  }
...
test("4.2.0 Core legal action frame is engine-sound and authorized view hides commands", async () => {
  ...
  for (const entry of decision.legalActionFrame!.actions) assert.equal(engine.execute(decision.state, entry.command).accepted, true, entry.actionId);
  ...
  assert.equal(decision.legalActionFrame!.actions.some((entry) => entry.family === "pass"), false);
  for (const entry of decision.legalActionFrame!.actions) assert.equal(engine.execute(decision.state, entry.command).accepted, true, entry.actionId);
...
test("4.2.0 Core Foundation Random Legal is deterministic and terminal", async () => {
  ...
  const setup = { profileId: CORE_FOUNDATION_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"] as const, seatOrder: ["P1", "P2"] as const, enabledModules: [] as const, seed: 4242 };
...
test("4.2.1 every enumerated Core effect declaration is engine-accepted and command-vault isolated", async () => {
  ...
  for (const identity of ["A♣","3♣","4♣","4♠","J♣","J♠","Q♣","K♠","RJ","BJ"]) moveCard(state, byIdentity[identity]!, "P1_HAND", "P1");
  moveCard(state, byIdentity["2♣"]!, "P2_PR", "P2"); state.cards[byIdentity["2♣"]!]!.state.pointValue = 2;
  moveCard(state, byIdentity["K♣"]!, "P2_ER", "P2"); state.cards[byIdentity["K♣"]!]!.state.anchorValue = 7;
  ...
  for (const required of ["effect-ace","effect-three","effect-four","attachment","anchor","effect-red-joker"]) assert.equal(families.has(required), true, required);
  for (const entry of frame.legalActionFrame!.actions) assert.equal(engine.execute(frame.state, entry.command).accepted, true, `${entry.family}:${entry.mode}`);
...
test("4.2.1 Guard, Aegis, rank immunity, and Q-spade clear immunity prune effect targets before policy selection", async () => {
  ...
  let decision = advanceCoreToDecision(state); state = engine.execute(decision.state, decision.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  for (const identity of ["3♣","4♣","J♣"]) moveCard(state, by[identity]!, "P1_HAND", "P1");
  moveCard(state, by["Q♥"]!, "P2_ER", "P2"); state.cards[by["Q♥"]!]!.state.anchorValue=0;
  moveCard(state, by["4♦"]!, "P2_PR", "P2"); state.cards[by["4♦"]!]!.state.pointValue=4;
  moveCard(state, by["Q♠"]!, "P2_ER", "P2"); state.cards[by["Q♠"]!]!.state.anchorValue=0;
  moveCard(state, by["2♥"]!, "P2_PR", "P2"); state.cards[by["2♥"]!]!.state.pointValue=2;
  applyAegis(state.cards[by["2♥"]!]!, "TEST", {playerId:"P2",startSequence:99});
  decision=advanceCoreToDecision(state); const actions=decision.legalActionFrame!.actions;
  ...
  const result=engine.execute(decision.state,clear.command); assert.equal(result.accepted,true);
...
test("4.2.1 Board Lock removes effects and Scuttle while preserving allowed Core actions and expires canonically", async () => {
  ...
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_RESPONSE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42103 });
  let decision=advanceCoreToDecision(state); state=engine.execute(decision.state,decision.legalActionFrame!.actions.find((entry)=>entry.mode==="enter-action")!.command).state;
  const bj=Object.values(state.cards).find((card)=>card.identity==="BJ")!; moveCard(state,bj.id,"P1_HAND","P1");
  decision=advanceCoreToDecision(state); const lock=decision.legalActionFrame!.actions.find((entry)=>entry.mode==="board-lock")!;
  state=engine.execute(decision.state,lock.command).state;
  // Drain the response window so the Board Lock Quick stack resolves
  decision=advanceCoreToDecision(state);
  while (decision.legalActionFrame?.actions.some((entry)=>entry.family==="response-decline")) {
    state=engine.execute(decision.state,decision.legalActionFrame.actions.find((entry)=>entry.family==="response-decline")!.command).state;
    decision=advanceCoreToDecision(state);
  }
  // Board Lock active: effects, attachment, anchor, and scuttle are suppressed
  ...
  // Allowed Core actions (draw/score) are preserved and ordinary pass is not offered
  assert.equal(decision.legalActionFrame!.actions.some((entry)=>entry.family==="pass"),false);
  assert.equal(decision.legalActionFrame!.actions.some((entry)=>entry.family==="draw"||entry.family==="score"),true);
  // Advance to the next Full Turn: Board Lock remains active, suppressing effects while preserving draw/score
  const miniTurnAction=decision.legalActionFrame!.actions.find((entry)=>entry.family==="draw"||entry.family==="score")!;
  state=engine.execute(decision.state,miniTurnAction.command).state;
  // Drain the response window for the primary action, resolve the stack, and advance to the next Start phase
  decision=advanceCoreToDecision(state);
  while (decision.legalActionFrame?.actions.some((entry)=>entry.family==="response-decline")) {
    state=engine.execute(decision.state,decision.legalActionFrame.actions.find((entry)=>entry.family==="response-decline")!.command).state;
    decision=advanceCoreToDecision(state);
...
test("4.2.1 Core Effect Random Legal is deterministic, terminal, and exercises effect declarations", async () => {
  ...
  const setup = { profileId: CORE_EFFECT_DECLARATION_PROFILE.id, playerIds: ["P1", "P2"] as const, seatOrder: ["P1", "P2"] as const, enabledModules: [] as const, seed: 42104 };
...
test("4.2.2 root effects are sealed on stack and every enumerated response is engine-accepted without command-vault leakage", async () => {
  ...
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  for (const identity of ["3♣", "A♣", "8♠", "9♣", "J♣", "K♣", "Q♣"]) moveCard(state, by[identity]!, identity === "3♣" ? "P1_HAND" : "P2_HAND", identity === "3♣" ? "P1" : "P2");
  moveCard(state, by["2♣"]!, "P2_PR", "P2"); state.cards[by["2♣"]!]!.state.pointValue = 2;
  moveCard(state, by["4♣"]!, "P2_PR", "P2"); state.cards[by["4♣"]!]!.state.pointValue = 4;
  moveCard(state, by["2♦"]!, "P1_PR", "P1"); state.cards[by["2♦"]!]!.state.pointValue = 2;
  decision = advanceCoreToDecision(state);
  ...
  for (const expected of ["response-decline:decline", "counter:ace-base", "instant:eight-spade-free-scuttle", "instant:nine-tap", "disrupt:jack"]) assert.equal(families.has(expected), true, expected);
  ...
  for (const entry of response.legalActionFrame!.actions) assert.equal(engine.execute(response.state, entry.command).accepted, true, `${entry.family}:${entry.mode}`);
...
test("4.2.2 Base Ace counter-on-counter chain preserves the root effect", async () => {
  ...
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["3♣"]!, "P1_HAND", "P1"); moveCard(state, by["A♦"]!, "P1_HAND", "P1"); moveCard(state, by["A♣"]!, "P2_HAND", "P2");
  moveCard(state, by["2♣"]!, "P2_PR", "P2"); state.cards[by["2♣"]!]!.state.pointValue = 2;
  ...
  for (let i = 0; i < 16 && (state.stack.length > 0 || state.priority?.open); i++) {
    d = advanceCoreToDecision(state); events.push(...d.events.map((event) => event.type)); state = d.state;
    if (d.status === "PLAYER_DECISION_REQUIRED") {
      const pass = d.legalActionFrame!.actions.find((entry) => entry.family === "response-decline");
      if (!pass) continue;
      const result = engine.execute(state, pass.command); assert.equal(result.accepted, true); events.push(...result.events.map((event) => event.type)); state = result.state;
    }
  }
...
test("4.2.2 Jack Disrupt resolves without negating the root and records the action restriction", async () => {
  ...
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["3♣"]!, "P1_HAND", "P1"); moveCard(state, by["J♣"]!, "P2_HAND", "P2");
  moveCard(state, by["2♣"]!, "P2_PR", "P2"); state.cards[by["2♣"]!]!.state.pointValue = 2;
  d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.family === "effect-three" && entry.targetCardIds.includes(by["2♣"]!))!.command).state;
  d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.family === "disrupt")!.command).state;
  const types: string[] = [];
  for (let i = 0; i < 16 && (state.stack.length > 0 || state.priority?.open); i++) {
    d = advanceCoreToDecision(state); types.push(...d.events.map((event) => event.type)); state = d.state;
    if (d.status === "PLAYER_DECISION_REQUIRED") { const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="response-decline"); if(!pass) break; const r=engine.execute(state,pass.command); types.push(...r.events.map((event)=>event.type)); state=r.state; }
  }
  assert.equal(state.cards[by["2♣"]!]!.zone, "DP");
  assert.ok(types.includes("CORE_JACK_DISRUPT_RESOLVED"));
  const runtime = state.metadata.coreAuthority as { disruptedActionTypesByPlayer?: Record<string,string[]> };
...
test("4.2.2 audited Quick and Instant responses resolve through stack authority", async () => {
  ...
  for(const identity of ["8♠","9♣"]) moveCard(state,by[identity]!,"P2_HAND","P2");
  moveCard(state,by["4♣"]!,"P2_PR","P2"); state.cards[by["4♣"]!]!.state.pointValue=4;
  moveCard(state,by["5♣"]!,"P1_PR","P1"); state.cards[by["5♣"]!]!.state.pointValue=5;
  d=advanceCoreToDecision(state); state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.family==="draw")!.command).state;
  d=advanceCoreToDecision(state);
  ...
  assert.ok(instant); state=engine.execute(d.state,instant.command).state;
  for(let i=0;i<10&&(state.stack.length>0||state.priority?.open);i++){d=advanceCoreToDecision(state);state=d.state;if(d.status==="PLAYER_DECISION_REQUIRED"){const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="response-decline");if(!pass) break;state=engine.execute(state,pass.command).state;}}
...
test("4.2.2 malformed or non-holder responses exact-rewind with zero events", async () => {
  ...
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((card)=>[card.identity,card.id]));moveCard(state,by["3♣"]!,"P1_HAND","P1");moveCard(state,by["A♦"]!,"P1_HAND","P1");moveCard(state,by["2♣"]!,"P2_PR","P2");state.cards[by["2♣"]!]!.state.pointValue=2;
  d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.family==="effect-three")!.command).state;
  const before=hashCanonical(state), target=state.stack.at(-1)!.id;
  const wrong={id:"WRONG-ACTOR",type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:"P1",action:{kind:"core-declare-base-ace-counter",sourceCardId:by["A♦"]!,targetStackItemId:target,sourceMode:"hand"}} as const;
  const rejected=engine.execute(state,wrong as any);
  assert.equal(rejected.accepted,false);assert.equal(rejected.events.length,0);assert.equal(hashCanonical(rejected.state),before);
});
...
test("4.2.2 Core Response Random Legal is deterministic, terminal, and exercises response declarations", async () => {
  ...
  const setup={profileId:CORE_RESPONSE_AUTHORITY_PROFILE.id,playerIds:["P1","P2"] as const,seatOrder:["P1","P2"] as const,enabledModules:[] as const,seed:42206};
...
test("4.2.3 every enumerated private-choice root is engine-accepted and command-vault isolated", async () => {
  ...
  state = engine.execute(decision.state, decision.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  for (const identity of ["3♣","5♣","6♣","7♣","9♣"]) moveCard(state, by[identity]!, "P1_HAND", "P1");
  for (const identity of ["A♦","K♦","Q♦","2♦"]) moveCard(state, by[identity]!, "P2_HAND", "P2");
  decision = advanceCoreToDecision(state);
  ...
  for (const mode of ["three-present-take","three-force-discard","five-recycle","six-dig","seven-topdeck","nine"]) assert.equal(modes.has(mode), true, mode);
  for (const entry of roots) assert.equal(engine.execute(decision.state, entry.command).accepted, true, entry.mode);
...
test("4.2.3 Three enforces exact discard-to-two and permits taking up to two presented cards", async () => {
  ...
  const engine = new IntrilexEngine();
  const settleToChoice = (start: any) => {
    let state = start;
    for (let i=0;i<20;i++) {
      const d = advanceCoreToDecision(state); state = d.state;
      if ((state.metadata.coreAuthority as any)?.privateChoice) return { state, decision:d };
      if (d.status === "PLAYER_DECISION_REQUIRED") {
        const pass = d.legalActionFrame!.actions.find((entry) => entry.family === "response-decline");
        if (!pass) continue;
        const r = engine.execute(state, pass.command); assert.equal(r.accepted,true); state=r.state;
      }
    }
  ...
  for (const id of [...state.players.P2!.hand]) moveCard(state,id,"DP");
  for(const identity of ["A♦","K♦","Q♦","2♦","4♦"]) moveCard(state,by[identity]!,"P2_HAND","P2");
  d=advanceCoreToDecision(state);
  ...
  let opened=settleToChoice(state); state=opened.state; d=opened.decision;
  ...
  d=advanceCoreToDecision(state); state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="enter-action")!.command).state;
  const by2=Object.fromEntries(Object.values(state.cards).map((card)=>[card.identity,card.id]));
  moveCard(state,by2["3♣"]!,"P1_HAND","P1");
  for (const id of [...state.players.P2!.hand]) moveCard(state,id,"DP");
  for(const identity of ["A♦","K♦","Q♦"]) moveCard(state,by2[identity]!,"P2_HAND","P2");
  d=advanceCoreToDecision(state); state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="three-present-take")!.command).state;
  opened=settleToChoice(state); state=opened.state; d=opened.decision;
  const present=d.legalActionFrame!.actions.find((entry)=>entry.targetCardIds.length===3)!; state=engine.execute(state,present.command).state;
  d=advanceCoreToDecision(state); assert.equal(d.decisionActorId,"P1");
  const takeTwo=d.legalActionFrame!.actions.find((entry)=>entry.targetCardIds.length===2)!; assert.ok(takeTwo);
  const taken=[...takeTwo.targetCardIds]; state=engine.execute(d.state,takeTwo.command).state;
...
test("4.2.3 Six choices are chooser-private and public projections expose no stable hidden IDs", async () => {
  ...
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((card)=>[card.identity,card.id]));moveCard(state,by["6♣"]!,"P1_HAND","P1");
  d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="six-dig")!.command).state;
  for(let i=0;i<12;i++){d=advanceCoreToDecision(state);state=d.state;if((state.metadata.coreAuthority as any)?.privateChoice)break;const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="response-decline");if(pass)state=engine.execute(state,pass.command).state;}
  const choice=(state.metadata.coreAuthority as any).privateChoice; assert.equal(choice.kind,"core-rank6-dig");
  const secretIds=[...(choice.context.drawnCardIds as string[])]; const secretIdentities=secretIds.map((id)=>state.cards[id]!.identity);
  const publicText=canonicalize(publicStateView(state)); const opponentText=canonicalize(privateStateView(state,"P2")); const chooserText=canonicalize(privateStateView(state,"P1"));
  for(const id of secretIds){assert.doesNotMatch(publicText,new RegExp(id));assert.doesNotMatch(opponentText,new RegExp(id));}
  for(const identity of secretIdentities){assert.doesNotMatch(publicText,new RegExp(identity.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.match(chooserText,new RegExp(identity.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));}
...
test("4.2.3 Seven generated effect becomes a normal response-stack child declaration", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine(); let state=createCoreMatchState({profileId:CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42305});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((card)=>[card.identity,card.id]));
  moveCard(state,by["7♣"]!,"P1_HAND","P1");moveCard(state,by["3♦"]!,"DP");moveCard(state,by["4♦"]!,"DP");
  for(const id of [by["4♦"]!,by["3♦"]!]){const i=state.zones.dp.indexOf(id);state.zones.dp.splice(i,1);state.zones.dp.unshift(id);} // 3♦ then 4♦ on top
  moveCard(state,by["2♣"]!,"P2_PR","P2");state.cards[by["2♣"]!]!.state.pointValue=2;
  d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="seven-topdeck")!.command).state;
  for(let i=0;i<12;i++){d=advanceCoreToDecision(state);state=d.state;if((state.metadata.coreAuthority as any)?.privateChoice)break;const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="response-decline");if(pass)state=engine.execute(state,pass.command).state;}
  d=advanceCoreToDecision(state);
  const assign=d.legalActionFrame!.actions.find((entry)=>entry.mode==="rank7-hand-and-effect"&&entry.targetCardIds[1]===by["3♦"]!);assert.ok(assign);state=engine.execute(d.state,assign.command).state;
  d=advanceCoreToDecision(state);
  const generated=d.legalActionFrame!.actions.find((entry)=>entry.mode.includes("three-bounce") && (entry.command as any).action?.submission?.generatedEffect?.targetCardId===by["2♣"]!);
  assert.ok(generated); state=engine.execute(d.state,generated.command).state;
  assert.equal(state.stack.length,1);assert.equal(state.priority?.open,true);assert.equal(state.stack[0]!.sourceCardIds[0],by["3♦"]!);
});

test("4.2.3 stale, tampered, and wrong-viewer private choices exact-rewind", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine();let state=createCoreMatchState({profileId:CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42306});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((card)=>[card.identity,card.id]));moveCard(state,by["5♣"]!,"P1_HAND","P1");
  d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((entry)=>entry.mode==="five-recycle")!.command).state;
  for(let i=0;i<12;i++){d=advanceCoreToDecision(state);state=d.state;if((state.metadata.coreAuthority as any)?.privateChoice)break;const pass=d.legalActionFrame!.actions.find((entry)=>entry.family==="response-decline");if(pass)state=engine.execute(state,pass.command).state;}
  const choice=(state.metadata.coreAuthority as any).privateChoice; const selected=choice.optionCardIds[0]; const before=hashCanonical(state);
  for(const cmd of [
  ...
  ]) {const r=engine.execute(state,cmd as any);assert.equal(r.accepted,false);assert.equal(r.events.length,0);assert.equal(hashCanonical(r.state),before);}
});
...
test("4.2.3 Core Private Choice Random Legal is deterministic, terminal, and exercises sealed choices", async () => {
  ...
  const setup={profileId:CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id,playerIds:["P1","P2"] as const,seatOrder:["P1","P2"] as const,enabledModules:[] as const,seed:42307};
...
test("4.2.5 Advanced profile declares a bounded public authority surface and fail-closed exclusions", async () => {
  ...
  ]) assert.ok(CORE_ADVANCED_AUTHORITY_PROFILE.supportedFamilies.includes(family as any), family);
  for (const blocked of [
    "super-three-private", "super-five-private", "super-six-private", "super-seven-sequential",
    "rank10-generated-effect-copy", "sudden-death-autonomy", "optional-modules", "multiplayer"
  ]) assert.ok(CORE_ADVANCED_AUTHORITY_PROFILE.excludedSystems.includes(blocked as any), blocked);
  assert.equal(CORE_ADVANCED_AUTHORITY_PROFILE.engineVersion,"4.2.6");
});

test("4.2.5 advanced legal frame exposes engine-accepted Royal Marriage, Supers, Rank 10, and Ultras without command leakage", async () => {
  ...
  const engine=new IntrilexEngine(); let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42401});
  let d=advanceCoreToDecision(state); state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  for(const identity of ["K♣","Q♣","2♣","2♦","4♣","4♦","8♣","8♦","J♣","J♦","10♥","A♣","3♣","5♠","6♥","7♠"]) moveCard(state,by[identity]!,"P1_HAND","P1");
  moveCard(state,by["9♦"]!,"EXILE"); moveCard(state,by["A♦"]!,"P2_PR","P2"); state.cards[by["A♦"]!]!.state.pointValue=4;
  d=advanceCoreToDecision(state); const advanced=d.legalActionFrame!.actions.filter((x)=>["royal-marriage","super","rank10","ultra"].includes(x.family));
  const modes=new Set(advanced.map((x)=>x.mode));
  for(const mode of ["♣","two-score","four-exchange-pr","eight-absolute-scuttle","jack-tempo","heart-tempo","2-black-2-red-draw"]) assert.equal(modes.has(mode),true,mode);
  for(const entry of advanced.slice(0,80)) assert.equal(engine.execute(d.state,entry.command).accepted,true,`${entry.family}:${entry.mode}`);
  const authorized=advanced.slice(0,20).map(toAuthorizedCoreAction); assert.doesNotMatch(canonicalize(authorized),/core-resolve-advanced|sourceCardIds|command"/);
});

test("4.2.5 Stack Theft transfers resolution authority but charges only its two printed skips", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine(); let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42402});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));moveCard(state,by["3♣"]!,"P1_HAND","P1");moveCard(state,by["10♠"]!,"P2_HAND","P2");moveCard(state,by["2♣"]!,"P2_PR","P2");state.cards[by["2♣"]!]!.state.pointValue=2;
  d=advanceCoreToDecision(state);const root=d.legalActionFrame!.actions.find((x)=>x.mode==="bounce-top"&&x.targetCardIds.includes(by["2♣"]!))!;state=engine.execute(d.state,root.command).state;
  d=advanceCoreToDecision(state);const theft=d.legalActionFrame!.actions.find((x)=>x.mode==="rank10-stack-theft")!;assert.ok(theft);state=engine.execute(d.state,theft.command).state;
  for(let i=0;i<20&&state.stack.length;i++){
    if(state.priority?.open===true){d=advanceCoreToDecision(state);state=d.state;const pass=d.legalActionFrame!.actions.find((x)=>x.family==="response-decline");if(pass)state=engine.execute(state,pass.command).state;}
    else {const r=engine.execute(state,{id:`RESOLVE-42402-${i}`,type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:state.activePlayerId,action:{kind:"core-resolve-response-top"}} as any);assert.equal(r.accepted,true);state=r.state;}
  }
  assert.equal(state.stack.length,0);assert.equal(state.cards[by["10♠"]!]!.zone,"EXILE");assert.equal(state.players.P1!.limits.pendingFullTurnSkips,1);assert.equal(state.players.P2!.limits.pendingFullTurnSkips,1);assert.equal(state.players.P1!.limits.miniTurnsUsed,1);assert.equal(state.players.P2!.limits.miniTurnsUsed,0);
  const end=engine.execute(state,{id:"END-42402",type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:"P1",action:{kind:"core-complete-turn"}} as any);assert.equal(end.accepted,true);assert.equal(end.state.players.P1!.limits.pendingFullTurnSkips,0);assert.equal(end.state.players.P2!.limits.pendingFullTurnSkips,0);assert.equal(end.state.activePlayerId,"P2");
});

test("4.2.5 Stack Theft is offered only for a pending single effect play", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine();let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42412});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  ...
  const illegal=engine.execute(d.state,{id:"ILLEGAL-STACK-THEFT-SCORE",type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:"P1",action:{kind:"core-declare-rank10-stack-theft",sourceCardId:by["10♠"]!,targetStackItemId:d.state.stack.at(-1)!.id}} as any);
  assert.equal(illegal.accepted,false);
  assert.equal(illegal.error?.code,"CORE_STACK_THEFT_TARGET");
});

test("4.2.5 3 Red Ultra counters a root and preserves its bottom-GY draw rider", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine();let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42403});let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));moveCard(state,by["3♣"]!,"P1_HAND","P1");moveCard(state,by["2♣"]!,"P2_PR","P2");state.cards[by["2♣"]!]!.state.pointValue=2;for(const x of ["A♦","4♥","6♦"])moveCard(state,by[x]!,"P2_HAND","P2");state.zones.gy=[];
  d=advanceCoreToDecision(state);const rootAction=d.legalActionFrame!.actions.find((x)=>x.mode==="bounce-top"&&x.targetCardIds.includes(by["2♣"]!))!;const rootSourceId=rootAction.sourceCardIds[0]!;state=engine.execute(d.state,rootAction.command).state;d=advanceCoreToDecision(state);const ultra=d.legalActionFrame!.actions.find((x)=>x.mode==="three-red-counter")!;assert.ok(ultra);const ultraSources=[...ultra.sourceCardIds];state=engine.execute(d.state,ultra.command).state;
  for(let i=0;i<20&&state.stack.length;i++){
    if(state.priority?.open===true){d=advanceCoreToDecision(state);state=d.state;const pass=d.legalActionFrame!.actions.find((x)=>x.family==="response-decline");if(pass)state=engine.execute(state,pass.command).state;}
    else {const r=engine.execute(state,{id:`RESOLVE-42403-${i}`,type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:state.activePlayerId,action:{kind:"core-resolve-response-top"}} as any);assert.equal(r.accepted,true);state=r.state;}
  }
  assert.equal(state.stack.length,0);assert.equal(ultraSources.filter((id)=>state.players.P2!.hand.includes(id)).length,1);assert.equal(state.cards[rootSourceId]!.zone,"GY");assert.ok(["Action","End","Start"].includes(state.phase));
});

test("4.2.5 3 Red Ultra inherits Super Ace's two-Queen declaration prohibition", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine();let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42413});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  for(const x of ["Q♣","Q♠"])moveCard(state,by[x]!,"P1_ER","P1");
  for(const x of ["A♦","4♥","6♦"])moveCard(state,by[x]!,"P2_HAND","P2");
  moveCard(state,by["3♣"]!,"P1_HAND","P1");
  moveCard(state,by["2♣"]!,"P2_PR","P2");state.cards[by["2♣"]!]!.state.pointValue=2;
  d=advanceCoreToDecision(state);
  ...
  d=advanceCoreToDecision(state);
  assert.equal(d.decisionActorId,"P2");
  assert.equal(d.legalActionFrame!.actions.some((x)=>x.mode==="three-red-counter"),false);
});

test("4.2.5 Voltage exposes only the complete public Five GY-bottom branch and remains single-use", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine();
  ...
  assert.ok(d.legalActionFrame!.actions.some((x)=>x.family==="voltage"&&x.mode.startsWith("three-")),"voltage-three should be available");
  assert.ok(d.legalActionFrame!.actions.some((x)=>x.family==="voltage"&&x.mode.startsWith("four-")),"voltage-four should be available");
  ...
  d=advanceCoreToDecision(state);
  assert.equal(d.legalActionFrame!.actions.some((x)=>x.family==="voltage"&&x.mode==="five-gy-bottom"),false);
});

test("4.2.5 Advanced Random Legal is deterministic and terminal", async () => {
  const { runCoreRandomLegalMatch, CORE_ADVANCED_AUTHORITY_PROFILE } = await import("../src/index.js");
  const setup={profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"] as const,seatOrder:["P1","P2"] as const,enabledModules:[] as const,seed:42406};
  const a=runCoreRandomLegalMatch(setup,12000),b=runCoreRandomLegalMatch(setup,12000);assert.ok(["NORMAL_VICTORY","EXHAUSTED_RESOLUTION","CANONICAL_DRAW"].includes(a.terminationReason),a.terminationReason);assert.equal(hashCanonical({state:a.state,commands:a.commands,decisions:a.decisions}),hashCanonical({state:b.state,commands:b.commands,decisions:b.decisions}));assert.ok(a.events.some((e)=>e.type.startsWith("CORE_ADVANCED_")));
});

test("4.2.5 incomplete advanced declarations fail closed with exact rewind", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine();
  ...
  let d=advanceCoreToDecision(state); state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  for(const identity of ["2♣","2♦"]) moveCard(state,by[identity]!,"P1_HAND","P1");
  moveCard(state,by["3♣"]!,"P2_PR","P2"); state.cards[by["3♣"]!]!.state.pointValue=3;
  for(const advanced of [
    {kind:"advanced-super-three-raid",sourceCardIds:[by["2♣"]!,by["2♦"]!],targetPlayerId:"P2",takeCardIds:[]},
    {kind:"advanced-sudden-death-declare",targetPlayerId:"P2"}
  ]) {
    const before=hashCanonical(state);
    const r=engine.execute(state,{id:`REJECT-${advanced.kind}`,type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:"P1",action:{kind:"core-declare-primary",action:{kind:"core-resolve-advanced",advanced}}} as any);
    assert.equal(r.accepted,false); assert.equal(r.events.length,0); assert.equal(hashCanonical(r.state),before);
  }
});

test("4.2.5 special scoring riders are absent and direct declarations exact-rewind", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine(); let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42408});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  for(const identity of ["7♣","10♣","BJ"]) moveCard(state,by[identity]!,"P1_HAND","P1");
  d=advanceCoreToDecision(state);
  for(const identity of ["7♣","10♣","BJ"]) {
    ...
    const r=engine.execute(d.state,{id:`SCORE-${identity}`,type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:"P1",action:{kind:"core-declare-primary",action:{kind:"core-score",cardId:by[identity]!}}} as any);
    assert.equal(r.accepted,false); assert.equal(r.events.length,0); assert.equal(hashCanonical(r.state),before);
  }
});
...
test("4.2.5 Super Eight prunes object Aegis and Super Four preserves tap state while replacing Aegis", async () => {
  ...
  const engine=new IntrilexEngine(); let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42409});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  for(const identity of ["8♣","8♦","4♣","4♦"]) moveCard(state,by[identity]!,"P1_HAND","P1");
  moveCard(state,by["6♣"]!,"P2_PR","P2"); state.cards[by["6♣"]!]!.state.pointValue=6;
  applyAegis(state.cards[by["6♣"]!]!,"OBJECT-AEGIS",{playerId:"P2",startSequence:9});
  moveCard(state,by["9♣"]!,"P1_PR","P1"); state.cards[by["9♣"]!]!.state.pointValue=9; state.cards[by["9♣"]!]!.state.tapped=true; state.cards[by["9♣"]!]!.state.tapState={kind:"explicit-event",sourceRef:"KEEP-TAP",eventKey:"SCORE"};
  d=advanceCoreToDecision(state);
  assert.equal(d.legalActionFrame!.actions.some((x)=>x.mode==="eight-absolute-scuttle"&&x.targetCardIds.includes(by["6♣"]!)),false);
  const exchange=d.legalActionFrame!.actions.find((x)=>x.mode==="four-exchange-pr"); assert.ok(exchange);
  state=engine.execute(d.state,exchange.command).state;
  for(let i=0;i<20&&state.stack.length;i++){
    if(state.priority?.open){d=advanceCoreToDecision(state);state=d.state;const pass=d.legalActionFrame!.actions.find((x)=>x.family==="response-decline");if(pass)state=engine.execute(state,pass.command).state;}
    else {const r=engine.execute(state,{id:`RESOLVE-EXCHANGE-${i}`,type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:state.activePlayerId,action:{kind:"core-resolve-response-top"}} as any);assert.equal(r.accepted,true);state=r.state;}
  }
...
test("4.2.5 Jack declarations prune already Jacked hosts and exact-rewind direct attempts", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_ADVANCED_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine=new IntrilexEngine(); let state=createCoreMatchState({profileId:CORE_ADVANCED_AUTHORITY_PROFILE.id,playerIds:["P1","P2"],seatOrder:["P1","P2"],enabledModules:[],seed:42410});
  let d=advanceCoreToDecision(state);state=engine.execute(d.state,d.legalActionFrame!.actions.find((x)=>x.mode==="enter-action")!.command).state;
  const by=Object.fromEntries(Object.values(state.cards).map((c)=>[c.identity,c.id]));
  moveCard(state,by["J♣"]!,"P1_HAND","P1"); moveCard(state,by["J♦"]!,"P2_ER","P2"); moveCard(state,by["A♦"]!,"P2_PR","P2");
  ...
  state.cards[by["A♦"]!]!.state.attachedByJackId=by["J♦"]!; state.cards[by["A♦"]!]!.state.jackPointBonus=1;
  d=advanceCoreToDecision(state); assert.equal(d.legalActionFrame!.actions.some((x)=>x.mode==="jack-pr"&&x.targetCardIds.includes(by["A♦"]!)),false);
  const before=hashCanonical(d.state); const r=engine.execute(d.state,{id:"DOUBLE-JACK",type:"RESOLVE_CORE_AUTHORITY_ACTION",actorId:"P1",action:{kind:"core-declare-primary",action:{kind:"core-resolve-effect",effect:{kind:"jack-attach",sourceCardId:by["J♣"]!,targetCardId:by["A♦"]!,row:"pr"}}}} as any);
  assert.equal(r.accepted,false);assert.equal(r.events.length,0);assert.equal(hashCanonical(r.state),before);
});

test("Solo Wild Copy: 2♥ copying 3♥ bounce resolves correctly and 2 goes to GY", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42501 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♥"]!, "P1_HAND", "P1");
  moveCard(state, by["5♣"]!, "P2_PR", "P2"); state.cards[by["5♣"]!]!.state.pointValue = 5;
  const before = hashCanonical(state);
  const r = engine.execute(state, { id: "SWC-BOUNCE", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"]!, targetRank: "3", copiedAction: { kind: "three-bounce", targetCardId: by["5♣"]! } } } as any);
  assert.equal(r.accepted, true);
  assert.equal(r.state.cards[by["2♥"]!]?.zone, "GY");
  assert.equal(r.state.cards[by["5♣"]!]?.zone, "DP");
  assert.ok(r.events.some((e) => e.type === "SOLO_WILD_COPY_RESOLVED"));
});

test("Solo Wild Copy: 2♠ copying 4♠ total-clear resolves correctly", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42502 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♠"]!, "P1_HAND", "P1");
  moveCard(state, by["5♣"]!, "P2_PR", "P2"); state.cards[by["5♣"]!]!.state.pointValue = 5;
  moveCard(state, by["6♦"]!, "P1_PR", "P1"); state.cards[by["6♦"]!]!.state.pointValue = 6;
  const r = engine.execute(state, { id: "SWC-TOTAL-CLEAR", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♠"]!, targetRank: "4", copiedAction: { kind: "total-clear" } } } as any);
  assert.equal(r.accepted, true);
  assert.equal(r.state.cards[by["2♠"]!]?.zone, "GY");
  assert.equal(r.state.cards[by["5♣"]!]?.zone, "GY");
  assert.equal(r.state.cards[by["6♦"]!]?.zone, "GY");
});

test("Solo Wild Copy: 2♥ cannot copy 4♠ total-clear (suit mismatch)", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42503 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♥"]!, "P1_HAND", "P1");
  const before = hashCanonical(state);
  const r = engine.execute(state, { id: "SWC-SUIT-FAIL", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"]!, targetRank: "4", copiedAction: { kind: "total-clear" } } } as any);
  assert.equal(r.accepted, false);
  assert.equal(hashCanonical(r.state), before);
});

test("Solo Wild Copy: 2♥ copying 5♥ recycle resolves and 2 goes to GY", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42504 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♥"]!, "P1_HAND", "P1");
  const r = engine.execute(state, { id: "SWC-RECYCLE", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"]!, targetRank: "5", copiedAction: { kind: "recycle-five" } } } as any);
  assert.equal(r.accepted, true);
  assert.equal(r.state.cards[by["2♥"]!]?.zone, "GY");
  assert.ok(r.events.some((e) => e.type === "SOLO_WILD_COPY_RESOLVED"));
});

test("Solo Wild Copy: 2♥ copying 7♥ topdeck resolves and 2 goes to GY", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42505 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♥"]!, "P1_HAND", "P1");
  const r = engine.execute(state, { id: "SWC-TOPDECK", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"]!, targetRank: "7", copiedAction: { kind: "topdeck-seven" } } } as any);
  assert.equal(r.accepted, true);
  assert.equal(r.state.cards[by["2♥"]!]?.zone, "GY");
  assert.ok(r.events.some((e) => e.type === "SOLO_WILD_COPY_RESOLVED"));
});

test("Solo Wild Copy: does not create ⭐2 (not a commandeer play)", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42506 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♥"]!, "P1_HAND", "P1");
  moveCard(state, by["5♣"]!, "P2_PR", "P2"); state.cards[by["5♣"]!]!.state.pointValue = 5;
  const r = engine.execute(state, { id: "SWC-NOT-STAR-2", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"]!, targetRank: "3", copiedAction: { kind: "three-bounce", targetCardId: by["5♣"]! } } } as any);
  assert.equal(r.accepted, true);
  // The 2 should go to GY (consumed), not to PR (which would happen if it were scored as ⭐2)
  assert.equal(r.state.cards[by["2♥"]!]?.zone, "GY");
  // No COMMANDEER_RESOLVED event should be emitted
  assert.equal(r.events.some((e) => e.type === "COMMANDEER_RESOLVED"), false);
});

test("Solo Wild Copy: 2♥ copying 4♥ row-clear clears opponent PR cards", async () => {
  const { advanceCoreToDecision, createCoreMatchState, CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE, moveCard } = await import("../src/index.js");
  const engine = new IntrilexEngine();
  let state = createCoreMatchState({ profileId: CORE_PRIVATE_CHOICE_AUTHORITY_PROFILE.id, playerIds: ["P1", "P2"], seatOrder: ["P1", "P2"], enabledModules: [], seed: 42507 });
  let d = advanceCoreToDecision(state); state = engine.execute(d.state, d.legalActionFrame!.actions.find((entry) => entry.mode === "enter-action")!.command).state;
  const by = Object.fromEntries(Object.values(state.cards).map((card) => [card.identity, card.id]));
  moveCard(state, by["2♥"]!, "P1_HAND", "P1");
  moveCard(state, by["3♣"]!, "P2_PR", "P2"); state.cards[by["3♣"]!]!.state.pointValue = 3;
  moveCard(state, by["6♦"]!, "P2_PR", "P2"); state.cards[by["6♦"]!]!.state.pointValue = 6;
  const r = engine.execute(state, { id: "SWC-ROW-CLEAR", type: "RESOLVE_RANK_ACTION", actorId: "P1", action: { kind: "solo-wild-copy", sourceCardId: by["2♥"]!, targetRank: "4", copiedAction: { kind: "four-row-clear", targetPlayerId: "P2", row: "pr" } } } as any);
  assert.equal(r.accepted, true);
  assert.equal(r.state.cards[by["2♥"]!]?.zone, "GY");
