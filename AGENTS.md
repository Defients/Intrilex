...
- **Match server URL config:** `apps/lab-web/src/play/network/match-server-config.js` — reads `window.__INTRILEX_CONFIG__.matchServerUrl` (build-time injected via `INTRILEX_MATCH_SERVER_URL` env var), falls back to localStorage override, then dev fallback `ws://localhost:3099` (localhost only). Returns `null` in production without config → lobby shows "Online Duel Unavailable" screen.
- **Build-time injection:** `scripts/bundle.mjs` injects `matchServerUrl` into `__intrilex-config.js` and adds the WSS URL to CSP `connect-src`. `scripts/dev-server.mjs` injects `ws://localhost:3099` in dev mode.
